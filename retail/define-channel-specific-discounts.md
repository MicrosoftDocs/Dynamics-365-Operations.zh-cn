...
零售商通常提供不同的折扣在不同的通道。 这可以刷新已本地的市场条件或处理的竞争经销商。

零售和商务在 Microsoft Dynamics 365 中的工序使用价格组定义通道特定的折扣。 价格组可分配给以下一个或多个实体：渠道、目录、隶属关系和会员计划。 本文讨论渠道，不过，相同的概念适用于目录折扣、隶属关系折扣和会员折扣。

## <a name="price-groups"></a>价格组
\[标题 id= "附件\_\["align= "”alignnone width= "\_"\][] (![价格组。/media/price 组 1024x608.png)](。/media/price-groups.png) 价格组用于\[零售/caption\]链接

上半图说明可能针对交易记录的实体之间的关系 (通道、目录、隶属关系、客户，会员卡) 和适用多种折扣类型可配置。 所有交易记录。通道时，通道，以便确保则交易记录上仍然存在该 剩余的实体是可选的。 在每个主数据页上，都有一个指向相关价格组的链接，您可以根据需要在其中查看和添加价格组。 价格组用于关联实体到最多四种不同折扣、价格调整和贸易协议。 我们建议您为您如何为您计划的策略的名称使价格组将它们组织。 一个选项是使用字母或数字前缀、后缀区分不同之间。 例如，通道价格组的价格组的 1-xxxxx 目录和 2-xxxxx。 有四个查询页侧重于每个可以具有与其关联的折扣的零售实体。
