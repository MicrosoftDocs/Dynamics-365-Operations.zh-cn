---
title: "收据模板和打印"
description: "本文介绍如何修改窗体布局以控制如何打印收据、发票和其他单据。 工序的 Microsoft Dynamics 365 -使用零售包括您可以轻松地创建和修改窗体布局以不同类型的窗体布局设计器&quot;。"
author: josaw1
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
本文介绍如何修改窗体布局以控制如何打印收据、发票和其他单据。 工序的 Microsoft Dynamics 365 -使用零售包括您可以轻松地创建和修改窗体布局以不同类型的窗体布局设计器"。

**重要：**您必须打印收据及其他单据从 Retail Modern POS 和覆盖 POS 的设置窗体布局和收据模板。 您可以包含在收据模板的多个窗体布局。 然后可以通过修改硬件配置文件将收据模板分配到打印机。

## <a name="set-up-a-receipt-format"></a>设置收据形式
1.  **单击零售和商务** &gt; **的设置渠道** &gt; **的设置 POS ** &gt; ** POS ** &gt; **格式**收货。
...
4.  在出现在 Internet Explorer 窗口底部的通知栏上，单击**打开**开始安装一键式设计器。 通知 (栏可以在其他浏览器中的其他位置中。) 进度指示器显示安装过程的进度。
5.  在安装完成后，请输入自己的操作的用户名和密码，然后单击** Dynamics 365 请登录**启动设计器。
...
1.  通过单击**零售和商务** **的设置渠道 &gt; 打开硬件配置文件设置。** &gt; **的 POS ** &gt; ** POS 模板** &gt; ** **硬件配置文件。
