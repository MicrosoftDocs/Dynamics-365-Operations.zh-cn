---
title: "配置和筛选工作区"
description: "本文提供有关如何配置和筛选工作区的概览。"
author: jasongre
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
ms.prod: 
ms.service: Dynamics365Operations
ms.technology: 
audience: Application User
ms.search.scope: AX 7.0.0, Operations, Core
ms.custom: 17491
ms.assetid: 541e6012-4680-4684-8494-e9b5ca4684ee
ms.search.region: Global
ms.author: jasongre
ms.search.validFrom: 2016-02-28
ms.dyn365.ops.version: AX 7.0.0
translationtype: Human Translation
ms.sourcegitcommit: af071e009cd3f5edd918a6ab2d6d4c0119900a4e
ms.openlocfilehash: 30d86674bce2ed65b0ccdac0862f208b02b02e5c
ms.lasthandoff: 03/31/2017


---

# <a name="configure-and-filter-workspaces"></a>配置和筛选工作区

本文提供有关如何配置和筛选工作区的概览。

<a name="configuring-a-workspace"></a>配置工作区
-----------------------

您可以通过更新应用于整个工作区的设置来更改一些工作区的外观和行为。 在可以配置工作区时，“操作”窗格包括指示您单击它进行配置更改的按钮。 例如，在下图中，按钮称为**配置我的工作区域**。 

[![配置和筛选器] (工作区。/media/configure 和筛选器 workspaces.png)](。/media/configure 和筛选器 workspaces.png)   

在单击按钮后，一个对话框将显示，从中可以修改该工作区的预定义设置。 此对话框中显示的特定设置因工作区而异，具体取决于工作区中可用的特定控件和业务数据。 

我![[] (配置工作区。/media/configure 我 workspace.png)](。/media/configure 我 workspace.png)

## <a name="filtering-a-workspace"></a>筛选工作区
许多工作区可以筛选在其中显示的内容。 可用控件可以筛选工作区的所有内容或工作区特定部分的内容。 工作区上的筛选器可能是查找、组合框、自由格式文本字段或其他控件类型。 但是，筛选器的每种类型具有相同的效果，如以下各节中所述。

### <a name="workspace-wide-filters"></a>工作区范围的筛选器

您可以使用工作区范围的筛选器来筛选整个工作区。 工作区范围的筛选器将出现在工作区的左上角。 在下拉列表中选择某个特定值时，将基于所选内容筛选工作区的内容。 

[] (![工作区筛选器。/media/workspace-filter.png)](。/media/workspace-filter.png) 

当单击以打开筛选器时，将为您显示多个选项。 

[工作区的![] (扩展筛选器。/media/workspace 筛选器 expanded.png)](。/media/workspace 筛选器 expanded.png) 

选择一个选项来基于该选项筛选工作区。

### <a name="workspace-section-filters"></a>工作区部分筛选器

如果工作区的各个部分具有筛选器，您可以单独筛选每个部分。 在下图中，筛选器（包含文本“Filter”的字段）是自由格式文本字段筛选器的示例。 

工作区![[] (部分筛选器。/media/workspace 部分 filters.png)](。/media/workspace 部分 filters.png) 

正如工作区范围的筛选器一样，在用于入筛选部分内容的字段中选择或输入值。


