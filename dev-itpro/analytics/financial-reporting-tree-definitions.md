...
-   首先，确定法人或公司所需的报告维度。
-   考虑如何设置结构，然后绘制您公司的组织结构图。 组织结构图将帮助您可视化将报告单位组合到一个或多个报告结构树的方式。
...
| 行定义       | 报告单位的行定义的名称。 对报告结构树的每个单位使用相同的行定义。 在生成报表时，此行定义将用于每个报告单位。 行定义可以包括多个财务维度链接。 如果在报告结构树中指定行定义，则选择报表定义的**报表**选项卡上的**使用报告结构树中的行定义**复选框。|
| 行链接              | 要用于报告单位的行链接。 为行定义定义行链接以标识要链接到的财务维度。|
...
您可以通过在图形视图中移动报告单位来重新组织报告结构树定义的组织结构。 您还可以在报告结构树中将报告单位提升到更高的级别，或将其降到更低级别。
