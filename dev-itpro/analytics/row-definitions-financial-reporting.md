...
-   指向财务数据的链接，在 Microsoft Dynamics 365 中可以包括维度值：**工序的注释**您可以设置一个行定义每次从财务维度系统中提取数据生成报表。
-   基于链接的财务数据的行汇总和公式
...
| 选项                | 说明                                                                                                                                                                                                                                                                      |
|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 维度             | 标识要添加到行定义的维度的模式。 此模式可包含与"号 (&) 或数字记号 (\#) 在维度中的每个位置的。 通常，对“主科目”维度使用所有与号，对其他维度使用所有数字符号。 |
...
4.  要将行定义限制到特定维度值范围，请在**维度范围起始值**单元格中输入起始维度值，然后在**维度范围结束值**单元格中输入结束维度值。 要包括所选维度的所有值，请将这些单元格保留为空。 **注释：**通配符 (?)\* 或维度范围可能并不的具体取决于 ERP 数据库如何返回您需要的所有结果，逐份打印数据。
5.  在**起始行代码**字段中指定要添加到行定义的第一个维度值的行代码。
6.  在**每行的增量**字段中指定连续的行代码之间的间距。 例如，如果第一个行代码为 100 和增量值为 30，则前具有新行代码 100，130，160，190 和 220。 使用提供足够的空间插入新格式和公式行的间隔的增量值。
...
3.  单击格式的工具栏按钮以应用。 例如，若要缩进某个行，请选择该行，然后单击工具栏**增加缩进**! [] (增加缩进 https://i-technet.sec.s-msft.com/dynimg/IC679497.gif“增加缩进”) 在工具栏上。
