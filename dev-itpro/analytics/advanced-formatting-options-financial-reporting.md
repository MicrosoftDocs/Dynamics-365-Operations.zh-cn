...
在设计报表时，您应先创建所有细节行以确保按预期方式拉入值。 然后添加 **NP**（不打印）格式覆盖以禁止显示包含最终值的详细信息。 **重要信息：**当您在行定义中使用 **CAL** 格式代码时，您无法深化到交易记录详细信息。 强制的，配方使用以下格式：&lt;目标列&gt;提供&lt;column.row 代码的&gt;&lt;=用&gt; 逗号和空格分隔该行的其他任何放置位置。 下面是一个示例：D=C.190,E=C.100
...
|----------|----------------------------------|-------------|-----------------------------|-----------------|----------------|---------------|--------------------|----------------------------|------------------------------|
| 100      | 期初的现金 (NP) |             |                             |                 |                |               |                    | 会计科目修饰符 \[= /BB\] | +Segment2 = \[\]         |
...
| 130      |                           |             |                                 |                      |                |               |                    |              |                                            |
| 190      | 美国销售额                  |             |                                 |                      | C              |               |                    |              | +Segment2 = \[\*\]，Segment3 = \[41\]    |
| 220      | 国际销售额       |             |                                 |                      | C              |               |                    |              | +Segment2 = \[\*\]，Segment3 = \[01:99\] |
| 250      |                           |             |                                 |                      |                |               |                    |              |                                            |
| 280      |                           |             |                                 |                      |                |               |                    |              |                                            |
...
行定义中的计算公式可以包括+** ** **，-** **，\*** **和运算符，并/** ** IF/THEN/ELSE **报表。 此外，计算还可能涉及各个单元格和绝对金额（公式中包含的实际数字）。 此公式最多可包含 1,024 个字符。 计算不能应用于包含**“链接到财务维度”**(FD) 类型的单元格的行。 但是，您可以包含对连续的行的计算，禁止显示这些行的打印，然后将计算行合计在一起。

### <a name="operators-in-a-calculation-formula"></a>计算公式中的运算符

计算公式使用的运算符比行合计公式的更复杂。 但是，您可以将\*乘以 () 和除以 (/) 金额的其他运算符。使用\*** ** **和/**运算符。 要在计算公式中使用范围或合计，则必须在任何行代码前使用与号 (@)，除非您在行定义中使用了列。 例如，若要将第 100 行中的金额加到第 330 行中的金额，你可以使用行总计公式 100+330 ** ** **或计算公式@100+@330**。 **注意：**您必须在您在计算公式中使用的每个行代码前使用与号 (@)。 否则，该数字将作为绝对金额读取。 例如，公式**@100添加 +330 ** USD@100到 330 行中的金额。 当您引用计算公式中的列时，不需要使用 at 符号 (@)。
...
在此示例中，计算公式@100+@330** **表示第@100行中的金额添加到第@330行中的金额。 行总计公式 340+370 ** **若要将第 340 行中的金额加到第 370 行中的金额。 (370 行中的金额来自计算公式中的金额。)
...
当行定义中的行的格式代码为 **CAL**，并且您在**“相关配方/行/单位”**单元格中输入了数学计算时，您还必须输入与报表上的相关列和行的字母。 例如，输入** ** A.120 表示 A 列，第 120 行。 或者，可以使用 at 符号 (@) 指示所有列。 例如，输入@120** **表示第@120行中的所有列。 不包含列字母或 at 符号 (@ 所有的数学计算假定为实数。) **注释：**，如果您使用一个标签行代码来引用行，则必须使用。) 作为期间 (在两列字母和标签之间的分隔符一 (例如，A.GROSS\_** MARGIN/A.SALES **)。 如果将 at 符号 (@)，不要求 (分隔符，例如@GROSS\_**MARGIN/@SALES**)。
...
| B=@130\*@130            | 同一计算仅对列 B 执行。                                                                      |
| A，B，C= (@100/@130)\*@100/ | A= (A.100/A.130)\*B= (B.100/B.130)\*.75 C= (C.100/C.130)\*\*                                                           |

### <a name="ifthenelse-statements-in-a-row-definition"></a>行定义中的 IF/THEN/ELSE 语句

**IF/THEN/ELSE** 报表可添加到任何有效计算并且可与 **CAL** 格式一起使用。 您应在**“相关配方/行/单位”**列的单元格中输入**“IF/THEN/ELSE”**计算公式。 ** ** IF/THEN/ELSE 计算公式使用以下格式：如果 &lt;类型&gt; 的伪 True/配方&gt; 供词然后 &lt;ELSE &lt;公式&gt; **&gt;配方**报表的 &lt;其他部分是可选的。
...
-   **，如果 A.2000&gt;** (简单计算)
-   **，如果 A.2000&gt;和 000，A.20010&lt;** (复杂语句)
-   **，如果 A.20010000&gt;(或 A.340/B.1200)\*&lt;1200)** (包含多个表达式的复杂语句)
...
**THEN** 和 **ELSE** 公式可以是任何有效的计算 - 从非常简单的值分配到复杂的公式。 例如，如果 A.2000&gt;**报表，然后 A=B.200 **“，则意味着，在单元格中的值在列 A 行 200 多 0 (0)，在列 B 放从单元格中的值。第 200 行到在当前行的 A 列的单元格”。 前面的 **IF/THEN** 语句会将值放入当前行的一个列。 但是，您还可以在 true/false 评估或公式中使用与号 (@) 来表示所有列。 下面是以下章节中所述的一些其他示例：

-   **如果 A.200 &gt;0 个，然后** B.200:如果单元格 A.200 中的值为正数，则会将单元格 B.200 中的值。放放到当前行的每一列中。
-   **如果 A.200 &gt;0，然后 @200**:如果单元格 A.200 中的值为正数，则会将每列第 200 行中的值。放放到当前行的相应列中。
-   **如果 @200&gt;@200，然后 @200**:如果当前列的第 200 行中的值为正数，则会将第 200 行中的值。放放到当前行的同一列中。

### <a name="restricting-a-calculation-to-a-reporting-unit-in-a-row-definition"></a>将计算限制到行定义中的报告单元

若要将计算限制为报告树中的一个或多个，报告单位，以便所得金额不会汇总到更高级别的单位，您可以在行定义的**相关的公式/行/单位**单元格可以使用@Unit** **代码。 **@Unit**代码在报告树的列 B 列出，** **单位名称。 在您使用@Unit** **代码时，不会汇总这些值，不过，该评估计算。各级报告树。 **注意：**要使用此功能，则必须将报告结构树与行定义关联。 该计算行可引用某个计算行或财务数据行。 该计算在行定义的**“相关配方/行/单位”**单元格和财务数据类型限制中记录。 计算必须使用以开始**，如果 @Unit**构造的状况的计算。 该示例是：如果 @Unit(销售) @100 @Unit此计算只在报表的每个列中然后从第 @100 行中的金额，但是，销售单位。 如果有多个单位名为 SALES，金额将以所有这些单位显示。 此外，行 100 可以是财务数据行，也可以定义为非打印。 在这种情况下，系统将阻止金额以树中的所有单位显示。 您还可以将金额限制到报表的单个列（如列 H），方法是使用列限制仅打印报表的该列中的值。 您可以在 **IF** 语句中包含 **OR** 组合。 该示例是：如果 @Unit(销售) 或 @Unit@Unit) (SALESWEST 其他您在一 @100 计算类型限制使用以下方法之一向然后可以指定单位：

-   输入单位名称以包含匹配的单位。 **，例如，如果 @Unit(销售) **称作启用销售中的任何单位的计算，即使在报告树的"销售单元。
-   输入公司和单位名称以将计算限制到特定公司内的特定单位。 例如，输入**，如果 @Unit(尖端：**销售) 销售单元中将计算限制到尖端公司中。
-   输入报告结构树中的完整层次结构代码以将计算限制到特定单位。 例如，输入**，如果 @Unit(SUMMARY^ACME^WEST COAST^SALES **)。 **注意：**要查找完整层次结构代码，请右键单击报告结构树定义，然后选择**“复制报告单位标识符(硬编码)”**。

#### <a name="restrict-a-calculation-to-a-reporting-unit"></a>将计算限制到报告单位

1.  在报表设计器中，单击**“行定义”**，然后打开行定义以修改。
2.  双击**“格式代码”**单元格，然后选择**“CAL”**。
3.  **单击"相关的公式/行/单位**单元格，然后输入从开始**，如果 @Unit**构造的状况的计算。

### <a name="ifthenelse-statements-in-a-column-definition"></a>列定义中的 IF/THEN/ELSE 语句

**IF/THEN/ELSE** 语句可使所有计算依赖于来自任何其他列的结果。 您可以引用其他列，但不能引用 **IF** 语句中的报表单元格。 任何计算都必须应用于整个列。 **例如，报表 B100&gt;然后，如果 B 其他 C\***\*美元，“，如果列" B "中的金额 1.25 多放，从列 B 中的值为** ** " CALC "列。 如果列 B 中的金额不大于 100，则将列 C 中的值乘以 1.25，并将结果放入 **CALC** 列。” 始终在 **IF** 语句后面跟上可评估为 true 或 false 的逻辑语句。 用于 **THEN** 语句和 **ELSE** 语句的公式可包含对任意数量的列的引用，并且您可以根据需求设置这些公式的复杂度。 **注意：**您不能将计算的结果放入任何其他列。 结果必须在包含公式的列中。
