...
-   列定义的标题区域包含报表中将出现的标题文本和格式。 标题应用于单个数据跨越多个列，或可以应用于某一条件应用于列。 列定义可包含任意多个列标题行，您需要。 **注意：**列标题将应用于报表上的每列数据。 报表标题将应用于整个报表。 在报表定义的**“页眉和页脚”**选项卡上定义报表标题。
...
| 列限制代码 | 描述                                                                                                                                                                                                                                                                                                                             |
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SU                      | 取消或下划线命令 (列的下划线**---**) 或双下划线命令 (===** **) 行定义中输入。 例如，您可能不希望对百分比计算得出的金额加下划线。                                                                        |
...
-   如果您指定重叠的列标题，则列标题将在打印到报表上时重叠。 生成报表时，显示以下警告，但在**报表队列状态**字段显示：“使用 Base 的列标题与其他列标题相交，可能会造成重叠的文本”。 例如，B 列的标题定义为" B 到 BASE+1，并且在 D 列中的标题定义在这种情况下为 BASE+1 到 F.，标题打印{{在：on_top_of}}彼此顶部并且是不可读取的。 在**“展开开始范围/展开结束范围”**定义中使用 BASE 时，务必查看生成的报表以查看标题是否重叠。
-   如果您在“非打印”(**NP**) 列中的展开定义中指定 BASE，则将忽略它，而不管列定义中的内容。 本质上，此方案等同于不创建列标题定义。
-   对于条件性打印列 (铅&lt;** ** **，P=B&gt;**)，条件性跨越标题正常运行与任何常规列标题定义。 例如，如果条件为 False，则匹配展开条件的所有后续列将开始打印标题。
...
| 打印控制       |      | P=B&lt;       | 铅&gt;        | P=B&lt;       | 铅&gt;        | P=B&lt;       | 铅&gt;        | P=B&lt;       | 铅&gt;        | P=B&lt;       | 铅&gt;        | P=B&lt;       | 铅&gt;        |
...
3.  在列表中选择列类型。 下表描述各种列类型。
    ...
    <tr class="odd">
    <td>ATTR</td>
    <td>如果您的会计系统支持属性，则将在列中显示科目或交易记录属性。 必须应用于单个完整科目的属性从财务数据中提取基础科目或交易记录信息。 会计科目级属性显示会计科目中的数据，交易记录级属性显示在生成的数据过帐交易记录。 如果选择 <strong>" ATTR "</strong> 作为列类型，请在 <strong>帐簿代码/属性类别</strong> 列定义中指定了一个属性类别的详细信息行。</td>
    </tr>
    </tbody>
    </table>
...
|---------|------------------------------------------------------------------------------------------------------------------------------|
| BASE    | 使用报表时间中指定的基准年。                                                                          |
| \#BASE+ | 使用不同 \# 年在基准年度之后的年度。 例如，要使用基准年之后的第三年，请输入 **BASE+3**。 |
| 基于\# | 使用不同 \# 年在基准年度前的年度。 例如，要使用上一年，请输入 **BASE-1**。                 |
| \#      | 输入实际会计年度。                                                                                                |
...
|-----------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| BASE            | 使用基准期间。                                                                                                                                                                                                                 |
| \#BASE+         | 使用基准期间之后 \# 是期间的期间。 例如，要使用基准期间之后的第三个期间，请输入 **BASE+3**。                                                                                               |
| 基于\#         | 使用不同 \# 期间期间之前到基准期间的期间。 例如，要使用上一个期间，请输入 **BASE-1**。                                                                                                                 |
| 基于\#: 基础    | 使用多个期间，从基准期间之前的多个期间到基准期间。 例如，要使用之前的三个期间和基准期间，请输入 **BASE-3:BASE**。                                                |
| 基础：\#BASE+    | 使用多个期间，从基准期间到基准期间之后的多个期间。 例如，要使用基准期间和之后的两个期间，请输入 **BASE:BASE+2**。                                                  |
| 基本\#:\#BASE+ | 使用多个期间，从基准期间之前的多个期间到基准期间之后的多个期间。 例如，要使用之前的三个期间、基准期间和后面的两个期间，请输入 **BASE-3:BASE+2**。 |
| 1:BASE          | 使用多个期间，从第一个期间到基准期间。                                                                                                                                                                 |
| \#              | 请始终使用具体期间编号。 我们建议您不要使用此选项，因为它会降低列定义的灵活性。                                                                                       |
| \#                                      : \#           | 始终使用具体期间范围。 我们建议您不要使用此选项，因为它会降低列定义的灵活性。                                                                                    |

您可超出任何期间规范中的会计年度边界，并且可在期间范围中混合年度。 例如，您指定期间** BASE-5 ** (代表过去) 运行具有六期间和基准期间。2. 的报表。 在这种情况下，报表显示指定会计年度的前两个期间和上一个会计年度的后四个期间。

### <a name="specify-the-periods-for-an-fd-column"></a>指定 FD 列的期间

1.  在报表设计器中，打开要修改的列定义。
2.  在 **FD** 列中，双击**“期间”**行中的单元格，然后选择列表中的选项。
3.  在导航窗格上方的公式栏中或**“期间”**单元格中，填写公式。 被相应的值替换所有数字符号 (\#)。
...
维度筛选器用于将 **FD** 列限制到特定维度值。 筛选器可包含单个维度、一系列维度或一组维度。 筛选器还可包括维度值集。 由于维度值可能会变化，"。财务维度\ \基于维度的系统不必都对应于一个清除的长度。 将应用筛选器，不管报表是否包含报告结构树都是如此。 您可以在任何位置使用通配符 (?或\* )。 当您指定多个会计科目时，请在会计科目之间个逗号，如以下示例所示：+Account=\[\]，+Account=\[1200\]，Department=\[\]?\] 要接收特定科目的所有部门，您可从维度筛选器中排除“部门”维度。 例如，将以相同方式处理以下两个维度筛选器：

-   +Account=\[\]，部门
-   +Account=\[\]

您还可使用字母数字字符的任何组合进行精确匹配，并且可定义部分维度。 **例如，位置 \[= \[\*\]**. 开始包含所有以 10 开头的位置维度值。
...
-   ** **指定币种显示–显示交易记录的货币类型 (自然、功能性，报告)。 此功能有时候称作币种转换。 币种转换是一种功能，采用可能不是公司的功能币种或输入交易记录时采用的币种报告总帐。
...
**注意：**此处介绍的选项可能根据 ERP 系统变化。 有关详细信息，请参阅您 Microsoft [] (https://www.microsoft.com/en-us/download/details.aspx?id=5916) ERP 系统文档。
...
| 公司信息中的功能币种 | 空                | ** $2,310\*–\***结果在指定公司信息中的功能性货币显示所有数据。 **注意：**此金额是采用功能币种的所有交易记录之和。 |
| 自然/原始币种                 | 空                | **$2,250** – 结果将以执行交易记录时采用的币种显示所有金额。                                                                                                 |

### <a name="calculation-column-in-a-column-definition"></a>列定义中的计算公式

** **列类型" CALC "列中定义支持在**配方**单元格的复杂计算，且可以包括+** ** **，-** **，\*** **和运算符，并/** ** IF/THEN/ELSE **报表。 还计算列可引用其他任何列，即便对于后续的列。 此外，计算列还可包括会计年度和期间以支持列的标题。 计算公式的长度最多可为 1,024 个字符。 要以百分比形式表达计算结果，请使用特殊格式覆盖。 **注意：**计算公式的结果不包含非打印列范围中的值。 例如，**A:D** 将打印 **0**（零），而针对非打印值的 **A+B+C** 将计算此值。
...
| :        | A: C A: 方法           | 一系列连续列相加。 例如，公式 **A:C** 加上列 A 到 C 的总计，公式 **A:C-D** 加上列 A 到 C 的总计，然后减去列 D 中的金额。                          |
| -        | A-C                 | 从列 C 中减去列 A 中的金额。**注意：**您还可使用减号 (-) 改变列中的符号。 例如，使用 **- A+B** 在列 A 中添加与列 B 中的金额相反的金额。 |
| \*       | C\*                | 用列 A 中的金额乘以列 C 中的金额。                                                                                                                                                                                     |
...
您可创建以基数的百分比形式显示指定列中所有值的列。 因此，您可显示各行之间的关系，如销售行的百分比或总支出行的百分比。 若要乘以或除以在特定列中的每个行乘以基准行，输入列使用在计算，然后输入\*BASEROW ** ** ** **或" /BASEROW "。 例如，输入\*** C " BASEROW "或" C/BASEROW ** ** **。 **注意：**当您在列定义中使用基准行计算时，确保用于此列定义的每个行定义至少包含一个计算基准行。
