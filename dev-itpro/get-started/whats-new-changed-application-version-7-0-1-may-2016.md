...
## <a name="pos--microsoft-dynamics-ax-retail"></a>POS –Microsoft Dynamics AX for Retail
|                                |                                                                                                                                                                                         |
|--------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **您能怎么做？**           | **为什么如此重要**                                                                                                                                                              |
...
|---------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **您能怎么做？**                                                | **为什么如此重要**                                                                                                                                                                                                                                                                                         |
| 重新构建财务报告数据市场。                          | 当您移动 Dynamics AX 数据库在环境之间或对环境中的其他入侵报告财务数据库的更改，可能必须重新生成。 Windows PowerShell 脚本提供现在重新生成您的数据库。                                                                |
| 您可以不再选择无效的报表设计器选项。 | 管理报表器的市场内版本中使用的几个报表设计器选项不适用于此版本的 Dynamics AX。 这些选项与设计财务报表"、"输出和链接。 这些选项从财务报表设计器将防止用户取消错误。 |
...
| 计算材料和资源计划成本的物料清单计算不同的级别：•对于物料清单物料，资源计划级别中的 ReqItemLevel ** **表中计算。 在计算中，已结束的生产订单将被忽略。 • 对于生产成本计算，在 **InventTable** 中计算物料清单级别。 在计算中，已结束的生产订单将被包括在内。 | • 在运行材料资源规划时，例如，主计划的计划和增加，仅需要重新计算用于材料资源计划的物料清单级别。 换句话说，不需要计算用于生产成本计算的物料清单级别。 • 在运行成本计算操作时，例如，库存结转，仅用于生产成本计算的物料清单级别需要重新计算。 |
