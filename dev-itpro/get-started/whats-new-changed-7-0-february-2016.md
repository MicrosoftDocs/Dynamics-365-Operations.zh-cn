---
title: "最后更新或更改以 Dynamics AX 7.0 (2016 二月)"
...
# <a name="whats-new-or-changed-in-dynamics-ax-70-february-2016"></a>最后更新或更改以 Dynamics AX 7.0 (2016 二月)
...
| 自动化生成、测试和部署。                                                                            | 部分可用                                                                                                                                                                                | 通过使用 Developer 和 Build VM 部署开发人员拓扑结构。 自动配置“构建虚拟机”以从 Visual Studio Online (VSO) 发现、构建模块并运行测试。 C\# 和 X++ 模块编译和引用支持。 | 它通过减少测试和验证的成本和工作，提高开发人员的工作效率。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| 利用 Business Connector 编写业务逻辑和支持集成方案。         | Business Connector、X++ 调用可用到从代码的代码。 我们建议您使用 Business Connector 编写只在 C\#业务逻辑，而不用于集成方案。 | 不再支持 Business Connector。 只有 X++ 已编译成托管代码才会提供编写要求。 因此，互操作更容易。 通过使用 OData，满足集成方案。       | 今后您不能使用 Business Connector。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
<td>FactBox 已添加到**会计科目表**页。</td>
<td> 当定义并分配会计科目表时，对重要信息的访问更容易。</td>
</tr>
<tr class="odd">
<td>查看**试算平衡表**列表页上的单独列中的结算单调整和期末交易记录。 </td>
<td>用户在单个列中查看这两种类型的交易记录。</td>
<td>一个附加参数已添加到**试算平衡表**列表页。</td>
<td>它允许对数据进行更简明的分析，也是某些国家/地区的管制报表所必需的。 </td>
</tr>
<tr class="even">
<td>使用新**全球普通日记帐**页。</td>
<td>不可用 </td>
<td>新**全球普通日记帐**页可以输入普通日记帐。 此页面的权限添加到**会计师**角色。</td>
<td>使共享服务会计师无需退出该窗体或者切换公司上下文即可跨公司输入普通日记帐。</td>
</tr> 
<tr class="odd">
<td>使用新**会计源管理器**页。</td>
<td>通过 Dynamics AX 2012 R3 CU10 提供。</td>
<td>新**会计源资源管理器**页面和操作可以从**试算平衡表**列表页和**凭证交易记录**页导入到那里。</td>
...
<td>已经添加了报表选项，因此，可以应用不同的筛选器，当用户查看报表时。 新报表基于这些筛选器然后生成。</td>
...
<td>通过使用“监控财务业绩”Power BI 内容，分析财务数据。</td>
<td>不可用</td>
<td>在 PowerBI.com 上，选择**获取数据**，然后选择 **Dynamics AX – 财务绩效**内容包。 输入您的 Dynamics AX 终结点的 URL 以查看反映在仪表板中的数据。</td>
<td>通过三到四次单击，组织可以部署一个包含重要财务数据的 Power BI 仪表板。 内容可由组织个性化。</td>
</tr>
<tr class="even">
<td>跟踪财务期间结帐流程。</td>
<td>不可用 </td>
<td>通过使用财务期间结帐配置，可以创建结帐模板和计划。 使用**财务期间结帐**工作区可跨多个公司跟踪结帐计划的进度。</td>
<td>此工作区消除用于定义、计划和沟通结帐活动的手动系统。 因此，结帐天数会减少。</td>
</tr>
<tr class="odd">
<td>使用**分类帐预算和预测**工作区和附加的查询窗体，监控预算值与实际值，创建分类帐预测。</td>
<td>不可用</td>
<td> 该工作区可以通过 Dynamics AX 仪表板访问到。 包含指向若干新查询页的链接：**实际和预算汇总**、**预算控制统计汇总**、**预算登记簿条目**和**预算计划**。</td>
<td>新的查询页便于您输入预算信息。 该工作区将所有预算维护和监控任务组合到一个地方，便于预算经理或会计经理使用。</td>
</tr>
<tr class="even">
<td>为预算计划和预测创建布局。</td>
<td>**预算计划**文档显示为一个行列表，其中各行具有财务维度组合的生效日期和金额。 用户必须创建和使用 Excel 模板查看数据透视表中的预算计划数据。</td>
...
<tr class="odd">
<td>通过使用**未结明细表**报表（其中包括逾期天数）中的信息，打印**供应商发票交易记录**报表。 </td>
<td>您必须首先打印两个不同报表：**未结明细表**和**供应商发票交易记录**。</td>
<td>有关这两个报表的信息已合并到**供应商发票交易记录**报表中。 **未结明细表**报表已弃用。 </td>
...
<td>不可用</td>
<td>在**销售税结算期间**页上，您可以指定结算流程应以批处理模式运行。</td>
...
<td>Dynamics AX 的当前版本中的帮助系统显示在 Microsoft Dynamics Lifecycle Services (LCS) BPM 中存储的任务指南。 该帮助系统还显示 Microsoft wiki 中的主题。 有关详细信息，请参阅"Dynamics AX 帮助-入门 [] () 和帮助 overview.md [有空新任务的指南 2016] (二月)(新任务指南 2 月 2016.md 可用)。</td>
...
<td>您必须通过使用未实现的销售增值税功能来生成**销售和采购增值税**报表，因此，用户可以基于状态来识别属于已实现部分和未实现部分的交易记录。</td>
<td>**销售和采购增值税**报表已经修改，现在仅通过未实现和已实现销售税代码的定义的特定结算期间来考虑特殊增值税功能。</td>
...
<td>不可用</td>
<td>在**固定资产参数**页上，您可以选择在会计期间或会计年度结束时跟进。</td>
...
<td>功能已经实现为电子申报 Configuration。 该功能可用于德国和奥地利。</td>
<td>这一更改为用户提供数据格式、转换的更多可能性，还提供了所有来自电子申报配置生命周期管理的优点，如轻松配置汇率、版本控制等。</td>
</tr>
<tr class="odd">
<td>(FR) 具有组合计科目的余额表。</td>
<td>具有组合计科目的余额表报表实施为 SSRS 报表 (LedgerAccountSum\_{{从：FROM}})。</td>
...
<td>内部统计商品代码列表单独组合使用的商品代码输入产品层次结构。</td>
<td>这将统一将商品代码分配给销售和采购单据中已发布产品和类别的方法。</td>
</tr>
<tr class="odd">
<td>(EU) 通过使用单位转换设置报告内部统计的其他单位的数量。</td>
<td>内部统计商品代码将标识一个文本字段，额外单位，并且产品** **卡以千克具有一个标识的字段数量额外单位。</td>
...
|------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **您能怎么做？**                                                                                                                                 | **Dynamics AX 2012**                                                                                                             | **Dynamics AX 7.0**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | **为什么如此重要**                                                                                                                                                                                                                                            |
| 在单独的页面上针对生产订单执行物料可用性检查，该页面可从**生产车间管理**工作区中打开。 | 不可用                                                                                                                    | 生产主管可以检查已计划的生产订单的物料是否在要求的日期可用。 在该工作区中，生产主管可以查看有多少生产订单处于已计划状态并等待下达。 基于动态主计划，如果实际订单或计划订单的现有库存量满足物料需求，则有关物料可用性的信息会被更新。 基于有关物料可用性的信息，主管可以在**物料可用性**页上下达订单。                                                                                                                                                                                                                                                                                                                        | 它可帮助生产主管在生产订单下达到车间时针对物料到订单的分配作出正确的决策。                                                                                                       |
| 通过使用新的**作业卡设备**页，启动并报告生产作业的进度。                                                              | **工作登记**窗体主要面向大型终端屏幕，UI 通常通过鼠标单击即可访问。 | 尽管新的"作业卡设备页** **简言之设计，则还接触设计。 该页适合移动设备，例如平板电脑和手机。 车间工作人员将发现信息重载更少，而且更直观、易用。 工作人员可以执行传统任务，例如开始、结束、报告作业进度。 除了实际工作或注销并下班外，工作人员可以查看附件、午餐休息和执行其他活动。 工作按计划的顺序排队分给对工作人员，不过，工作也可以由工作人员选取。 该页主要面向离散制造工序，在那里为生产准备了物料。 对于与联产品和副产品的报告相关的情况，以及按照跟踪维度领取的物料，请使用**工作登记**页。 | 通过引入一种针对触摸而设计且可以从所有类型的设备上访问的备选 UI（例如终端屏幕和移动设备），此功能将帮助减少车间登记的传统推出的实施成本。 |

## <a name="master-planning-and-forecasting"></a>主计划和预测
|                                                                                                                            |                                                                                                                                                                                                                                                                                               |                                                                                                                                                                                                                                                                                                                                                                   |                                                                                                                                                         |
|----------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|
| **您能怎么做？**                                                                                                       | **Dynamics AX 2012**                                                                                                                                                                                                                                                                          | **Dynamics AX 7.0**                                                                                                                                                                                                                                                                                                                                               | **为什么如此重要**                                                                                                                              |
| 如果销售订单或生产订单没有做好按计划日期交货的准备，则将警告用户。                         | 由主计划创建的警告叫做“预期消息”**。 *Futures*是借贷双方之间的合同或采购销售资产为今天达成的价格 (*futures)，price*，尽管交货付款和发生在一个 (*delivery 积分将来的日期*。) | *Futures messages*和*futures dates*就将重命名*calculated delays*和*delayed 日期，*respectively。                                                                                                                                                                                                                                                   | AX 2012 中使用的术语不准确，导致不正确的翻译。                                                               |
...
### <a name="retail-hq"></a>Retail HQ 能让
...
<tr class="odd">
<td>通过使用“监控性能”零售渠道 Power BI 内容，零售渠道分析销售数据。</td>
<td>不可用</td>
...
<li>它根据用户可以选择的最快交货 (最早可用日期) 条件提供“现成”替代交货包装，如组合站点/仓库/款式，/运输方式。</li>
