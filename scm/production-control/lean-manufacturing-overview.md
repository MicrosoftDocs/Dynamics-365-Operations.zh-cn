...
-   “固定数量”****看板规则与处理单位的固定编号相关，这意味着，活动看板的数量是固定的。 每当从看板的所有产品使用，并且已手动处理单位，同一类型的新看板创建。 在创建固定数量看板规则时，您可以计算最佳看板数量和使用的产品数量。 该计算考虑到预测、未结订单的实际需求、补货物料的提前期以及历史需求。
