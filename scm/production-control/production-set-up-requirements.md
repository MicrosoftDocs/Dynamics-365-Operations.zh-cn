...
生产控制将与在其他模块的功能。 通过这一相互关联，您可以更改生产订单并确保它们在系统中的所有其他相关流程和计算中自动更新。 以下设置过程按照应完成的顺序列出。
...
**生产控制参数** –定义如何设置系统的基本的生产参数处理和处理生产订单。 定义如何创建、评估、计划和使用生产订单。 您还可以选择所需反馈类型以及执行成本核算的方式。
