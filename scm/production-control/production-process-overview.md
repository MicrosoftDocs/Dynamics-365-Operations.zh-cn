---
title: "生产流程概览"
description: "此文章提供生产流程的概览。 该描述，生产订单和批次订单不同阶段看板，从订单创建到财务结算期间。"
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
此文章提供生产流程的概览。 该描述，生产订单和批次订单不同阶段看板，从订单创建到财务结算期间。 
...
**生产控制**模块链接到其他模块，例如**产品信息管理** **，库存管理** **，总帐** **，仓库管理** **，项目核算**，和** **管理组织。 这一集成支持完成成品制造所需的信息流。  

生产流程通常受为特定生产流程选择的成本核算和库存估价方法影响。 工序的 Dynamics 365 支持两实际成本 (先进先出 \[先进先出\];last in，first out \[后进先出\];移动平均；并定期加权平均) 和标准成本方法。 lean manufacturing 基于倒冲成本计算法原则实施。  

成本度量方法的选择也定义在生产流程期间报告有关材料和资源消耗的需求。 通常，实际成本方法要求作业级别的准确报告，而定期成本计算方法允许材料和资源消耗报告相对粗略。

## <a name="mixed-mode-manufacturing"></a>混合模式制造
不同的产品和生产拓扑要求应用不同的订单类型。 工序的 Dynamics 365 可能应用各种订单类型混合模式。 换言之，所有订单类型可以在生产一件成品的端到端流程期间发生。
...
2.  **已估计** – 您可以计算物料和资源的估计消耗量。 估计生成状态为**在单**的原材料的库存交易记录。 主要产品、联产品和副产品的计划收货生成生产订单或批次订单什么时候评估。 如果物料清单包含行**被确认的供应**，键入物料的采购订单的外协或修改工序服务生成并且固定到生产订单或批次订单。 物料或订单将根据生产订单的预留策略预留，并且，成品价格基于参数设置计算。
...
5.  **准备**领料–/** **材料和资源，它在任何位置时在生产阶段，生产物料清单行或看板行更新向某个省** **领料。 限定供应订单和相关仓库工作通常在此阶段完成。 报告生产进度所需的看板卡或作业卡应分配和打印。
