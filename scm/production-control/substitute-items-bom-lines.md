...
consider {{在：following}}方案后：机器公司制造包含来自供应商的到期目录中两个月的组件。 向前从到期日期，供应商将提供可{{用：for}}旧组件将被覆盖的新的组件。 开始日期和结束日期可以在物料清单行上设置。 对于本示例，您通过在**“结束日期”**字段中输入到期日期将旧组件设置为到期。 然后，在物料清单上，您设置新的替换组件，以便它从旧组件到期那天后生效。 为此，请在**“开始日期”**字段中输入开始日期。
