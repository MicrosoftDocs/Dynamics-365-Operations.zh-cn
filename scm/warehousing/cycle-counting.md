...
当物料数量减少至低于库位中某个特定的临界值，可以创建周期盘点工作。 例如，具有 60 个物料具有在周期盘点阈值 40 的位置。 在销售订单交易记录时，物料 25 从库位领料在暂存库位。 因为新的物料数量 35 低于临界值，将自动为该库位创建周期盘点工作。
...
-   盘点的结果值不在**工作用户**页中的**最大百分比限制**或**最大数量限制**字段中指定的偏差范围内。 例如，现有库存量为 50，数量在位置工作，并且用户的偏差限制为 10。 如果用户不工作输入介于 40 和 60 之间，发生变化的值。
