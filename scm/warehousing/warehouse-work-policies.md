...
此工作策略控制是否为生产中的仓库流程创建仓库工作。 您可以使用“**工作订单类型**”、“**库存库位**”和“**产品**”组合设置工作策略。 例如，报告为已完工产品 L0101 输出位置 001。 完好输出位置在 001 的其他生产订单后使用。 在这种情况下，您报告，在产品 L0101 为已完成时，输出位置 001 可以设置工作策略防止成品的放置离开工作创建。 工作策略是可以通过以下信息描述的单个实体：
