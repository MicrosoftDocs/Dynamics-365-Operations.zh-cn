...
托运库存是由供应商所拥有，但存储在您的站点的库存。 在您准备好消耗或使用库存时，您接过库存的所有权。 此主题中的信息介绍如何实际接收供应商拥有现有库存量，而不必创建总帐交易记录，如何开始供应商拥有库存可以是物理预留的生产流程。 作为生产订单处理一部分，以及如何更改所有权的为了能处理原材料消耗量。 还提供了关于供应商如何使用供应商协作界面监控库存消耗量的一些信息。 有关如何启用和配置入站托运流程的信息，请参阅“[设置托运](set-up-consignment.md)”。
...
托运补货订单是通过记录已订购的库存交易记录请求和跟踪记录供应商计划在特定的日期间隔内交货的产品库存量的文档。 通常，这基于特定产品的预测和实际需求。 要对照托运补货订单收货的库存仍然归供应商所有。 仅记录与实际收货有关的产品拥有更新，因此不会发生总帐交易记录更新。 “**所有者**”维度用于分隔与供应商拥有的库存有关和与接收法人拥有的库存有关的信息。 托运补货订单行具有** **未结订单状态，只要这些物料行的完整数量未接收或未取消。 将全部数量接收或取消时，状态更改** **已完成。 可以使用登记流程和收货更新流程记录与托运补货订单有关的实际现有库存量。 登记可作为物料到达流程的一部分或通过手动更新订单行完成。 使用收货更新流程时，在收货日记帐中进行记录，可用来向供应商确认收货。 

[![托运补货] (订单。/media/consignment 补货 order.png)](。/media/consignment 补货 order.png)
...
-   ** ** **托运使用库存的采购订单** -查看相关采购订单的详细信息。从进行托运的流程更改所有权。
