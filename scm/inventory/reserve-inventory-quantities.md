...
| 物料销售预留  | 确定手动或自动预留物料。 如果自动预留，则当创建订单行时预留库存。 可以在物料编号级别为物料清单进行预留（**自动**选项）或者为物料清单的单个元素进行预留（**分解**选项）。 **物料销售预留**的默认值可能继承自**应收帐款参数**。 在此页上，该值在“**常规**”选项卡上的“**销售默认值**”**部分**的“预留”字段中进行设置。 |
