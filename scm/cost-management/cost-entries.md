...
登记转移日记帐事件。 事件转移物料 A 一件 1 从站点的站点。2. 站点库存被视为维度一部分成本的对象。 因此，该事件将创建两个库存交易记录和两个成本条目。
...
| 凭证 | 日期       | 引用      | 编号 | 批次 ID  | 参考批次 | 退货批次 ID | 数量 | 金额  |
|---------|------------|----------------|--------|---------|---------------|---------------|----------|---------|
...
**“成本条目”**页面支持按文档 ID 和文档日期进行筛选。 **注释：**成本的条目是仅可为对象成本 [] (成本) object.md 或下达的产品。
