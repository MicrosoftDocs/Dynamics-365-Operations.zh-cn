...
库存交易记录用工序的 Microsoft Dynamics 365 中可以在中实际更新和财务更新。 某些实际交易记录和财务交易记录类型会增加库存数量，而其他则会减少数量。
...
以移动平均成本价过帐增加数量的交易记录。 工序的 Dynamics 365 计算基于每个库存维度的上述各交易记录的成本的财务跟踪移动平均成本价。 有关移动平均成本价的信息，请参阅[移动平均成本价](running-average-cost-price.md)。

## <a name="transactions-that-decrease-quantity"></a>减少数量的交易记录
工序的 Dynamics 365 使用计算出的移动平均成本价，而过帐减少数量的交易记录时，无论与该库存的库存模型。 减少数量的交易记录不得在另一个交易记录过帐前标记为该交易记录。 如果现有库存量变为负值，工序的 Dynamics 365 使用用于的库存成本**物料**物料定义的页面。 **注意：**如果启用多站点功能，则该成本将改为在**默认订单设置**页上定义的库存成本。
