...
-   **销售提前期** –销售提前期为物料创建的销售订单的装运和之间的时间。 邮政日期计算基于默认天数和不考虑库存可用性、已知的需求或供应计划的。
-   ** ATP (–) 可承诺** ATP，可承诺可被为特定的日期对某一客户的物料的数量。 ATP 计算包括未承诺库存、提前期、计划收货和发货。
-   **ATP + 发货宽限期 **– 装运日期等于 ATP 日期加上物料的发货宽限期。 发货宽限期是准备物料以进行装运所需的时间。
-   **CTP（可承诺量）**– 通过分解计算可用性。

## <a name="atp-calculations"></a>ATP 计算
通过使用“含预期的累积 ATP”方法中，ATP 计算数量。 此计算 ATP 的方法的主要优点是无法处理发货量之和在收货中的低于最新收货大于 (大小写，例如，在来自更早收货的数量的数量必须用于满足需求时)。 “含预期的累积 ATP”方法计算的包括前的所有发货，接收的累计数量超出累积发货数量。 因此，该 ATP 计算方法评估更早期间的某些数量是否可用于以后的期间。  

该 ATP 数量是第一个期间中的未承诺库存余额。 通常，会为计划收货的每个期间计算它。 程序将按天数计算 ATP 期间，并且将当前日期计算为该 ATP 数量的第一个日期。 在第一个期间中，ATP 将包括现有库存量减去到期和逾期的客户订单。  

ATP 使用以下公式计算：  

ATP =前一期间的 ATP + 当前期间的收货–当前期间的发货–期间的净发货数量前的各个将来期间的收货，在的总和所有未来期间，截至将来的期间，超出的发货总和截至将来的期间。  
...
由于交货日期控制方法是 ATP，所以会计算 ATP 数据以找到最早的可能装运日期。 基于设置，延迟的采购订单和销售订单，并且考虑，当前日期的生成的 ATP 数量是 0。 明天，在延迟的预期采购订单接收时，ATP 数量多 0 计算。(在这种情况下，它计算为 125)。 从现在起但是，10 天，因此，当 100 件的其他采购订单预期接收时，ATP 数量将变为 150 多。  

从现在起因此，装运日期设置为 10 天，基于 ATP 计算。 因此，您告知客户请求数量的可以为从现在起的 10 天交货。
