...
-   确定您想让您的员工能够订购的产品。 在目录节点可以显示或隐藏一特定产品，或者可以在节点可以显示或隐藏所有产品。
...
目录定义在创建采购申请时哪些产品可用，不过，您可以使用类别访问策略规则来应用附加限制。 由于目录中的节点是采购类别，因此可以按类别访问策略规则来抑制它们。 在这种情况下，该类别的产品是不可用。员工可以使用在申请。 您定义采购类别。**策略**访问页面的策略规则。 下表描述了要设置目录必须完成的任务。
...
[设置采购类别层次结构 (任务指南)](http://ax.help.dynamics.com/en/wiki/set-up-a-procurement-category-hierarchy/)
