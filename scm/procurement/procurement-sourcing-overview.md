...
采购订单包括将*采购查询*发送到供应商的选项，作为更为全面的询价流程的备选。 采购查询可用于帮助设定诸如订单的价格、折扣和交货日期等期限。 如果供应商设置为使用该**供应商** **门户，采购查询功能可以禁用。 而如果该订单在**供应商**门户共享，当*确认请求*发送时，供应商可以直接确认该订单。  
...
-   标识作为需求需要采购主计划的结果。 此流程，生成计划采购订单，因此，这些在发放时，采购订单生成的。
-   通过处理生成采购的采购申请。
...
在创建采购订单时，许多字段 prepopulated 使用来自默认信息存储有关的值**供应商**页面的供应商。 这意味着您需要在采购订单上填写有限数量的字段，虽然您可以选择覆盖默认值。
