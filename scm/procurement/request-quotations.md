...
# <a name="request-for-quotations-rfqs"></a>要求报价单 (RFQs)
...
[] (![询价流程。/media/rfq 流程 458x1024.jpg)](。/media/rfq-process.jpg)  

您可以计划订单、从采购申请或者从手动输入创建询价。 您创建的询价称为询价案例，这是您用于将询价发布给每个供应商的基础单据。 在您准备案例和询价添加供应商后，请单击**发送**案例在询价和询价日志为每个供应商生成您发送询价。 您可以配置用于发送操作的打印管理设置为每个供应商打印的报表到存档或可以将报表发送到每个供应商的电子邮件地址。 此外，每个供应商的询价日记帐可用于生成您可以发送或在以后重新发送到供应商的报表。 您还可以将“发送”操作配置为生成供应商可以填写的回复表。  
...
在您决定出价或部分出价后，可以接受它们并拒绝其余出价。 接收日记帐、拒绝日记帐和相应的报表将生成。 这些字段根据您的设置，打印管理要打印存档，以及发送。 在您接受一投标或特定行。投标时，采购协议或采购订单生成，或者采购申请更新询价，根据采购类型。 您可以创建以后用于任何回复的贸易协议，不论您是接受还是拒绝它们。  
...
您可以覆盖特定询价案例的这些值。 还应配置改正流程。 作为此配置的一部分，您可以打开字段锁定。 在字段锁定打开时，要改正询价的采购专业人员必须在选项卡**报价单**的**改正**部分单击**创建**。 在询价更新该校正个专业必须通过采购后，单击"完成流程** **请完成。** **完成操作生成通知有关已修正的询价的一个供应商的电子邮件。 您为发送到**采购参数**页面的供应商的电子邮件通知选择模板。 在模板创建时，可以包含以下替换标志：
...
如果输入了回复，但需要从供应商新建一提供，您可以重新发送询价。 这将生成新的日志和您自行可以使用从供应商请求的更改。  
