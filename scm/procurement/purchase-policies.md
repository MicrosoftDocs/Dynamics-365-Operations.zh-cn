...
是低但减去复杂一罐设置采购策略按法人的组织，以及可能仅使用公司组织层次结构。  
...
Fabrikam 创建两个采购策略。 策略 A 应用于其美国法人，例如 1111。 策略 B 应用于其加拿大法人，例如 2222。 当某一员工在法人中 1111 创建采购申请时策略规则，例如从策略 A.，派生员工在目录策略规则列表指定策略 A. 的产品目录。  

当某一员工在法人 2222 中创建采购申请时，从派生 B 策略策略规则。  
...
创建采购订单和合并需求规则定义策略规则使用采购订单什么时候从某一审核的采购申请生成。 当您创建此规则的类型时，您可以在各个选项卡上设置选项：
...
-   在**“价格容差”**选项卡上，当采购目录物料的价格上涨时，您可以定义通过查看流程传送已获批的采购申请的规则。 设置采购申请的行项上的净额的最大金额可以延长已审核的采购申请和已创建的采购订单之间的时间。 通过使用以下公式，净额计算：(\[数量× (单位价格–折扣) ÷计价单位\] + 采购杂项费用) × (\[–折扣百分比) 超过\] ÷价格容差设置您的采购申请行保持为手动处理。 您在**“错误处理”**选项卡上配置的规则将确定采购申请行的处理方式。
