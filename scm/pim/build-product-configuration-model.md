...
# <a name="build-a-product-configuration-model"></a>配置创建产品模型
...
有几个产品配置建模原则，例如基于规则、基于维度和基于约束的建模。 研究表明，与其他建模原则相比，基于约束的方法可在模型中减少大约 50% 的代码行数。 因此，此方法可降低总拥有成本 (TCO)。 通过移动根据为基于约束的模型的 X++ 代码的基于规则的模型，您不再要求一个开发人员许可证来维护产品模型。
...
一个系统中定义的表约束通过操作表的 Microsoft Dynamics 365 可以用作字段引用然后选择此表窗体"列将约束的选择定义的。 表约束的行为。存在配置时 Dynamics 365 行的行动的表。  
...
测试模型类似于运行一实际配置会话。 用户可以通过配置"页和支持通过验证模型结构配置流程。 用户可验证属性值是否正确，以及属性描述是否指导用户选择了正确的值。 最后，在测试会话完成后，系统将尝试创建对应于所选属性值的物料清单和工艺路线，并在出现任何问题时显示错误消息。
...
已实施专门的应用程序编程接口 (API)，以便让合作伙伴和具有开发人员许可证的其他人员扩展产品配置模型的功能。 主要用途是通过建立合作伙伴和客户，将使用现有产品生成器迁移代码在产品生成器模型嵌入到的 API 的机制。 这样，他们就可以将其模型从产品生成器迁移到产品配置。 但是，新合作伙伴和客户还可能受益于使用 API 扩展新产品配置模型。

### <a name="pcadaptor-class"></a>PCAdaptor 类

API 是使用一组 **PCAdaptor** 类实施的，这些类可公开产品配置模型的数据结构。 必须为将是扩展的每个模型创建 PCAdaptor ** **类的实例。 在配置会话完成后，此类和实例运行的系统检查该物料，则查找。  
