...
成本元素作为跟踪和分类成本流向的功能使用。 成本元素有两种类型：主要成本和辅助成本。 ** **主要成本主要成本要素表示成本维护来自财务科目的到成本核算。 成本元素结构对应于总帐中的损益科目结构，其中成本元素可能对应于主科目。 并非所有的主科目都必须表示为成本元素，具体取决于业务要求。 以下是主要成本元素的一些示例：
...
- **固定成本** -无论在处于活动程度上更改，最近固定成本是不，发生变化的成本。 例如，固定成本可以是公司的基本运营费用，例如租金，不会因活动水平的增加或减少而受到影响。

- **可变成本**可变成本-根据"活动级别中所做的更改。 例如，特定直接物料成本与出售的每个产品相关联。 售出的产品越多，直接物料成本越高。

- **半变量所用的**半变量-部分部分成本是固定和可变成本。 例如，网络接入费用包括每月标准接入费和宽带使用费。 每月标准接入费是固定成本，宽带使用费是可变成本。
