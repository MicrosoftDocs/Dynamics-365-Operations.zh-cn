...
付款方案查询包含不同的选项卡，每个选项卡有不同的选项用于选择要支付的发票。 ** **参数选项卡包含大多数组织通常使用的选项。 **在到涵盖的 FastTab 记录**，可以指定哪些发票或供应商的付款包括通过为各种功能定义的范围。 例如，如果要支付，供应商仅指定一个范围，您可以定义一筛选器的供应商范围。 此功能通常的付款方式选择特定的发票。 例如，您，则定义一筛选器** **付款方式 = = ** **支票，该付款方式，只有具有的发票为付款选择，在此情况下，它们还与查询中指定的其他条件的情况下。 **高级参数**选项卡包含其他选项，某些可能不与您的组织相关。 例如，此选项卡包含支付集中付款的发票的选项。

## <a name="parameters"></a>参数设置
-   **在的日期范围内的所选发票** –发票** **开始日期和结束日期** **字段指定可按到期日期，现金折扣日期或这两者的选择。 如果使用现金折扣日期，系统将首先查找则在开始日期和结束日期之间的现金折扣日期的发票。 系统然后通过使用会话日期确定发票是否满足现金折扣资格，以确保现金折扣日期尚未过期。
-   **开始日期**和**结束日期** – 具有在此日期范围内的到期日期或现金折扣日期的发票将为付款选择。
-   **付款日期** – 如果日期已定义，所有付款在此日期创建。 忽略**最小付款日期**字段。
-   **最小付款日期** – 输入最小付款日期。 **，例如开始日期和结束** ** **，字段指定范围 9 月 1 日到日，9 月 10，最小是 9 月 5. 付款日期。 在这种情况下，具有个到期日期。9 月 1 日到 9 月 5 日在内的所有发票具有付款日期 9 月 5。 但是，安排一个到期日期。9 月 5 日到 9 月 10 日的所有发票具有等于各发票到期日期等于上的付款日期。
-   **限额** – 输入所有付款的最大总金额。
-   **创建付款，而无需发票预览**，–如果此选项设置** **是付款，立即在** **页面将供应商付款创建。 **付款方案**跳过页面。 因此，付款将更快创建。 付款仍可以从**供应商付款**页修改。 或者，您可以使用**编辑所选付款的发票**按钮返回**付款方案**页。

## <a name="advanced-options"></a>高级选项
-   **检查供应商余额** – 如果此选项设置为**是**，则系统会在支付任何发票前验证供应商没有借方余额。 如果供应商有借方余额没有，付款创建。 例如，某个供应商可能具有过帐，但尚未结算的贷方通知单或付款。 在这些情况下，不应支付供应商。 而是，贷方通知单或付款应对照未清发票结算。
...
-   **发票总和的付款** – 如果**删除负付款**选项设置为**是**，且供应商存在未结发票和付款的付款，则为付款同时选择未结发票和付款，并且金额一同添加以生成总计付款金额。 仅在总计导致退款时出现异常。 在这种情况下，发票和付款均不选择。 如果该**删除负付款** **选项设置，不**，并且发票和未付款，付款发票和付款为付款选择和金额加起来生成付款金额。
...
-   **付款币种** –此字段中指定币种所有付款创建。 如果币种未定义，每个发票在发票的币种支付。
-   **付款日** – 输入应进行付款的一周的第几天。 此字段只有当特定日付款的付款方法设置为总计发票时才使用。
-   **对方科目类型和科目对方** ** ** –设置这些字段定义特定科目类型 (如分类帐或银行** ** ** **) 和一个对方科目 (例如特定银行账户)。 发票的付款方式定义默认的对方科目类型和对方科目，不过，您还可以使用这些字段中覆盖默认值。
-   **其他筛选器** –在**到涵盖的 FastTab 记录**，您可以定义附加的一组条件。 例如，如果要支付，只有供应商范围，您可以定义一筛选器的供应商范围。 此功能通常的付款方式选择特定的发票。 例如，您，则定义一筛选器** **付款方式 = = ** **支票，该付款方式，只有具有的发票为付款选择，在此情况下，它们还与查询中指定的其他条件的情况下。
...
April 将**现金折扣**选择作为方案类型。 她输入日期范围 6 月 26 日到 7 月 10 日。 以下发票在方案包括：
...
April 将**按到期日期**选择作为方案类型。 她输入日期范围 6 月 26 日到 7 月 10 日。 以下发票在方案包括：
...
April 将**到期日期和现金折扣**选择作为方案类型。 她输入日期范围 6 月 26 日到 7 月 10 日。 以下发票在方案包括：
...
-   “**维度控制**”字段已激活，且启用进一步的维度。 现在您定义如何将维度复制到日记帐。 例如：•**选择 BusinessUnit **此复选框可以创建一个付款方案。付款方式的业务单位，•**选择 CostCenter **此复选框可以创建一个付款方案。付款方式的成本中心

**注意：**如果您在第三个选项中选择多个维度，则为维度组合创建一个付款方案。

#### <a name="bank-account-selection"></a>银行帐户选择

您可以按付款方式定义标准借记付款帐户，无论国家/地区上下文。 这将在由方案生成的付款行中设置。 通过银行账户功能，您可以定义多个通过维度和币种或其组合管理的借记银行账户，以根据每种组合使用不同的借记银行账户。 您可以设置组合**这些付款方法**页通过使用**银行账户**按可用于具有** **过帐科目类型 = = ** **每个银行的付款方式。
