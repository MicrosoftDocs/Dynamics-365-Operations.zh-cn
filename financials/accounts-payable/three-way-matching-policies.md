...
2.  April 是 Fabrikam 的应付帐款协调员，输入和验证 Contoso 提交的发票。 她验证了以下信息：
    ...
    -   对于要求或双向三通符合的物料，对发票行上的价格。在 Operations.This 的 Microsoft Dynamics 365 中定义包括以下类型的价格匹配的容差范围内。:
...
在工序的 Microsoft Dynamics 365 行，发票中包括以下信息。
...
2.  April 是 Fabrikam 的应付帐款协调员，输入和验证 Contoso 提交的发票。 她验证了以下信息：
    -   针对要求三向匹配的物料，发票行上的数量与接收的数量匹配。 产品收据上指示的接受的数量与发票上的数量匹配。
    -   对于要求或双向三通符合的物料，对发票行上的价格。工序中 Microsoft Dynamics 365 中定义的容差范围内。 这包括价格匹配的以下类型：
...
有关详细信息，请参阅 [] 应付帐款发票匹配 (应付发票帐户 matching.md)。
