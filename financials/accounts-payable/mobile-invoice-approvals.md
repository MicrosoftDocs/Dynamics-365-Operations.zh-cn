---
title: "移动发票审核"
description: "商业用户可通过 Microsoft Dynamics 365 for Operations 中的移动功能设计移动体验。 对于高级方案，开发人员还可通过此平台根据需要扩展功能。 若要了解有关移动的一些新概念，最有效的方法是设计一些新方案。 本主题旨在通过以供应商发票的移动审核为使用案例，提供移动方案的设计方法实践。 本主题应该可以帮助您设计这些方案的变型，还可以适用于与供应商发票无关的其他方案。"
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
商业用户可通过 Microsoft Dynamics 365 for Operations 中的移动功能设计移动体验。 对于高级方案，开发人员还可通过此平台根据需要扩展功能。 若要了解有关移动的一些新概念，最有效的方法是设计一些新方案。 本主题旨在通过以供应商发票的移动审核为使用案例，提供移动方案的设计方法实践。 本主题应该可以帮助您设计这些方案的变型，还可以适用于与供应商发票无关的其他方案。
...
| 必备项                                                                                            | 说明                                                                                                                                                          |
|---------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 移动手册预习                                                                                |(/dynamics365/operations/dev-itpro/mobile-apps/mobile-platform.md)                                                                                                  |
...
| 为 Dynamics 365 for Operations 安装了此移动应用程序的 Android、iOS 或 Windows 设备 | 在相应的应用程序商城中搜索该应用程序。                                                                                                                     |
...
1.  在浏览器中，打开 Dynamics 365 for Operations 并登录。
...
8.  从列表页面添加必须在移动页面中向用户显示的所需列。 添加顺序为字段向最终用户显示的顺序。 只能通过重新选择所有字段才能更改字段的顺序。 根据此方案的要求，需要下面的八个字段： 但是，某些用户可能会认为八个字段在移动设备上提供的信息太多。 因此，我们将在移动列表视图中仅显示最重要的字段。 其余字段将在我们以后将设计的详细信息视图中显示。 现在我们将仅添加以下字段。 单击这些列中的加号 (**+**) 添加到移动页面。
    ...
    添加字段之后，移动页面必须类似下图。 [![添加字段后的页面](./media/mobile-invoice-approvals03.png)](./media/mobile-invoice-approvals03.png)
...
若要为移动设计发票明细页面，请使用 Dynamics 365 for Operations 中的 **VendMobileInvoiceHeaderDetails** 页面。 请注意，此页面根据您在系统中的发票数量，显示时间最久的发票（即最先创建的发票）。 若要查找特定发票，必须使用左侧的筛选器。 但是，此示例不需要特定发票。 我们仅需要一些发票数据，以便设计移动页面。 [![工作流页面](./media/mobile-invoice-approvals04-1024x425.png)](./media/mobile-invoice-approvals04.png)

1.  在 Dynamics 365 for Operations URL 中，将菜单项的名称替换为 **VendMobileInvoiceHeaderDetails** 以打开窗体
...
11. 添加了前面两步骤中的所有字段之后，单击**完成**。 此页面必须类似下图。 [![添加字段后的页面](./media/mobile-invoice-approvals05.png)](./media/mobile-invoice-approvals05.png)
12. 单击**完成**退出编辑模式。
...
若要添加工作流操作，请使用 Dynamics 365 for Operations 中的 **VendMobileInvoiceHeaderDetails** 页面。 若要打开此页面，请替换 URL 中的菜单项名称，如前面执行的操作。 然后通过**设置**（齿轮）按钮打开移动设计器。 执行以下步骤在明细页面上添加工作流操作。
...
11. 单击**发布工作区**以保存工作
12. 重复步骤 3 到 11，录制需要的所有工作流操作。 请注意，必须将状态为可提供您要设计的工作流操作的发票分配给您。
13. 打开“记事本”或 Microsoft Visual Studio，然后粘贴以下代码。 将文件保存为 .js 文件。 此代码执行两项操作：
    1.  隐藏我们前面在移动列表页面中添加且与工作流有关的额外列。 我们添加这些列是为了让应用程序在上下文中有这些信息，并且可以执行下一步。
    2.  它根据处于活动状态的工作流步骤应用逻辑，以便仅显示这些操作。

请注意，这些页面和 JS 代码中的其他控件的名称必须与工作区中的相同。
...
此方案的要求确认将仅有行级别的分配，并且一张发票始终只有一行。 由于此方案非常简单，所以移动设备上的用户体验还必须足够简单，这样用户才不必向下滚动多次即可查看分配。 Dynamics 365 for Operations 中的供应商发票包含用于显示发票抬头中的所有分配的选项。 此体验正是移动方案需要的。 因此，我们将使用 **VendMobileInvoiceAllDistributionTree** 页面设计移动方案的此部分。 

> [!NOTE] 
> 了解要求可以帮助我们在设计方案时确定要使用哪个特定页面和到底如何为用户优化移动体验。 在第二个方案中，我们将使用其他页面显示分配，因为该方案的要求不同。

1.  在 URL 中，替换 URL 中的菜单项名称，如前面执行的操作。 显示的页面应类似下图。 [![“全部分配”页面](./media/mobile-invoice-approvals06.png)](./media/mobile-invoice-approvals06.png)
2.  通过**设置**（齿轮）按钮打开移动设计器。
...
**注释：****查看会计**移动页面现在未链接到我们迄今为止设计的任何移动页面。 因为用户应该可以从移动设备上的**发票明细**页面导航到**查看会计**页面，所以我们必须提供从**发票明细**页面到**查看会计**页面的导航。 我们使用通过 JavaScript 的更多逻辑建立此导航。
...
> [!NOTE] 
> 这些页面和 JS 代码中的其他控件的名称必须与工作区中的相同。
...
从您的移动设备打开应用程序，然后连接到您的 Dynamics 365 for Operations 实例。 确保在供应商发票分配给您检查的位置登录公司。 您应该可以执行以下操作：
...
可根据方案 2 的要求为方案 1 完成以下变型。 请将此部分用作练习，为学习目的完成。
...
您可以通过移动平台和应用程序功能设计针对组织中的用户群优化的移动方案。 可以根据此主题中提供的示例，尝试其他变型和创建可满足特定需求的不同体验。
