...
以下参数将影响如何结算在工序 365 的 Microsoft Dynamics 中处理。 结算是对照付款或贷方通知单结算发票的流程。 这些参数位于**应收帐款参数**和**应付帐款参数**页的**结算**区域。

-   **自动结算** – 如果交易记录在过帐时应自动对照其他未结交易记录结算，则设置此选项为**是**。 如果此选项设置为**否**，用户可以使用**结算交易记录**页在输入付款或之后手动结算交易记录。
-   **现金折扣管理** –如何请指定" [现金折扣。处理，当多支付发票] (现金折扣 overpayments.md 处理)。 对于超额支付，现金折扣可以减少，则可以出于差异，或者可以为供应商或客户将不分期付款。
...
-   **计算贷方通知单的现金折扣** – 将此选项设置为**是**以自动计算贷方通知单的现金折扣。 在应收帐款中，贷方通知单交易记录是在**普通发票**页的**发票**字段中具有值的负交易记录，或在**销售订单**页上的退货。
    -   此选项的此影响取决于**结算交易记录**页的**使用现金折扣**字段的值。 如果将此选项设置** **是，折扣的操作，在****使用现金折扣****字段设置时诺默尔** **。 在****使用现金折扣****字段设置**始终**，则无论此字段，设置现金折扣始终，获得。 在****使用现金折扣****字段设置**永不**，则无论此字段，不设置现金折扣，获得。
...
-   **设置结算的优先级（仅应收帐款）** – 设置此选项为**是**将启用**输入客户付款**和**结算交易记录**页的**按优先级标记**按钮。 此按钮允许用户将预先确定的顺序结算到交易记录。  在结算交易记录应用到订单后，订单和付款分配可以在过帐之前按需修改。
-   **自动的结算使用优先级** –此选项设置** **是使用定义的优先级，则自动结算交易记录时。 只有当**此字段才可用，请设置优先级，以及结算** **自动结算** ** **是设置选项。
