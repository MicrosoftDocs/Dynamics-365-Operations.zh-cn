...
采用外币的未结供应商交易记录的理论价值或帐面价值会由于汇率波动在不同时间发生变化。 若要更新应收帐款和应付帐款的未结交易记录的值，请运行外币重估流程。 应付帐款和应收帐款均可运行外币重估。 该流程将在指定日期使用新的汇率重估未结金额。 原始之间的差额过帐金额，并且将导致重估的金额或者具有损益的每个未结交易记录。 应付帐款和应收帐款 subledgers 然后更新反映或有利润或损失，并且分类帐分录过帐到总帐。
...
使用下**外币重估**页面**定期任务**重估未结交易记录。 您可以通过使用批处理实时运行此流程或计划此流程以进行运行。 在您定义了重新计算流程的时设置验证，请确保要打印结果的报告。 在流程完成重估后，报表不能重新打印。 如果您生成外币重估报表，该报表将显示客户/供应商级别和币种级别的各个余额：
...
每次运行外币重估时，都将保留记录。 从**外币评估**页的记录中，选择**交易记录**查看由于重估创建的交易记录的详细列表。 每个凭证交易记录所表示。重估的未结交易记录。 如果未结交易记录。重估原多一次，您将看到使用同一个凭证中的两个记录。 一记录将针对以前或具有损益的冲销，而另一记录将针对新的或有损益。 若要运行重估流程，单击**外币重估**按钮。 为以下参数定义适当设置：
