...
-   利息金额应用于所有币种。
-   可输入可选利息金额限制。
-   **百分比** ** ** **请选择在计算基于的利息** **的字段设置利息代码**页。

例如，项目 5% 设置利息每两个月的利息代码发票付款超出了交易到期日期，则在**则计算利息**每个字段可以输入 2 并且选择** **月份。

## <a name="interest-rates-based-on-amounts"></a>基于金额的利率
您可以设置计算针对指定币种的利率金额。
-   为利息代码中的每个币种指定利息金额。
-   可输入可选利息金额限制。
-   ** **金额所**请基于计算的利息** **的字段设置利息代码**页。

例如，可设置为每 20 天估计利息 25.00 的利息代码发票付款超出了交易到期日期，则在**则计算利息**每个字段可以输入 20 并且选择** **天数。
