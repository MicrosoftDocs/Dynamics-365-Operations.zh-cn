...
[![预算功能组件](./media/budgeting-functionality-components.jpg)](./media/budgeting-functionality-components.jpg) 长期战略计划和年度预算计划的流程通过预算计划文档支持。 预算计划文档以 Microsoft Excel 紧密集成。 用户可配置无数个货币和数量方案，也可以定义预算组织层次结构以支持自上而下和自下而上的预算方法。 在 Microsoft Dynamics 365 for Operations 中建立和审核预算后，预算计划将转换为预算登记分录。 预算登记分录提供了维护预算和通过预算代码保持金额的可追溯性的工具。 预算登记分录可让您修订原始预算、执行转移和从上一年结转预算金额。 根据建立的预算，公司可启用预算控制。 控制的级别取决于组织文化和组织成熟度级别。 成熟度较低的组织可能将预算保留“原样”，当预算未达到期望时，它们可能更倾向于被动地响应，而不会主动采取措施。 其他组织可能启用预算控制策略以防止用户在没有预算资金时进行采购。 最后，非常成熟的组织可能建立一种组织文化，在这种文化中，员工将受到有关组织目标的教育，并通过“考虑联机会议而不是出差”之类的策略实现这些目标。 Dynamics 365 for Operations 包含让公司的管理层选择硬控制（防止超过预算的过帐）或软控制（此时，用户被警告将超过可用预算资金，但可以自行决定如何继续）的预算控制框架。 最后，您可使用滚动预测。 滚动预测是预算与实际支出的常规比较，用于定义公司基于预算的运营状况。 滚动预测还用于确定趋势。 在 Microsoft Dynamics 365 for Operations 中，滚动预测通过作为初始规划活动的预算计划文档获得支持。 滚动预测可以与针对即将到来的预算周期的规划并行执行。
...
-   [预算计划调整文档](budget-planning-justification-docs.md)
-   [预算计划的 Microsoft Excel 模板](budget-planning-excel-templates.md)
...
-   **需求预测** - 基于历史交易数据，您可以预估未来库存需求并创建需求预测。
