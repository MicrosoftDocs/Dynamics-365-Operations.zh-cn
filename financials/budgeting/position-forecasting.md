...
[] (顶部![图形。/media/graphic-top.png)](。/media/graphic-top.png) 

职位预测使用三种主要成分。职位支出提供准确预算金额。 这些金额然后可以放入预算计划以进行预算计算。 
...
在职位预测的典型流程，您首先创建安装组件 (预算成本要素和薪酬) 组。 然后基于现有职位生成预测职位。 随后您可以进行调整。 例如，您可以添加或结束职位、更改付薪比率和福利成本并添加工资涨幅。 您可以创建多个预测职位版本来推进不同预算方案之间的对比。 接下来，您可以将预测职位包括在预算计划内，并作为预算计划行放入预测职位的成本。
...
Jodi 的组织提供了一种 5% 的津贴员工的基本工资。 Jodi 想要为此成本创建预算成本元素。 她创建新的预算成本元素并分配**福利**预算成本类型。
...
薪酬比率表中的生效日期和到期日期让您可以更改任何日期的付薪比率。 当协商单位针对预算周期中间的全面增加进行协商时，此功能十分有用。 在这种情况下，您更改现有的表的到期日期。，在比率更改的日期并添加在新的日期开始的新的标准表的前一天。 在您创建某一新的付薪比率，表，则选择**选择根据现有未达标创建一个新网格的薪酬网格**，可以选择从人力资源的现有表比率。 在创建的比率表中，**批量更改**选项可以对网格中的所有比率应用百分比或固定金额增加或降低。 
...
对于最精确预算计划，才能创建预测职位将通过使用从现有职位的详细信息。工序的 Microsoft Dynamics 365 中，而不管职位是当前填充或未实现的。 
...
**分配薪酬组**选项的设置优先于**预算成本元素分配**设置。 可以同时使用这两设置。 

[] (![graphic4。/media/graphic4.png)](。/media/graphic4.png) 

另一个选项是将周年纪念日日期。 已分配的工作人员的所选日期（调整后的开始日期、工作人员开始日期、雇用开始日期或受聘日期）随后会设置为预测职位的周年日期，并用于提供信息和生成工资涨幅。
...
组织考虑减去会计职位的成本的两个选项。 一个选项是结束全年方法的职位部分。 另一个选项是将职位更改为全年半工。 Brad 在基准方案中为现有会计职位创建了一个预测职位。 他将此基准预测职位复制到方案 A，设置退休日期为 5 月 31 日，并重新计算。 然后，Brad 将基准预测职位复制到方案 B，更改 FTE 值为 **0.50**，并重新计算。 Brad 现在有三个版本，每个版本均有与其选项一致的成本合计。
...
两个选项在" **拖动预算成本要素**安装"页面上：**则添加与某个职位** **职位**和更新。 **添加到职位**选项将预算成本元素添加到选定预测职位。 如果元素已分配给某一预测职位，将跳过该预测职位。 **更新职位**选项将当前值（主科目、百分比、年度金额，等等）应用于所选预测职位。 

每个流程具有可选择预测职位的类似页面。 **添加到职位**页显示可供选择的所有预测职位，而**更新职位**页只显示具有已分配的预算成本元素的那些预测职位。 ** (因此，职位**更新页授予您预测职位已附加的成本要素。) 的一种方式查看您从一个上部网格的预测职位向更低的移到网格中包括它们更新。 
...
创建和修改预测职位的目的是添加到预算计划，因此，预算计划包括最精确的预算金额。 将预测职位添加到预算计划有两种方法。 您可以在预算计划中使用生成流程或选择流程。
