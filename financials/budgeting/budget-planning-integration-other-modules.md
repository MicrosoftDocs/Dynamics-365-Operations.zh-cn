...
-   **请创建一个新的预算计划**创建具有属性。**选择的新计划**目标部分。 这些属性不必是唯一的。 因此，两个计划可以具有相同的名称和其他值。
...
对于所有进程，**源** ** **使用筛选器按钮，选项卡可筛选数据。 默认情况下，字段特定添加到每个流程的筛选器。 例如，对于流程**从总帐生成预算计划**，**会计科目**和**主科目**类别可用，并且在生成页显示。 您添加到筛选器的所有字段也将添加到此页，以及您添加的任何条件。
...
**目标**选项卡上的很多字段将变为可编辑或只读，具体取决于您选择的操作。 在您将创建新预算计划更改为更新现有计划时，**预算计划名称**字段将变为不可用，与选择现有计划相关的字段将变为可用。 **在两个目标**源** **选项卡和选项卡上，**会计**字段始终不可用，因为所选预算计划过程取决于值。 
...
**最小值**字段可以设置创建预算计划行的阈值金额。 如果源金额少于此数字，预算计划行不创建。 **值 0.00 **允许所有金额，但是不界限行的正金额。 (对正金额的值未界限行。 负金额始终包括的并且通常表示贷方分录。)
...
**从固定资产生成预算计划**流程不具有按期间或天聚合的选项。 还未设置计划的选项。历史。 您可以使用该定期过程到包括项目的交易记录以用于在固定资产的预算计划。
...
在预算计划的行将按会计科目合计和职位。 但是，您可以排除位置编号，行，以便只按会计科目合计。 在**目标**选项卡上，将**在预算计划行中包含职位**选项设置为**否**。
