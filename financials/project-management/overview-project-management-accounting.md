...
项目为其设计提供服务，生产产品或这些结果一组活动。 一种收入或资产的窗体中，项目使用资源生成和财务结果。

## <a name="projects-across-industries"></a>跨行业的项目
项目管理和核算功能可在多个行业中使用，如下图所示。 在行业中的![[] (项目。/media/projects-accross-industries.jpg)](。/media/projects-accross-industries.jpg) 

在呼叫中心，票据可用于描述解决呼叫所需的一系列行动。 查阅公司，例如管理或技术参考的组织或广告商，请参阅为其活动项目。 在市场活动，则市场表示必须交付的某项工作。 在基于项目的制造中，生产订单与必须完成才能生产某些成品的各种工作相关。 任何名称。项目包含它们使用，这些资源、计划和成本，而项目管理与核算功能在 Microsoft Dynamics 365 中的工序与这些项目可帮助、计划执行和分析。
...
在项目开始后，可以使用多参数流程情况。 可以使用项目报价单将估计的人工、支出和材料与客户关联起来。 您可以记录付款条款，限制，在项目合同的协议。 您可以使用工作 (WBS) 结构分解计划估计和工作。 可以设置预测预算和引导项目执行。 下图显示的项目结构。[] (![项目结构。/media/project-structure1.jpg)](。/media/project-structure1.jpg)  
...
您在 Microsoft Dynamics 365 中的六可以创建项目种类型操作。 每个项目类型。成本和收入确认设置存在不同的要求。 项目类型的选择取决于项目的用途。 下表描述了每个项目类型的典型用法。

| 项目类型      | 描述 
|-------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 时间和材料 | 在时间和材料项目中，客户按项目中产生的所有成本进行结算。 这些成本包括工时、支出、物料和费用的成本。                                                                                                                  |
| 固定价格       | 在固定价格项目，分期付款发票包括交易记录。 固定价格项目根据基于项目合同缴费开票。 可使用“已完成百分比”方法在整个项目过程中计算和过帐固定价格项目的收入。 或者，也可以使用“已完成的合同”方法在完成项目时计算和过帐收入。 公司通常可以从使用在制品 (WIP) 计算项目或项目组的完成级别中获益。                                                                                                                                     |
| 投资        | 投资项目是不会立即产生收益的项目。 他们通常用于必须对成本进行资本化的长期内部项目。 只能为投资项目记录物料、工时和支出的成本。 使用估计功能跟踪并控制投资项目中的成本。 设置投资项目时可以采用可选的最大资本化。 随着投资项目的进展，您应将其成本记录在 WIP 帐户中，在项目完成前，该成本处于暂停状态。 在清除项目后，您将 WIP 值转移到固定资产、会计科目或新项目。
> [!NOTE] 
> 在投资项目的交易记录。** ** **过帐成本，应计收入** **或创建发票方案**页面不显示。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 _=__=_成本项目_=_像投资项目，成本项目通常用于跟踪，内部项目，并且工时、支出和物料仅可用于它们记录。 但是，成本项目通常比投资项目期限短。 此外，与投资项目不同，成本项目无法资本化为资产负债表科目。 而仅将其项目交易记录过帐到损益科目。 
[!NOTE] 
> 成本项目的交易记录。** ** **过帐成本，应计收入** **或创建发票方案不会反映**页面。 因为成本项目通常用于跟踪内部项目，它们通常不必与客户帐户。 但是，如果您的设置要求为采购订单创建物料需求，则必须将成本项目与客户相关联。 此关联是必需的，因为物料需求被当作销售订单行管理，并且系统要求指定客户。 但是，此设置不会导致根据采购订单自动创建物料需求。 对于成本项目，将忽略**“创建物料需求”**设置。 如果您在成本项目中需要物料需求，您可以手动创建它，前提是客户与项目关联。 _=__=_内部_=_内部项目用于跟踪费用在的是内部与组织之间的项目。 内部项目可以提供多种信息管理工具计划资源消耗量。 **注释：**内部项目的交易记录上应计收入** **或** **页面创建发票方案不会反映。                                                                                                                                                                                                                                                                       _=__=_时间_=_时间项目用于跟踪与不应计费和非生产活动的时间，例如项目跟踪病假的工作人员。 时间项目中的交易记录不会过帐到分类帐。 而是，它们包含在工作人员利用报表中。 在时间项目上只能记录工时交易记录。 您使用 1 小时日志或工时将这些工时登记到项目。 在注册工时后，它们显示为项目交易记录，但没有对应的凭证交易记录。

> [!NOTE] 
> 交易记录将反映在时间项目上过帐成本，** ** **应计收入** **或创建发票方案**页。                                                                                                                                                                                       |
...
有两种方式管理和控制您的项目：项目预测和项目预算。 如果您的组织有工序方面的考虑并且重点关注从特定交易产生的收入和成本，您可以使用预测。 但是，如果您的组织更关注财务金额，您可以使用预算。
...
-   ****订单****的 item/no 链接–将项目链接到物料需求。 通过使用此方法，在生产的状态为**“已开始”**或完工入库时可以查看实际项目成本。 该成本将过帐为原材料的多个项目物料交易记录和生产的工时消耗量。 还可以定义生产中项目跟踪到的物料清单层次结构中的级别。
...
-   要从公司库存订购物料，您必须将订单作为物料需求输入。 **如果使用物料需求**页上，可以设置该需求，以便您接收物料作为部分交货。 因此，您可以延迟一定数量的物料的消耗，直到需要这些物料。
-   要从外部供应商订购物料，您必须在**“采购订单”**页上将此订单作为采购订单创建。

> [!NOTE] 
> 如果物料为包装，已标记某一项目相关销售订单的装箱单不能取消。 
...
<tr class="odd">
<td>总成本 - 实际</td>
<td>估计成本必须手动输入。 填写<strong>“成本预估”</strong>页上的<strong>“总成本”</strong>或<strong>“总数量”</strong>列后，应将用户输入的总计减去实际成本。 结果就是完成项目的成本。通常，成本进度的跟踪不是基于每个期间内记录的酒店住宿和餐饮次数。 相反， 跟踪通常基于与估计工时的总量的较。 此方法不需要预测模型，并且可以手动更改总成本或总数量。 当此值或 <strong>总成本</strong> 在列时 <strong>总数量</strong> 输入，工序 365 的 Microsoft Dynamics {{对：against}}在期间中过帐的实际交易记录进行比较此值，然后或减去列中 <strong>完成数量</strong>  <strong>完工尚需成本</strong> 的值。</td>
</tr>
<tr class="even">
<td>总预算 - 实际</td>
...
工序的 Microsoft Dynamics 365 不自动减少预测的评估。 因此，最好在固定价格项目上保留原始预测模型，以建立在完成项目时进行比较的基准。 
> [!NOTE] [!NOTE]，如果您选中此方法时，使用至少两个预测模型。 一个模型应包含原始预测。 对于其他模型，应从另一个模型中复制预测交易记录。 此方法仅对固定价格项目和投资项目有效。</td>
> </tr>
<tr class="odd">
<td>剩余预算</td>
...
-   **项目日期：**2012 年 3 月 5 日。
-   **发票日期：** 2012 年 3 月 31 日。 此日期基于发票频率确定。 在本例中，您将开票频率设置为当前月。 因此，在三月份过帐的所有交易记录都将在该月的最后一天开票。
-   **到期日期：** 2012 年 4 月 14 日。 此日期基于为项目设置的付款期限确定。 在本例中，您选择了 14 天的付款期限。 因此，发票日期加上 14 天就是到期日期 2012 年 4 月 14 日。
-   **预期销售付款日期：** 2012 年 4 月 27 日。 日期通过添加此天数。**项目管理与核算参数** **页面的一般缓冲天数**字段为**各缓冲天数**字段**项目合同**页面的天数，然后添加到合计的计算日在** **到期日期字段。 在本例中，您在**“一般缓冲天数”**字段中输入了 **3**，并在**“个别缓冲天数”**字段中输入了 **10**。 因此，到期日期加上 13 天就是预期销售付款日期 2012 年 4 月 27 日。
...
-   **发票日期：**2012 年 3 月 2 日。
-   **到期日期：** 2012 年 3 月 16 日。 付款期限设置为 14 天。
-   **预期销售付款日期：** 2012 年 3 月 29 日。 计算包含三个一般缓冲天数和 10 的各缓冲天数。
...
> [!NOTE] 
> 项目，在创建采购订单时，采购订单的到期日期基于供应商交易记录。 该到期日期不由任何默认设置决定。 
...
> [!NOTE] 
> **在您使用成本控制**时"页以查看项目成本当前状态，使用原始和剩余预算所选的预测模型。 如果您在计算成本时选择了其他预测模型，计算结果将不准确。
