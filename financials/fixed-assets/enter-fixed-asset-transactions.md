...
# <a name="fixed-asset-transaction-options"></a>固定资产交易记录的选项
...
您可以为普通发票页面的固定资产交易记录。  在普通发票页，在网格发票行，请选择该行项。 单击“行明细”快速选项卡。 为处置交易记录输入固定资产编号和帐簿。 对于普通发票，固定资产交易记录的类型始终是“处置 - 出售”。
