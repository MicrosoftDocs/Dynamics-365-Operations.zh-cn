...
# <a name="recalculate-replacement-costs-and-insured-values-for-fixed-asset-groups"></a>重新计算重置成本和保险额的固定资产组值
...
-   \[(资产组的重置成本系数/\[) + 100\] \* 资产的现有重置成本
-   \[(资产组的保险额系数/\[) + 100\] \* 资产的现有保险额
...
默认情况下，该更新流程将更新当天尚未更新、但前几天可能已更新的所选资产。 例如，&lt; 当前日期表示“今天之前”。 您在更新重置成本和保险额可以更改该日期通过单击"选择"按钮。 您指定的日期条件将与该资产的最后定期更新的日期进行比较（“固定资产”窗体中的“最近期间价值/成本更新”字段）。 每次您成功更新某一固定资产的重置成本或保险额后，系统都将自动使用当前日期更新“最近期间价值/成本更新”字段。 

示例 

您昨天按 5% 更新了车辆、办公家具和建筑物组的重置成本，并且现在考虑准确更新这些资产。 要排除这些资产，在当天中更新其他固定资产，则在某在昨天的最后一个期间值/成本更新字段中输入日期 (&lt; 昨天日期) 前，车辆的，因为最后一次更新，办公家具和建筑物组，在发生日期条件。您输入。
