---
title: "普通日记帐处理"
description: "文章在此可以帮助将普通日记帐处理更容易，还可以帮助保障工序的 Microsoft Dynamics 365 功能中描述的正确数据捕获，并且内部控制没有影响。"
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
一个到设置的重要区域是日志名称。 它是一个环节。定义特定的日志名称。每个目的，如内部公司，应计调整和错误更正。 您可以剪裁每个日志名称有助于使每个目的数据输入易于和安全。 

在**“日记帐名称”**页上，您可以设置以下因素：

-   **工作流审核** - 要增强内部控制，请基于总借方金额等条件定义日记帐工作流（用于确定审查和审核步骤的重要性限制）。 您一般日志的设置工作流。**总帐工作流**页。
-   **默认值** - 为对方科目、币种和财务维度选择的默认值。
-   **日记帐控制** - 您可以设置对公司和科目类型的限制以及对科目段值的限制。 

**示例**

日记帐名称只能用于调整。 在这种情况下，您可以指定只有**“分类帐”**科目类型在所有公司都有效。 日志控制![[] (科目类型。/media/journal 控制帐户 types1.png)](。/media/journal 控制帐户 types1.png)

日记帐名称只能用于某个特定科目段或用于主科目的某个范围。 [] (![日志控制科目段。/media/journal 控制 segment1.png)](。/media/journal 控制 segment1.png)

**“自动冲销”**选项可用于普通日记帐。 例如，您有一个应计调整，其中的实际单据尚未处理，如下图所示。
冲销![[] (的普通记帐日志。/media/general 日志 reversing1.png)](。/media/general 日志 reversing1.png) 

日志条目的 Microsoft Excel 外接程序提供附加级别的自动化并将数据输入更容易。 **“在 Excel 中打开行”**操作可用于**“普通日记帐”**和**日记帐凭证**页。 

在**“期间日记帐”**页上，您可以设置重复日记帐以自动执行日记帐处理。 

您可以在任何时间使用凭证模板。 **在普通记帐日志**页，**保存** **，并且选择"凭证模板**操作。**日志凭证**页面找到，则下**功能**的凭证行。
...
有关详细信息，请参阅：[计划会计科目表{{计划：plan-chart-of-accounts.md}}] ({{图表：plan-chart-of-accounts.md}} {{accounts.md:plan-chart-of-accounts.md}})。 
