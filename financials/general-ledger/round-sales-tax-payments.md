...
销售税需要定期申报和缴纳给税务主管机构。 可以通过运行在页面的销售税结算流程完成和过帐增值税。 增值税期间要结算增值税帐户，并且增值税余额将过帐到增值税结算科目。 销售税余额，在销售增值税结算帐户过帐，可以通过在“销售税”页设置舍入规则来按照税务主管机构的要求舍入。 
...
3.  在“舍入”字段中，输入 1.00。
4.  在向税务主管机构支付销售税时，打开“结算和过帐销售税”页。 (单击纳税 &gt;&gt; 申报增值 &gt; 税结算和过帐增值税。)
...
有关详细信息，请参阅增值税概览 [] (overview.md 间接税)。 
