...
分组多个跟踪违反记录以及与审核案例相关。 默认情况下，由审核策略规则分组每个审核策略的案例。 如果您需要，可以使用**案例分组条件**页选择其他分组条件。 例如，可以按支出项目 ID 组标题和供应商发票按供应商帐户。 在这种情况下，具有相同项目 ID 的所有支出标题违反在同一箱肉和具有相同供应商帐户的所有供应商发票在同一箱肉将组进行分组。 

> [!NOTE]
> 对于基于"审核的策略规则**查询类型，请重复**未违反政策规则或按组由在的条件组条件**的案例**指定页面。 相反，由建立到审核策略规则的条件对它们进行分组。 例如，如果策略规则评估相同金额、商人 ID 和日期的副本支出的支出报表，在这些字段中具有相同值的所有费用是一个情况。 具有不同值的所有支出将作为单独案例。
...
有关详细信息，请参阅已审核策略规则 [] () rules.md 审计策略
