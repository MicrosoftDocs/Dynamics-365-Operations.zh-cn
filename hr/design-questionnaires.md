---
title: "设计调查表"
description: "此主题描述调查表创建的流程。 第一步是设计调查表。 在您设计调查表时，您不仅编写问题和回答，还要创建结构，以便对回答进行记录和制表。"
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
此主题描述调查表创建的流程。 第一步是设计调查表。 在您设计调查表时，您不仅编写问题和回答，还要创建结构，以便对回答进行记录和制表。 
...
编号序列代码用于组织问题和回答。 基于这些编号规则代码值，会自动向该物料。 

在开始创建数据之前，应先定义所有参数。 可以随时修改调查表参数设置。

## <a name="questionnaire-components"></a>调查表组成部分
调查表包含三个主要元素：包含不同选择问题、问题和调查表的回答的回答组。 您可以选择彼此组对调查表的问题为结果组。 结果组能让您对问题进行分类，并提供对调查表的进一步分析。 
