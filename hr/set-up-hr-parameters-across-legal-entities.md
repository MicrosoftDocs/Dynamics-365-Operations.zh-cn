...
在**“标识”**选项卡上，您必须选择表示页面上列出的标识号的标识类型。 在您可以输入工作的人员的标识信息之前，必须设置标识类型。 有关社会保险号、国家/地区 ID 号、侨民 ID 号和个人 ID 代码的信息保留在**“标识类型”**页上。 若要定义一新标识键入或查看现有列表的类型，单击** ** &gt; **人力资源设置** &gt; ** **标识类型。 您可以输入简单的代码和描述。 
...
-   ** ** –始终可以分配工作人员。新职位，当创建职位时。 当创建职位时，**可用于分配**日期和时间。**职位** ** **页面的常规选项卡将自动设置为创建日期和时间。
...
[设置的特定公司人力资源参数] (设置特定于公司的 hr parameters.md)
