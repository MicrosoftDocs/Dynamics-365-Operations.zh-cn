...
固定薪酬指员工的标准总薪水或工资。 此主题描述必须设置的组件，然后才可以创建固定薪酬计划和登记员工。

固定薪酬金额可以基于绩效、区域和预算增加等系数为您的员工计算。 支持工序的 Microsoft Dynamics 365 步骤，并且结合，分段薪酬类型。

## <a name="fixed-compensation-components"></a>固定薪酬组件
### <a name="compensation-levels"></a>薪酬级别

您使用薪酬水平** **可以设置各种作业的薪酬，帮助进行保障那些工作的员工向金额。 **在薪酬水平**页，您可以设置为获得，每个步骤都要求，并且结合计划的薪酬水平。 使用 **Up** 和 **Down** 按钮根据级别类型将它们放到正确顺序。 通过为工作设置薪酬级别，您可以帮助确保位于此职位的所有员工在同一级别付薪。

### <a name="reference-points"></a>参考点

**参考点**是定义每个级别的薪酬范围的网格中的列。 薪酬级别是网格中的行。 等级类型的计划是典型的参考点最小值、中点最大数量。 **在您创建参考点设置的参考点**页面。
...
**雇用规则**设置使您可以指定任意员工是否应接收相同增加，而不考虑其雇用日期的雇用规则 = (** ** = ****无)，或者员工是否应接收奖励百分比，它们基于处于已雇用了在周期 ( ** = 雇用规则 entity_PLACEHOLDER = ** **百分比)。 
...
在您定义固定薪酬计划的基础后，您可以设置计划的薪酬结构。 **薪酬**单击安装。 为您提供三个选择的滑块打开"对话框：
...
在固定薪酬计划中登记员工的第一步是确定适用于在计划中定义的薪酬的员工。 在您确定资格前，您将无法将计划分配到任何员工。 若要设置资格，打开**资格规则**页。 在此处，您创建薪酬计划的新的资格规则并定义员工必须与有资格享受计划的条件。 您可以基于部门、工会、薪酬区域（位置）、工作、工作职能、工作类型或薪酬级别限制资格。 只有当员工满足在资格规则中设置的所有条件时，他们才能在薪酬计划中登记。 
...
资格规则还会考虑与员工关联的工会 (在** **员工"页上，在** **工作人员"选项卡上，依次单击**个人信息** &gt; **工会**)。
...
您现在可以将员工分配到固定薪酬计划。 打开**员工**页，然后选择要在薪酬计划中登记的员工。 在操作窗格上，依次单击** ** &gt; ** **固定薪酬计划。 您现在创建该员工的新固定薪酬操作。 
