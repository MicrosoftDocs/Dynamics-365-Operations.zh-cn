...
对于 Attract 和 Onboard，所有数据都是面向应用的 Common Data Service (CDS) 的本地数据。

## <a name="can-i-create-a-new-mapping-without-using-the-templates"></a>我可以不使用模板创建新映射吗？

模板是起点。 您可以创建自己的模板，但在创建集成项目时始终需要模板。 有关数据集成器 (DI)、模板和项目的详细信息，请参阅[将数据集成到面向应用的 Common Data Service](https://docs.microsoft.com/en-us/powerapps/administrator/data-integrator)。
...
Financial dimensions 当前不在面向应用的 CDS 中，因此不是默认模板的一部分。 此实体已计划，但当前未确定发布时间。
...
字段可以在集成任务中添加或删除。 并非面向应用的 CDS (CDS 2.0) 上存在的所有数据字段都从 Core HR 填充。
...
数据集成器的首次运行始终是完整运行。 后续运行基于更改跟踪。 在错误运行执行时，它会提取运行作用域内的记录并发出 CDS 中的最近更改。
...
可以，字段可以在现有模板中添加或删除。 模板可以修改成包括其他面向应用的 CDS 实体中的其他数据。 实体必须在面向应用的 CDS 中才能够包含在模板中。 
...
- 数据转移导致源 (CDS) 发生了重复的记录提取。

- 数据转移在 Finance and Operations 中必填的字段中包含空值。 验证 CDS 中的数据以及是否满足 Finance and Operations 的要求。
...
与 Finance and Operations 的集成是一个两步流程。 首先，验证 Talent 数据在 CDS 中已更新并可用。 这是一个接近实时的同步，可以在 PowerApps 中通过查看数据实体中的数据验证。

![CDS 中的数据](media/DataInCDS.png)

如果数据未按预期出现在 CDS 中，请验证集成中是否支持该实体。 若要在 CDS 中包括附加数据，需要 Microsoft 一端进行更改。

如果实体受支持，且数据在 CDS 中可用，请验证映射在数据集成器中是正确的。 如果集成器映射看起来正常，则验证数据管理作业是否成功运行。 错误可能在执行批处理作业时发生。 有关数据管理的详细信息，请参阅[数据管理](https://docs.microsoft.com/en-us/dynamics365/unified-operations/dev-itpro/data-entities/data-entities-data-packages?toc=/fin-and-ops/toc.json)。
...
**位置 ID** 的编号规则在 Talent 和 Finance and Operations 中使用相同模式。 编号规则在两端都需要是唯一的，以便在将数据从 CDS 集成到 Finance and Operations 时没有地址冲突。
...
确保您在映射到正确的法人。 法人同步不是默认模板的一部分，因此，Talent 和 CDS 中存在的每个法人也会出现在 Finance and Operations 中。
...
- 数据集成器 (DI)： 
  ...
  - [将数据集成到面向应用的 Common Data Service](https://docs.microsoft.com/en-us/powerapps/administrator/data-integrator)

  - [数据集成器错误管理和故障排除](https://docs.microsoft.com/en-us/powerapps/administrator/data-integrator-error-management)

  - [响应 PowerApps、Microsoft Flow 和面向应用的 Common Data Service 中系统生成日志的 DSR 请求](https://docs.microsoft.com/en-us/powerapps/administrator/powerapps-gdpr-dsr-guide-systemlogs)
