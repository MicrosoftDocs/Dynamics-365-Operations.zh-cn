...
5. **版本 10.0.9 及更高版本：** 确定是否应将视图发布为所选用户的默认视图。 将某视图设为默认视图时，用户在下次打开目标页面时将看到该视图。 每个目标用户的一个全局默认视图将被更改。 不过，发布后，用户仍然可以更改其默认视图。
6. 添加与此视图针对的用户对应的安全角色。 
7. **版本 10.0.13 及更高版本：** 确定是否要将视图发布到所选的每个安全角色的子角色。 如果这样做，请选中相应安全角色行中的 **包括子角色** 复选框。 请注意，此复选框不可用于没有子角色的角色。
7. 添加此视图应该可用于的法人。 
8. 选择 **发布**。
...
4. 选择 **发布**。 
5. **版本 10.0.8 及更早版本：** 如果更新了已发布视图的名称，您还必须删除具有旧名称的已发布视图。 （有关详细信息，请参阅[管理已发布视图](saved-views.md#managing-published-views)一节。）

**版本 10.0.9 及更高版本：** 如果最初选择此发布视图作为默认视图，在重新发布后，它将再次成为用户的默认视图。

如果对已发布视图的更改涉及修改与该视图关联的个性化设置或筛选器，请执行以下步骤： 

**版本 10.0.13 及更高版本：** 直接对视图进行所需的更改。 星号 (\*) 应显示在视图名称旁边。
...
- **发布的视图** – 此部分列出已为您的组织发布的所有视图。 从这里，您可以在调整视图所对应的安全角色或法人后重新发布视图。 您还可以导出、删除或取消发布视图。 在版本 10.0.12 及更高版本中，您可以使用 **另存为个人** 操作创建视图的个人副本，以便您可以更新视图或更好地了解它的内容。 
- **未发布视图** – 此部分列出系统中当前尚未发布的所有组织视图。 这些视图大部分是通过导入功能进入系统的。 您可以发布、导出或删除这些视图。 在版本 10.0.12 中添加的 **快速发布** 操作可以使用现有的安全角色和法人配置，在一个操作中发布此部分的多个视图。 在版本 10.0.12 及更高版本中，您可以使用 **另存为个人** 操作创建这些视图的个人副本，以便您可以更好地了解视图的内容。
...
有权访问 **个性化设置** 页面的用户还可以使用操作窗格上的 **导入视图** 按钮导入个人或组织视图。 在版本 10.0.12 及更高版本中，添加了一种机制，用于在导入视图时立即发布视图。
...
**已保存视图** 功能已不再处于预览阶段。 现在，它可以在任何环境中通过“功能管理”直接使用。

**版本 10.0.9 到 10.0.12**

在任何环境中，**已保存视图** 功能均可直接在“功能管理”中使用。 对于其他预览功能，能否在生产环境中启用此功能取决于[补充使用条款协议](https://go.microsoft.com/fwlink/?linkid=2105274)。

**10.0.8/平台更新 32 及更早版本**

可以在第 1 层（开发/测试）和第 2 层（沙盒）环境中启用 **已保存视图** 功能，以按照以下步骤进行其他测试和设计更改。
