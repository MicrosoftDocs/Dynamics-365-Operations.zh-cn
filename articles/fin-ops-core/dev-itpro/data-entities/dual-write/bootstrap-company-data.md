...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="e0fad-103">使用公司数据自扩展常见问题</span><span class="sxs-lookup"><span data-stu-id="e0fad-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [preview-banner](../../includes/preview-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="e0fad-104">为什么需要自扩展？</span><span class="sxs-lookup"><span data-stu-id="e0fad-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="e0fad-105">您可能已有一个包含业务数据的 Common Data Service 或其他 Dynamics 365 应用实例，并且想要对其启用双写入连接。</span><span class="sxs-lookup"><span data-stu-id="e0fad-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="e0fad-106">在这种情况下，您需要在启用双写入连接之前使用公司信息自扩展 Common Data Service 或其他 Dynamics 365 应用数据。</span><span class="sxs-lookup"><span data-stu-id="e0fad-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="e0fad-107">我什么时候应该使用自扩展？</span><span class="sxs-lookup"><span data-stu-id="e0fad-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="e0fad-108">应在启用双写入实体映射之前（步骤 #5）使用自扩展。</span><span class="sxs-lookup"><span data-stu-id="e0fad-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="e0fad-109">要在 Finance and Operations 应用的实例与 Common Data Service 或其他 Dynamics 365 应用之间设置双写入连接，请以管理员身份登录到 Finance and Operations 应用。</span><span class="sxs-lookup"><span data-stu-id="e0fad-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="e0fad-110">转到**数据管理**模块，然后单击**双写入**按钮。</span><span class="sxs-lookup"><span data-stu-id="e0fad-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="e0fad-111">这将启动**数据集成器**。</span><span class="sxs-lookup"><span data-stu-id="e0fad-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="e0fad-112">为一个或多个公司创建双写入连接。</span><span class="sxs-lookup"><span data-stu-id="e0fad-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="e0fad-113">![创建双写入连接](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="e0fad-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="e0fad-114">启用 **Cdm_companies** 实体映射。</span><span class="sxs-lookup"><span data-stu-id="e0fad-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="e0fad-115">这会使公司从 Finance and Operations 应用同步到 Common Data Service。</span><span class="sxs-lookup"><span data-stu-id="e0fad-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="e0fad-116">![启用实体映射](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="e0fad-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="e0fad-117">在 Common Data Service 或其他 Dynamics 365 应用实例上运行示例自扩展代码。</span><span class="sxs-lookup"><span data-stu-id="e0fad-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="e0fad-118">当自扩展完成并且系统准备好实时同步后，启用实体映射。</span><span class="sxs-lookup"><span data-stu-id="e0fad-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="e0fad-119">启用实体映射会触发已启用实体映射的初始数据同步。</span><span class="sxs-lookup"><span data-stu-id="e0fad-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="e0fad-120">与在双写入连接上选择的公司相对应的数据将在 Finance and Operations 应用与 Common Data Service 之间同步。</span><span class="sxs-lookup"><span data-stu-id="e0fad-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="e0fad-121">如何使用代码示例？</span><span class="sxs-lookup"><span data-stu-id="e0fad-121">How to I use the code sample?</span></span>
<span data-ttu-id="e0fad-122">示例代码是一个 C# 应用程序，您可以在 Visual Studio 中加载它。</span><span class="sxs-lookup"><span data-stu-id="e0fad-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="e0fad-123">它获取 Common Data Service 上的 NuGet 包依赖项，您可以通过标准 Visual Studio 工具进行刷新。</span><span class="sxs-lookup"><span data-stu-id="e0fad-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="e0fad-124">解压缩并在 Visual Studio 中打开解决方案并还原 NuGet 包后，在代码中搜索 **TODO**。</span><span class="sxs-lookup"><span data-stu-id="e0fad-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="e0fad-125">**TODO** 指出了针对您要如何自扩展公司信息需要做的每个决策，以及用于规范实现的示例代码。</span><span class="sxs-lookup"><span data-stu-id="e0fad-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="e0fad-126">示例代码仅显示您可以按公司对实体记录进行分类的多种方式中的一种。</span><span class="sxs-lookup"><span data-stu-id="e0fad-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="e0fad-127">通过更改 **TODO** 部分的逻辑，您可以创建自定义分类。</span><span class="sxs-lookup"><span data-stu-id="e0fad-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="e0fad-128">我会看到什么结果？</span><span class="sxs-lookup"><span data-stu-id="e0fad-128">What should I expect?</span></span>
<span data-ttu-id="e0fad-129">默认情况下，示例应用程序使您可以提供业务单位到公司代码映射的字典。</span><span class="sxs-lookup"><span data-stu-id="e0fad-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="e0fad-130">您使用 **OwningBusinessUnit** 字段进行自扩展的任何实体都会自动设置为使用指定的公司。</span><span class="sxs-lookup"><span data-stu-id="e0fad-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="e0fad-131">没有 **OwningBusinessUnit** 字段的任何实体（例如产品）都将根据映射使用空业务单位值设置公司。</span><span class="sxs-lookup"><span data-stu-id="e0fad-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="e0fad-132">控制台应用程序需要一个参数，即 **–simulate** 或 **–apply**。</span><span class="sxs-lookup"><span data-stu-id="e0fad-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="e0fad-133">如果您使用 **–simulate** 命令行参数，则不会更新数据。</span><span class="sxs-lookup"><span data-stu-id="e0fad-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="e0fad-134">在与工具相同的目录中仅生成 **simulation_<entityname>.csv** 文件，每个更新的实体一个。</span><span class="sxs-lookup"><span data-stu-id="e0fad-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="e0fad-135">您可以在确保代码按预期更新公司值的同时迭代地查看这些文件。</span><span class="sxs-lookup"><span data-stu-id="e0fad-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="e0fad-136">完成模拟更新后，使用 **–apply** 参数。</span><span class="sxs-lookup"><span data-stu-id="e0fad-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="e0fad-137">这将更新当前具有不正确公司值的所有记录，按一次更新 1000 个记录的批次进行（默认）。</span><span class="sxs-lookup"><span data-stu-id="e0fad-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="e0fad-138">所提供的代码是幂等的，这意味着您可以重新运行它，并且仅会更新错误分配的公司。</span><span class="sxs-lookup"><span data-stu-id="e0fad-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="e0fad-139">当使用 **–apply** 运行时，代码将输出所做更改的 CSV 文件，这些文件名为 **applied_<entityname>.csv**。</span><span class="sxs-lookup"><span data-stu-id="e0fad-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
