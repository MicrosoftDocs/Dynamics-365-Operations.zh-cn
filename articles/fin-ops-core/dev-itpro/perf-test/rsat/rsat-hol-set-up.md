...
8. 如果要在本地保存任务录制，然后通过 LCS 上载到 BPM，请执行以下步骤：
    ...
    4. 在 **概述** 选项卡上，选择 **上载**。
        ...
        ![“上传”按钮。](./media/setup_rsa_tool_37.png)

    5. 选择 **浏览**，然后选择之前保存的 .axtr 文件。 然后选择 **上载**。

        ![选择要上传的 .axtr 文件。](./media/setup_rsa_tool_38.png)
...
4. 转到 **可信根证书主管机构 \> 证书**。
    ...
    ![“受信任的根证书颁发机构”文件夹下的“证书”文件夹。](./media/setup_rsa_tool_47.png)
...
6. 如果系统中未安装浏览器驱动程序，将收到警告消息，说明“此操作需要 \<browser name\> 驱动程序。 是否要立即自动下载并安装？“ 选择 **是**。
    ...
    ![“结果”字段设置为“失败”。](./media/setup_rsa_tool_72.png)
...
5. 选择 **上载** 将 Excel 参数文件保存到 Azure DevOps。
    ...
    ![上传成功消息。](./media/setup_rsa_tool_82.png)
...
- 选择 **运行** 以重新运行测试用例，并且验证测试用例是否已通过。 可按照[查看测试结果](#view-the-test-results)部分中的说明查看测试结果。
    ...
    ![“结果”字段设置为“通过”。](./media/setup_rsa_tool_84.png)
...
4. 选择为 **发布产品** 创建的新测试用例，然后选择页面右下角的 **添加测试用例**（下图中未显示此按钮）。
    ...
    ![“向套件添加测试用例”页面。](./media/setup_rsa_tool_100.png)
