...
- 为了与应用程序一起使用而配置的 SharePoint Server 的访问权限
- 访问 Power BI 框架
...
4. 在 **文档类型** 页面上创建将用来访问您刚才创建的 SharePoint 文件夹的新文档类型。 在 **组** 字段中输入 **文件**，在 **位置** 字段中输入 **SharePoint**，然后输入 SharePoint 文件夹的地址。
...
5. 单击 **重复执行** 以计划此批处理执行所需的重复周期。 重复周期定义了更新数据转移至 Power BI 的频率。
...
5. 为此数据集配置强制定期更新的刷新计划。 定期更新允许通过定期执行 ER 报表的新业务数据通过在 SharePoint Server 上创建的 Excel 文件的新版本消耗。
...
3. 将 Power BI 报表保存为 **导入和导出详细报表**。
    ...
    [![导入和导出详细信息报表。](./media/ger-power-bi-added-report-1024x498.png)](./media/ger-power-bi-added-report.png)

    注意地图显示 Excel 文件中提及的国家/地区（在此示例中为奥地利和瑞士）。 这些国家/地区通过颜色显示每个国家/地区开票金额的比例。

4. 更新内部统计交易记录列表。 添加由意大利发起的导出交易记录。

    [![内部统计交易列表。](./media/ger-power-bi-new-run-new-transaction-1024x321.png)](./media/ger-power-bi-new-run-new-transaction.png)
...
## <a name="access-power-bi-report-in-finance"></a>在 Finance 中访问 Power BI 报表
设置与 Power BI 的集成。 有关详细信息，请参阅[配置工作区的 Power BI 集成](configure-power-bi-integration.md)。
...
3. 单击操作项以打开显示您在 Power BI 中设计的报表的页面。
    ...
    [![在 Power BI 中设计的导入和导出详细信息报表。](./media/ger-power-bi-review-bi-report-in-ax-form-1024x586.png)](./media/ger-power-bi-review-bi-report-in-ax-form.png)
