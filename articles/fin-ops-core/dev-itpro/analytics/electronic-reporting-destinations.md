...
您只能为已[导入](general-electronic-reporting.md#importing-an-er-component-from-lcs-to-use-it-internally)到 Finance 实例的 ER 配置以及**电子报告配置**页提供的格式设置目标。 ER 目标管理功能在**组织管理** \> **电子报告** \> **电子报告目标**中可用。 在**电子报告目标**页面上，您可以覆盖配置的默认行为。 直到选择**新建**，然后在**引用**字段中选择要为其创建目标设置的配置，才会在此页面上显示导入的配置。
...
>
> 目前，从 Excel 输出生成的 PDF 文档中仅支持横向页面方向。
>
