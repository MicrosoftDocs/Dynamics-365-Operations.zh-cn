---
title: 整理报表设计器中的报表组件
description: 本文介绍了如何组织现有报表、构建基块和报表设计器中的对象。
author: aprilolson
ms.date: 11/22/2022
ms.topic: article
ms.prod: ''
...
您可以在 Report designer 中重命名文件夹、报表、构建基块和其他对象以帮助整理您的文件。 根据重命名的对象的类型，您可能必须更新与该对象的关联。

## <a name="rename-a-folder-or-building-block-in-report-designer"></a>在 Report designer 中重命名文件夹或构建基块
在 Report designer 中，您可以重命名文件夹、报表定义、行定义、列定义和报告树定义。

### <a name="rename-a-folder-or-building-block-in-report-designer"></a>在 Report designer 中重命名文件夹或构建基块

1. 在 Report designer 中，使用导航窗格找到要重命名的文件夹或对象。
...
1. 在 Report designer 中，单击导航窗格中的 **报表组**。
2. 在 **文件** 菜单上，单击 **新建** &gt; **报表组定义** 在查看器窗口中打开一个新报表组。 或者，单击工具栏上的 **报表组** 按钮 ![报表组。](media/report-group.gif "报表组") 。
3. 单击 **报表组** 选项卡。要替代用于生成此报表的各个报表定义的相关信息，请选中 **替代各个报表定义中的公司、详细信息和日期设置** 复选框。 公司名称、详细级别、临时设置和日期信息将自动输入，但您仍可以进行更新。
...
4. 在 **文件** 菜单上，单击 **保存** 以保存修改的报表组，或者单击工具栏中的 **保存** 按钮 ![保存。](media/save.gif "保存") 。

> [NOTE] 如果您已计划了报表以便它们以设置的间隔生成，则可以替代这些设置并立即生成报表。
...
<tbody>
<tr>
<td>通过各个报表定义重写公司、详细信息和日期设置</td>
<td>选中此复选框可替代此报表组中的报表的各个报表定义（仅用于生成这些报表）。</td>
</tr>
<tr>
<td>公司名称</td>
<td>选择要用于报表的公司。</td>
...
<tr>
<td>包括所有申报币种</td>
<td>在 Microsoft Dynamics 365 Finance 系统中配置的所有其他申报币种均在此处列出。 选择此复选框以生成使用指定的币种的其他报表。 若要在 Web 查看器中查看这些报表，请单击<strong>币种</strong>，然后选择一种货币。</td>
</tr>
<tr>
