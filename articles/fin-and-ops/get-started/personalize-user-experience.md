...
可对页面执行的某些最典型、最重要的更改直接以快捷菜单中的选项的形式提供。 例如，若要在网格中添加或隐藏列，只需右键单击网格列标题，然后选择**添加列**或**隐藏此列**。
...
如果希望移动或隐藏元素，或者对一个页面进行多项更改，可使用**个性化设置**工具栏。 若要打开**个性化设置**工具栏，请在元素的属性窗口中选择**个性化设置此窗体**。 也可以在各页面的操作窗格的**选项**选项卡上**个性化**组中选择**个性化设置此窗体**。

[![个性化设置工具栏](./media/personalization-personalizationtoolbar.jpg)](./media/personalization-personalizationtoolbar.jpg)

当**个性化设置**工具栏打开后，页面将为非交互式。 因此，您无法输入数据或展开或折叠分区。 只能更改构成该页面的元素。
