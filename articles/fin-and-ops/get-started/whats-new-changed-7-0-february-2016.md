...
<td>具有组合计科目的余额表作为 SSRS 报表 (LedgerAccountSum\_FR) 实现。</td>
...
<td>内部统计商品代码有一个文本字段，以标识其他单位，**产品** 卡有一个字段来标识其他单位（千克）的数量。</td>
...
| 通过使用新的**作业卡设备**页，启动并报告生产作业的进度。                                                              | “工作登记”****窗体主要面向大型终端屏幕，UI 通常通过鼠标单击即可访问。 | 尽管新的 **作业卡设备** 页在设计上力求简化，但同时也考虑了触摸体验。 该页适合移动设备，例如平板电脑和手机。 车间工作人员将发现信息重载更少，而且更直观、易用。 工作人员可以执行传统任务，例如开始、结束、报告作业进度。 除了实际工作或注销并下班外，工作人员可以查看附件、午餐休息和执行其他活动。 工作按计划的顺序排队分给对工作人员，不过，工作也可以由工作人员选取。 该页主要面向离散制造工序，在那里为生产准备了物料。 对于与联产品和副产品的报告相关的情况，以及按照跟踪维度领取的物料，请使用**工作登记**页。 | 通过引入一种针对触摸而设计且可以从所有类型的设备上访问的备选 UI（例如终端屏幕和移动设备），此功能将帮助减少车间登记的传统推出的实施成本。 |
