...
|                                       与 Microsoft Office 集成。                                       |                                                                                           不可用                                                                                            |                                              新增功能包括 Excel Data Connector 应用程序、“工作簿设计器”<strong></strong>页、“导出 API”和文档管理。                                              |                                                                                                                                                                                                                                                                      您可以为您的最终用户创建工作效率解决方案。                                                                                                                                                                                                                                                                       |
...
<td>FactBox 已添加到“会计科目表”<strong></strong>页。</td>
<td> 当定义并分配会计科目表时，对重要信息的访问更容易。</td>
</tr>
<tr class="odd">
<td>查看“试算平衡表”<strong></strong>列表页上的单独列中的结算单调整和期末交易记录。 </td>
<td>用户在单个列中查看这两种类型的交易记录。</td>
<td>一个附加参数已添加到“试算平衡表”<strong></strong>列表页。</td>
...
<td>跟踪财务期间结帐流程。</td>
<td>不可用 </td>
<td>通过使用财务期间结帐配置，可以创建结帐模板和计划。 使用“财务期间结帐”<strong></strong>工作区可跨多个公司跟踪结帐计划的进度。</td>
...
<tr class="even">
<td>为预算计划和预测创建布局。</td>
<td>“预算计划”<strong></strong>文档显示为一个行列表，其中各行具有财务维度组合的生效日期和金额。 用户必须创建和使用 Excel 模板查看数据透视表中的预算计划数据。</td>
...
<tr class="odd">
<td>通过使用“未结明细表”<strong></strong>报表（其中包括逾期天数）中的信息，打印“供应商发票交易记录”<strong></strong>报表。 </td>
<td>您必须首先打印两个不同报表：“未结明细表”<strong></strong>和“供应商发票交易记录”<strong></strong>。</td>
<td>有关这两个报表的信息已合并到“供应商发票交易记录”<strong></strong>报表中。 “未结明细表 ”<strong></strong>报表已弃用。 </td>
...
<td>不可用</td>
<td>在“销售税结算期间”<strong></strong>页上，您可以指定结算流程应以批处理模式运行。</td>
...
<td>您必须通过使用未实现的销售增值税功能来生成“销售和采购增值税”<strong></strong>报表，因此，用户可以基于状态来识别属于已实现部分和未实现部分的交易记录。</td>
<td>“销售和采购增值税”<strong></strong>报表已经修改，现在仅通过未实现和已实现销售税代码的定义的特定结算期间来考虑特殊增值税功能。</td>
...
<td>不可用</td>
<td>在“固定资产参数”<strong></strong>页上，您可以选择在会计期间或会计年度结束时跟进。</td>
...
| 在单独的页面上针对生产订单执行物料可用性检查，该页面可从“生产车间管理”<strong></strong>工作区中打开。 |                                                                 不可用                                                                  |
|                               通过使用新的“作业卡设备”<strong></strong>页，启动并报告生产作业的进度。                                | “工作登记”<strong></strong>窗体主要面向大型终端屏幕，UI 通常通过鼠标单击即可访问。 |
