...
1. 打开 Regression Suite Automation Tool (RSAT)，然后选择您在[设置和安装 Regression Suite Automation Tool](./hol-set-up-regression-suite-automation-tool.md) 中创建的测试用例。
2. 选择**新建 \> 创建派生测试用例**。
...
可使用此功能验证是否执行了某个操作。 例如，创建、估计，然后开始履行生产订单时，Microsoft Dynamics 365 for Finance and Operations 将显示“生产 - 开始”消息，通知您已开始履行生产订单。
...
运行测试用例之后，将把 Excel 参数文件中的消息与 Finance and Operations 中显示的消息进行比较。 如果这些消息不匹配，测试用例将失败。
