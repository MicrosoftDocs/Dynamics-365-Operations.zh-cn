...
## <a name="before-you-begin"></a><span data-ttu-id="d5793-110">开始之前</span><span class="sxs-lookup"><span data-stu-id="d5793-110">Before you begin</span></span>
...
### <a name="create-a-new-azure-devops-project"></a><span data-ttu-id="d5793-128">创建新 Azure DevOps 项目</span><span class="sxs-lookup"><span data-stu-id="d5793-128">Create a new Azure DevOps project</span></span>
...
3. <span data-ttu-id="d5793-143">填写以下字段，然后选择**创建**：</span><span class="sxs-lookup"><span data-stu-id="d5793-143">Fill in the following fields, and then select **Create**:</span></span>
    ...
    - <span data-ttu-id="d5793-144">**项目名称**</span><span class="sxs-lookup"><span data-stu-id="d5793-144">**Project name**</span></span>
...
### <a name="configure-lcs-to-connect-to-the-azure-devops-project"></a><span data-ttu-id="d5793-181">配置 LCS 以连接到 Azure DevOps 项目</span><span class="sxs-lookup"><span data-stu-id="d5793-181">Configure LCS to connect to the Azure DevOps project</span></span>
...
1. <span data-ttu-id="d5793-237">打开 Finance and Operations 客户端。</span><span class="sxs-lookup"><span data-stu-id="d5793-237">Open the Finance and Operations client.</span></span>
...
### <a name="install-an-authentication-certificate"></a><span data-ttu-id="d5793-306">安装身份验证证书</span><span class="sxs-lookup"><span data-stu-id="d5793-306">Install an authentication certificate</span></span>
...
3. <span data-ttu-id="d5793-342">双击 .msi 包运行安装程序。</span><span class="sxs-lookup"><span data-stu-id="d5793-342">Double-click the .msi package to run the installer.</span></span> <span data-ttu-id="d5793-343">然后，在安装完成时，选择**完成**。</span><span class="sxs-lookup"><span data-stu-id="d5793-343">Then, when the installation is completed, select **Finish**.</span></span>
...
5. <span data-ttu-id="d5793-417">选择**关闭**关闭对话框。</span><span class="sxs-lookup"><span data-stu-id="d5793-417">Select **Close** to close the dialog box.</span></span>
...
- <span data-ttu-id="d5793-492">选择**运行**以重新运行测试用例，并且验证测试用例是否已通过。</span><span class="sxs-lookup"><span data-stu-id="d5793-492">Select **Run** to rerun the test case, and verify that the test case has passed.</span></span> <span data-ttu-id="d5793-493">可按照[查看测试结果](#view-the-test-results)部分中的说明查看测试结果。</span><span class="sxs-lookup"><span data-stu-id="d5793-493">You can view the test results as described in the [View the test results](#view-the-test-results) section.</span></span>

    ![“结果”字段已设置为“通过”](./media/setup_rsa_tool_84.png)

### <a name="chaining-of-test-cases"></a><span data-ttu-id="d5793-495">链接测试用例</span><span class="sxs-lookup"><span data-stu-id="d5793-495">Chaining of test cases</span></span>

<span data-ttu-id="d5793-496">RSAT 的一项关键功能是链接测试用例（即一个测试将值传递给其他测试这项功能）。</span><span class="sxs-lookup"><span data-stu-id="d5793-496">One key feature of RSAT is the chaining of test cases (that is, the capability of a test to pass values to other tests).</span></span> <span data-ttu-id="d5793-497">将根据在 Azure DevOps 测试计划中为测试用例定义的顺序运行测试用例。</span><span class="sxs-lookup"><span data-stu-id="d5793-497">Test cases are run according to their defined order in the Azure DevOps test plan.</span></span> <span data-ttu-id="d5793-498">（也可以在测试工具本身中更新此顺序。）因此，如果要将变量从一个测试用例传递到另一个，则测试顺序务必正确。</span><span class="sxs-lookup"><span data-stu-id="d5793-498">(This order can also be updated in the test tool itself.) Therefore, if you want to pass variables from one test case to another, it's very important that the tests be in the correct order.</span></span>

<span data-ttu-id="d5793-499">此部分中，将在第一个测试用例中创建已保存变量，创建第二个测试用例，然后将已保存变量从第一个测试用例传递到第二个测试用例。</span><span class="sxs-lookup"><span data-stu-id="d5793-499">In this section, you will create a saved variable in the first test case, create a second test case, and pass the saved variable from the first test case to the second test case.</span></span> <span data-ttu-id="d5793-500">然后将在 RSAT 中把测试用例作为链式测试用例运行。</span><span class="sxs-lookup"><span data-stu-id="d5793-500">You will then run the test cases as a chained test case in RSAT.</span></span>

#### <a name="modify-an-existing-task-recording-to-create-a-saved-variable"></a><span data-ttu-id="d5793-501">修改现有任务录制以创建已保存变量</span><span class="sxs-lookup"><span data-stu-id="d5793-501">Modify an existing task recording to create a saved variable</span></span>

1. <span data-ttu-id="d5793-502">打开 Finance and Operations 客户端。</span><span class="sxs-lookup"><span data-stu-id="d5793-502">Open the Finance and Operations client.</span></span>
2. <span data-ttu-id="d5793-503">选择**设置**按钮（齿轮符号），然后选择**任务录制器**。</span><span class="sxs-lookup"><span data-stu-id="d5793-503">Select the **Settings** button (the gear symbol), and then select **Task recorder**.</span></span>
3. <span data-ttu-id="d5793-504">选择**编辑录制**。</span><span class="sxs-lookup"><span data-stu-id="d5793-504">Select **Edit Recording**.</span></span>

    ![“编辑录制”按钮](./media/setup_rsa_tool_85.png)

4. <span data-ttu-id="d5793-506">选择**从 Lifecycle Services 打开**。</span><span class="sxs-lookup"><span data-stu-id="d5793-506">Select **Open from Lifecycle Services**.</span></span>

    ![“从 Lifecycle Services 打开”按钮](./media/setup_rsa_tool_86.png)

5. <span data-ttu-id="d5793-508">选择**选择 Lifecycle Services 库**。</span><span class="sxs-lookup"><span data-stu-id="d5793-508">Select **Select the Lifecycle Services library**.</span></span>

    ![“选择 Lifecycle Services 库”按钮](./media/setup_rsa_tool_87.png)

    <span data-ttu-id="d5793-510">将从 LCS 加载 BPM 库。</span><span class="sxs-lookup"><span data-stu-id="d5793-510">BPM libraries are loaded from LCS.</span></span>

    ![进度指示器](./media/setup_rsa_tool_88.png)

6. <span data-ttu-id="d5793-512">从 LCS 加载 BPM 库之后，选择任务录制关联的 **RSAT** BPM 库和**创建新产品**业务流程。</span><span class="sxs-lookup"><span data-stu-id="d5793-512">After the BPM libraries are loaded from LCS, select the **RSAT** BPM library and the **Create a new product** business process that the task recording was associated with.</span></span> <span data-ttu-id="d5793-513">然后选择**确定**。</span><span class="sxs-lookup"><span data-stu-id="d5793-513">Then select **OK**.</span></span>

    ![选择 BPM 库和业务流程](./media/setup_rsa_tool_89.png)

7. <span data-ttu-id="d5793-515">将在**录制名称**字段中输入相应任务录制的名称。</span><span class="sxs-lookup"><span data-stu-id="d5793-515">The name of the appropriate task recording is entered in the **Recording name** field.</span></span> <span data-ttu-id="d5793-516">选择**开始**。</span><span class="sxs-lookup"><span data-stu-id="d5793-516">Select **Start**.</span></span>

    ![“录制名称”字段中的任务录制名称](./media/setup_rsa_tool_90.png)

8. <span data-ttu-id="d5793-518">转到**产品信息管理 \> 产品**，然后选择**新建**打开在其中录制了原始任务录制（即**创建产品**）的页面。</span><span class="sxs-lookup"><span data-stu-id="d5793-518">Go to **Product information management \> Products**, and select **New** to open the page where the original task recording, **Create a product**, was recorded.</span></span>
9. <span data-ttu-id="d5793-519">选择**插入步骤**。</span><span class="sxs-lookup"><span data-stu-id="d5793-519">Select **Insert step**.</span></span>

    > [!NOTE]
    > <span data-ttu-id="d5793-520">将在窗格中选择的步骤**后面**插入新步骤。</span><span class="sxs-lookup"><span data-stu-id="d5793-520">The new step is inserted **after** the step that you selected in the pane.</span></span>

    ![“插入步骤”按钮](./media/setup_rsa_tool_91.png)

10. <span data-ttu-id="d5793-522">右键单击**产品编号**字段，然后选择**任务录制器 \> 复制**。</span><span class="sxs-lookup"><span data-stu-id="d5793-522">Right-click the **Product number** field, and then select **Task recorder \> Copy**.</span></span>

    ![“复制”命令](./media/setup_rsa_tool_92.png)

11. <span data-ttu-id="d5793-524">将在窗格中添加一个新步骤。</span><span class="sxs-lookup"><span data-stu-id="d5793-524">A new step is added in the pane.</span></span> <span data-ttu-id="d5793-525">记下**产品编号**字段中的值，因为后面需要。</span><span class="sxs-lookup"><span data-stu-id="d5793-525">Make a note of the value in the **Product number** field, because you will need it later.</span></span>

    ![添加了新步骤](./media/setup_rsa_tool_93.png)

12. <span data-ttu-id="d5793-527">选择**完成编辑**。</span><span class="sxs-lookup"><span data-stu-id="d5793-527">Select **Done editing**.</span></span>
13. <span data-ttu-id="d5793-528">选择**保存到 Lifecycle Services**，然后将新任务录制与原始任务录制关联的相同 BPM 库和业务流程关联。</span><span class="sxs-lookup"><span data-stu-id="d5793-528">Select **Save to Lifecycle Services**, and associate the new task recording with the same BPM library and business process that the original task recording was associated with.</span></span> <span data-ttu-id="d5793-529">有关详细信息，请参阅[创建任务录制并保存到 BPM 库](#create-a-task-recording-and-save-it-to-the-bpm-library)部分。</span><span class="sxs-lookup"><span data-stu-id="d5793-529">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>
14. <span data-ttu-id="d5793-530">转到 BPM 库，然后选择**同步测试用例**以按照[测试从 BPM 到 Azure DevOps 的同步](#test-the-synchronization-from-bpm-to-azure-devops)部分中的说明在 Azure DevOps 中覆盖附加到测试用例的任务录制。</span><span class="sxs-lookup"><span data-stu-id="d5793-530">Go to the BPM library, and select **Sync testcases** to overwrite the task recording that is attached to the test case in Azure DevOps, as described in the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>
15. <span data-ttu-id="d5793-531">打开 RSAT，然后选择**加载**以重新加载测试套件中的所有测试用例。</span><span class="sxs-lookup"><span data-stu-id="d5793-531">Open RSAT, and select **Load** to reload all the test cases in the test suite.</span></span> <span data-ttu-id="d5793-532">必须按照[加载和运行测试用例](#load-and-run-test-cases)部分中的说明为相应测试用例重新生成自动化文件和参数文件，方法是选择测试用例，然后选择**新建 \> 生成测试执行和参数文件**。</span><span class="sxs-lookup"><span data-stu-id="d5793-532">You must regenerate the automation and parameter files for the appropriate test case by selecting the test case and then selecting **New \> Generate Test Execution and Parameter files**, as described in the [Load and run test cases](#load-and-run-test-cases) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="d5793-533">如果 Excel 参数文件未关闭，重新生成将失败。</span><span class="sxs-lookup"><span data-stu-id="d5793-533">If the Excel parameter file was left open, regeneration will fail.</span></span> <span data-ttu-id="d5793-534">因此，生成新的 Excel 参数文件之前，确保关闭测试用例的 Excel 参数文件。</span><span class="sxs-lookup"><span data-stu-id="d5793-534">Therefore, make sure that the Excel parameter file for the test case is closed before you generate the new Excel parameter file.</span></span>

16. <span data-ttu-id="d5793-535">选择**编辑**打开新的 Excel 参数文件。</span><span class="sxs-lookup"><span data-stu-id="d5793-535">Select **Edit** to open the new Excel parameter file.</span></span> <span data-ttu-id="d5793-536">将在行 9 中看到新的**已保存变量**条目。</span><span class="sxs-lookup"><span data-stu-id="d5793-536">You will see a new **Saved variable** entry on line 9.</span></span> <span data-ttu-id="d5793-537">此变量（即 **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**）将保存到任务录制的 XML 文件，并且可以在后续测试中使用。</span><span class="sxs-lookup"><span data-stu-id="d5793-537">This variable, **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**, is saved in the task recording's XML file and can be used in subsequent tests.</span></span>

    ![“已保存变量”条目](./media/setup_rsa_tool_94.png)

#### <a name="create-a-new-test-case"></a><span data-ttu-id="d5793-539">创建新的测试用例</span><span class="sxs-lookup"><span data-stu-id="d5793-539">Create a new test case</span></span>

1. <span data-ttu-id="d5793-540">转到 **RSAT** BPM 库。</span><span class="sxs-lookup"><span data-stu-id="d5793-540">Go to the **RSAT** BPM library.</span></span>
2. <span data-ttu-id="d5793-541">选择**示例支持业务流程**流程，然后选择右侧的**编辑模式**。</span><span class="sxs-lookup"><span data-stu-id="d5793-541">Select the **Sample Support Business Process** process, and then, on the right, select **Edit mode**.</span></span>
3. <span data-ttu-id="d5793-542">将**名称**字段和**说明**字段的值更改为**发布产品**。</span><span class="sxs-lookup"><span data-stu-id="d5793-542">Change the value of both the **Name** field and the **Description** field to **Release a product**.</span></span> <span data-ttu-id="d5793-543">然后选择**保存**。</span><span class="sxs-lookup"><span data-stu-id="d5793-543">Then select **Save**.</span></span>

    ![“名称”和“说明”已更改为“发布产品”](./media/setup_rsa_tool_95.png)

#### <a name="create-a-new-task-recording-that-has-a-validate-function"></a><span data-ttu-id="d5793-545">创建具有验证功能的新任务录制</span><span class="sxs-lookup"><span data-stu-id="d5793-545">Create a new task recording that has a Validate function</span></span>

- <span data-ttu-id="d5793-546">创建任务录制以将之前创建的产品发布给 USRT 法人。</span><span class="sxs-lookup"><span data-stu-id="d5793-546">Create a task recording to release the product that was created earlier to the USRT legal entity.</span></span> <span data-ttu-id="d5793-547">有关详细信息，请参阅[创建任务录制并保存到 BPM 库](#create-a-task-recording-and-save-it-to-the-bpm-library)部分。</span><span class="sxs-lookup"><span data-stu-id="d5793-547">For more information, see the [Create a task recording and save it to the BPM library](#create-a-task-recording-and-save-it-to-the-bpm-library) section.</span></span>

    > [!NOTE]
    > <span data-ttu-id="d5793-548">对于链式测试用例，我们始终建议您通过*手动键入字段的值*查找或筛选您需要的记录。</span><span class="sxs-lookup"><span data-stu-id="d5793-548">For chained test cases, we always recommend that you find or filter for the record that you require by *manually typing the value of the field*.</span></span> <span data-ttu-id="d5793-549">这样，此工具就可以确定后续测试用例中必须对其执行操作的记录。</span><span class="sxs-lookup"><span data-stu-id="d5793-549">In that way, the tool can determine the record that the action must be taken against in the subsequent test case.</span></span>

    ![具有验证功能的新任务录制](./media/setup_rsa_tool_96.png)

    <span data-ttu-id="d5793-551">如上图所示，使用快速筛选器找到产品之后，选择**发布产品**之前，请验证**产品编号**字段的值以确保产品 ID 是之前创建的产品 ID。 </span><span class="sxs-lookup"><span data-stu-id="d5793-551">As the preceding illustration shows, after the product is found by using the Quick Filter, but before you select **Release products**, you validate the value of the **Product number** field to make sure that the product ID is the product ID that was created earlier.</span></span> <span data-ttu-id="d5793-552">若要验证该值，请右键单击**产品编号**字段，然后选择**任务录制器 \> 验证 \> 当前值**。</span><span class="sxs-lookup"><span data-stu-id="d5793-552">To validate the value, right-click the **Product number** field, and then select **Task recorder \> Validate \> Current Value**.</span></span>

    ![验证当前值](./media/setup_rsa_tool_97.png)

#### <a name="save-the-task-recording-to-bpm"></a><span data-ttu-id="d5793-554">将任务录制保存到 BPM</span><span class="sxs-lookup"><span data-stu-id="d5793-554">Save the task recording to BPM</span></span>

1. <span data-ttu-id="d5793-555">完成任务录制后，选择**保存到 Lifecycle Services**。</span><span class="sxs-lookup"><span data-stu-id="d5793-555">After the task recording is completed, select **Save to Lifecycle Services**.</span></span>

    ![保存选项](./media/setup_rsa_tool_98.png)

2. <span data-ttu-id="d5793-557">将从 LCS 加载库信息。</span><span class="sxs-lookup"><span data-stu-id="d5793-557">Library information is loaded from LCS.</span></span>

    ![进度指示器](./media/setup_rsa_tool_99.png)

3. <span data-ttu-id="d5793-559">选择要与任务录制关联的 BPM 库。</span><span class="sxs-lookup"><span data-stu-id="d5793-559">Select the BPM library to associate the task recording with.</span></span> <span data-ttu-id="d5793-560">对于本教程，请选择之前创建的 **RSAT** BPM 库及其下的**发布产品**业务流程。</span><span class="sxs-lookup"><span data-stu-id="d5793-560">For this tutorial, select the **RSAT** BPM library that was created earlier and the **Release a product** business process under it.</span></span> <span data-ttu-id="d5793-561">然后选择**确定**。</span><span class="sxs-lookup"><span data-stu-id="d5793-561">Then select **OK**.</span></span>

#### <a name="sync-bpm-to-azure-devops"></a><span data-ttu-id="d5793-562">将 BPM 同步到 Azure DevOps</span><span class="sxs-lookup"><span data-stu-id="d5793-562">Sync BPM to Azure DevOps</span></span>

1. <span data-ttu-id="d5793-563">转到 BPM 库，然后打开 **RSAT** 库。</span><span class="sxs-lookup"><span data-stu-id="d5793-563">Go to the BPM library, and open the **RSAT** library.</span></span>
2. <span data-ttu-id="d5793-564">选择 **VSTS 同步**，然后选择**同步测试用例**。</span><span class="sxs-lookup"><span data-stu-id="d5793-564">Select **VSTS sync** and then **Sync test cases**.</span></span> <span data-ttu-id="d5793-565">有关详细信息，请参阅[测试从 BPM 到 Azure DevOps 的同步](#test-the-synchronization-from-bpm-to-azure-devops)部分。</span><span class="sxs-lookup"><span data-stu-id="d5793-565">For more information, see the [Test the synchronization from BPM to Azure DevOps](#test-the-synchronization-from-bpm-to-azure-devops) section.</span></span>

    <span data-ttu-id="d5793-566">同步完成后，Azure DevOps 中**面板 \> 工作项**内将显示**发布产品**业务流程的新工作项和相应测试用例。</span><span class="sxs-lookup"><span data-stu-id="d5793-566">After the synchronization is completed, the new work item and the corresponding test case for the **Release a product** business process appear in Azure DevOps at **Boards \> Work Items**.</span></span>

#### <a name="add-the-new-test-case-to-the-existing-test-suite"></a><span data-ttu-id="d5793-567">将新测试用例添加到现有测试套件</span><span class="sxs-lookup"><span data-stu-id="d5793-567">Add the new test case to the existing test suite</span></span>

1. <span data-ttu-id="d5793-568">转到**测试计划 \> 测试计划**，然后选择 **RSAT 测试计划**计划。</span><span class="sxs-lookup"><span data-stu-id="d5793-568">Go to **Test plans \> Test plans**, and select the **RSAT Test Plan** plan.</span></span>
2. <span data-ttu-id="d5793-569">选择**添加现有**。</span><span class="sxs-lookup"><span data-stu-id="d5793-569">Select **Add existing**.</span></span>
3. <span data-ttu-id="d5793-570">在**向套件添加测试用例**页中，选择**运行查询**。</span><span class="sxs-lookup"><span data-stu-id="d5793-570">On the **Add test cases to suite** page, select **Run query**.</span></span>
4. <span data-ttu-id="d5793-571">选择为**发布产品**创建的新测试用例，然后选择页面右下角的**添加测试用例**（下图中未显示此按钮）。</span><span class="sxs-lookup"><span data-stu-id="d5793-571">Select the new test case that was created for **Release a product**, and then select **Add test cases** in the lower-right corner of the page (this button isn't shown in the following illustration).</span></span>

    ![“向套件添加测试用例”页](./media/setup_rsa_tool_100.png)

    <span data-ttu-id="d5793-573">此测试套件现在有两个测试用例。</span><span class="sxs-lookup"><span data-stu-id="d5793-573">The test suite now has two test cases.</span></span>

    ![测试套件中的两个测试用例](./media/setup_rsa_tool_101.png)

#### <a name="load-test-cases-into-rsat"></a><span data-ttu-id="d5793-575">将测试用例加载到 RSAT 中</span><span class="sxs-lookup"><span data-stu-id="d5793-575">Load test cases into RSAT</span></span>

1. <span data-ttu-id="d5793-576">打开 RSAT，然后选择**加载**。</span><span class="sxs-lookup"><span data-stu-id="d5793-576">Open RSAT, and select **Load**.</span></span>
2. <span data-ttu-id="d5793-577">将加载测试用例，而您会收到警告，说明“此操作将覆盖 Excel 测试数据文件，本地更改将丢失。</span><span class="sxs-lookup"><span data-stu-id="d5793-577">The test cases are loaded, and you receive a warning that states, "This action will overwrite Excel test data files, local changes will be lost.</span></span> <span data-ttu-id="d5793-578">是否要继续?”</span><span class="sxs-lookup"><span data-stu-id="d5793-578">Do you want to proceed?"</span></span> <span data-ttu-id="d5793-579">选择**是**以更新本地系统中的 Excel 参数文件，但是不更新上载到 Azure DevOps 的 Excel 参数文件。</span><span class="sxs-lookup"><span data-stu-id="d5793-579">Select **Yes** to update the Excel parameter files in the local system but not the Excel parameter files that were uploaded to Azure DevOps.</span></span>

    ![警告消息](./media/setup_rsa_tool_102.png)

    <span data-ttu-id="d5793-581">将加载这两个测试用例，以及第一个测试用例的 Excel 参数文件。</span><span class="sxs-lookup"><span data-stu-id="d5793-581">Both the test cases are loaded, together with the Excel parameter file for the first test case.</span></span> <span data-ttu-id="d5793-582">因为在上一个运行中选择了**上载**，所以将从 Azure DevOps 提取参数文件。</span><span class="sxs-lookup"><span data-stu-id="d5793-582">Because you selected **Upload** in the last run, the parameter files are pulled from Azure DevOps.</span></span>

    ![已加载测试用例](./media/setup_rsa_tool_103.png)

3. <span data-ttu-id="d5793-584">仅选择第二个测试用例，然后选择**新建 \> 生成测试执行和参数文件**。</span><span class="sxs-lookup"><span data-stu-id="d5793-584">Select only the second test case, and then select **New \> Generate test execution and parameter files**.</span></span>

#### <a name="edit-the-excel-parameter-file"></a><span data-ttu-id="d5793-585">编辑 Excel 参数文件</span><span class="sxs-lookup"><span data-stu-id="d5793-585">Edit the Excel parameter file</span></span>

1. <span data-ttu-id="d5793-586">仅选择第二个测试用例，然后选择**编辑**打开相应 Excel 参数文件。</span><span class="sxs-lookup"><span data-stu-id="d5793-586">Select only the second test case, and then select **Edit** to open the corresponding Excel parameter file.</span></span>
2. <span data-ttu-id="d5793-587">将已保存变量 **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}**（请参阅[修改现有任务录制以创建已保存变量](#modify-an-existing-task-recording-to-create-a-saved-variable)部分）从第一个测试用例复制到使用产品编号的所有字段中。</span><span class="sxs-lookup"><span data-stu-id="d5793-587">Copy the **{{EcoResProductCreate\_Identification\_ProductNumber\_Copy}}** saved variable (see the [Modify an existing task recording to create a saved variable](#modify-an-existing-task-recording-to-create-a-saved-variable) section) from the first test case into all the fields where the product number is used.</span></span> <span data-ttu-id="d5793-588">在此情况下，将把变量复制到 **EcoResProductListPage** 表的**产品编号**和**验证产品编号**字段中。</span><span class="sxs-lookup"><span data-stu-id="d5793-588">In this case, you copy the variable into the **Product number** and **Validate Product number** fields on the **EcoResProductListPage** sheet.</span></span>

    ![“产品编号”和“验证产品编号”字段](./media/setup_rsa_tool_104.png)

    > [!NOTE]
    > <span data-ttu-id="d5793-590">只有在同一个测试运行期间才能在测试之间传递变量。</span><span class="sxs-lookup"><span data-stu-id="d5793-590">Variables can be passed between tests only during the same test run.</span></span> <span data-ttu-id="d5793-591">变量的名称必须完全匹配。</span><span class="sxs-lookup"><span data-stu-id="d5793-591">The names of the variables must match exactly.</span></span>

3. <span data-ttu-id="d5793-592">选择**保存**，然后关闭 Excel 工作簿。</span><span class="sxs-lookup"><span data-stu-id="d5793-592">Select **Save**, and then close the Excel workbook.</span></span>
4. <span data-ttu-id="d5793-593">选择**上载**保存对 Excel 参数文件执行的更改。</span><span class="sxs-lookup"><span data-stu-id="d5793-593">Select **Upload** to save the changes that you made to the Excel parameter file.</span></span>

#### <a name="run-the-chained-test-cases"></a><span data-ttu-id="d5793-594">运行链式测试用例</span><span class="sxs-lookup"><span data-stu-id="d5793-594">Run the chained test cases</span></span>

1. <span data-ttu-id="d5793-595">同时选择这两个测试用例，然后选择**运行**。</span><span class="sxs-lookup"><span data-stu-id="d5793-595">Select both the test cases, and then select **Run**.</span></span>
2. <span data-ttu-id="d5793-596">验证这两个测试用例是否均已通过。</span><span class="sxs-lookup"><span data-stu-id="d5793-596">Verify that both test cases have passed.</span></span>
