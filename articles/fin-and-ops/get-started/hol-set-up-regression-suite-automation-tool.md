...
- 了解如何使用 Microsoft Dynamics 365 for Finance and Operations 中的任务录制器功能和 Microsoft Dynamics Lifecycle Services (LCS) 与 Microsoft Azure DevOps 创建、配置、运行、调查和报告测试用例。
- 让功能用户可以通过使用 Finance and Operations 中的任务录制器录制业务任务，然后将任务录制转换为一套自动化测试，而不必编写源代码。

## <a name="before-you-begin"></a>开始之前

### <a name="prerequisites"></a>先决条件

- 此教程需要运行 Microsoft Dynamics 365 for Finance and Operations 版本 10.0（2019 年 4 月）或更高版本的 Finance and Operations 环境。 也支持运行 Microsoft Dynamics 365 for Finance and Operations Enterprise Edition 7.3 平台更新 20 (PU20) 或更高版本的客户。
- 此类用户必须拥有此 Finance and Operations 环境的管理员权限。
...
## <a name="finance-and-operations-environment"></a>Finance and Operations 环境

### <a name="version-requirement"></a>版本要求

需要运行版本 10 的 Finance and Operations 测试或沙盒环境。 也支持运行版本 7.3 平台更新 20 或更高版本的客户。

> [!NOTE]
> RSAT 必须可以通过 Web 浏览器访问 Finance and Operations 测试环境。
...
需要执行此步骤才能与 LCS 连接，这样就可以通过 Finance and Operations 将任务录制保存到 LCS 中的相应 BPM 库。

1. 打开 Finance and Operations 客户端。
2. 转到**系统管理 \> 设置 \> 系统参数**。
...
> [!NOTE]
> 确保在 Finance and Operations 的主仪表板上启动您的所有任务录制。 让各录制简短，并注重一个业务任务，如创建销售订单。
...
可将 RSAT 安装到运行 Windows 10 并可通过 Web 浏览器（Internet Explorer 或 Google Chrome）连接到 Finance and Operations 环境的任何计算机上。
...
> [!NOTE]
> 如果 Finance and Operations 环境是二级以上环境，则必须在 wif.config 文件中为环境的**所有** AOS 计算机更新证书指纹。
...
4. 通过添加证书的新主管机构条目和主管机构名称，更新 **wif.config** 文件，如以下示例中所示。
    ...
    > [!NOTE]
    > 如果多个用户在使用同一个 Finance and Operations 应用程序，则每个用户必须生成单独的指纹，并且必须将这些指纹全部添加到 **\<keys\>** 部分中。
...
如果使用二级以上（标准接受度测试沙盒或更高）的 Finance and Operations 环境，请在安装 RSAT 的计算机上验证或设置以下注册表设置。 需要执行此步骤，是因为可以帮助您避免身份验证或 RSAT 连接错误。
...
3. 选择右上角的**设置**按钮（齿轮符号），然后填写显示的对话框中的以下字段：
    ...
    - **主机名** – 输入 Finance and Operations 测试环境的主机名，如 **\<myaos\>.cloudax.dynamics.com**。 请勿在其中包含 **https://** 或 **http://** 前缀。
    - **SOAP 主机名** – 输入 Finance and Operations 测试环境的 SOAP 主机名。 SOAP 主机名通常与主机名相同，但带有 **soap** 后缀。 下面是示例：**\<myaos\>soap.cloudax.dynamics.com**。 请勿在其中包含 **https://** 或 **http://** 前缀。
    ...
    - **管理员用户名** – 输入 Finance and Operations 测试环境中的管理员用户的电子邮件地址。
    ...
    - **公司名称** – 输入在创建 Excel 参数文件时要用作默认 Finance and Operations 公司的公司名称。 以后可通过编辑 Excel 参数文件更改公司。
...
3. 若要为每次运行生成唯一的产品编号，同时不必每次都重新打开 Excel 参数文件并手动更新产品编号，请使用以下 Excel 公式。
    ...
    > [!NOTE]
    > 除了**常规**选项卡，测试用例访问的每个 Finance and Operations 窗体页在 Excel 参数文件中都有一个数据选项卡。
