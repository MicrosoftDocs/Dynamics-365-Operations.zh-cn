...
**开始日期** 和 **结束日期** 值定义计算要重估的外币余额的日期间隔。 当您重估损益科目时，将重估发生在该日期间隔中的所有交易记录的总和。 在您重估资产负债表科目时，忽略 **开始日期**。 相反，要重估的余额通过从会计年度期初到 **结束日期** 确定。 

**汇率日期** 可以定义应默认使用汇率的日期。 例如，您可以重估日期范围为 1 月 1 日至 1 月 31 日期间的余额，但使用为 2 月 1 日定义的汇率。 

选择要重估的主科目：所有、资产负债表或损益。 将仅重估标记为重估的主科目（在 **主科目** 页面上）。 如果您要进一步限制主科目的范围，则使用 **要包括的记录** 选项卡定义主科目范围或单独的主科目。 
...
如果要从重估流程中排除使用 **申报币种调整日记帐** 过帐的调整，将 **排除申报币种调整** 设置为 **是**。 默认情况下，申报币种调整包含在重估中。 
