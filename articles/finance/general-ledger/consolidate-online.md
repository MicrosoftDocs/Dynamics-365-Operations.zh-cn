...
- **合并期间** – 此部分中的字段用于定义合并期间。
    ...
    - **从中选择合并金额** - 使用此字段指定是使用源公司的会计币种金额还是申报币种金额来更新合并公司的会计币种金额。

        - 选择 **会计币种**，以使用源公司的会计币种金额还是申报币种金额来更新合并公司的会计币种金额。 选择此值后，使用 **合并会计币种** 字段定义如何计算合并公司中的会计币种。
        - 选择 **申报币种** 以使用源公司的申报币种金额来计算合并公司的会计币种金额。

            - 如果源公司的申报币种与合并公司的会计币种相同，则从源公司将申报币种金额复制到合并公司。
            - 如果源公司的申报币种与合并公司的会计币种不同，则使用在页面的 **货币折算** 选项卡上定义的兑换信息折算这些值，以计算合并公司价值。

    - **合并申报币种** - 仅当将 **选择合并金额的来源** 字段设置为 **会计币种** 时，此字段才可用。 使用它来指定源公司的会计币种金额是通过汇率进行折算还是复制到合并公司。 选择 **使用货币折算** 以使用在 **货币折算** 选项卡上定义的汇率信息来计算合并帐户余额。 选择 **使用会计币种金额** 将会计币种金额从源公司复制到合并公司。

        - 如果源公司的会计币种与合并公司的会计币种相同，则从源公司将币种金额复制到合并公司。
        - 如果源公司的会计币种与合并公司的会计币种不同，则使用在 **货币折算** 选项卡上定义的兑换信息折算这些值，以计算合并公司价值。
...
在 **货币折算** 选项卡中，可定义法人、科目和汇率类型，以及汇率。 如果合并公司与源公司映射到了不同的主科目，则必须在 **开始日期** 和 **结束日期** 字段中输入合并公司的主科目，而不是源公司的主科目。 对于法人和主科目的每一行，**应用来自以下对象的汇率** 字段中提供了以下三个选项：

- **合并日期** - 在 **条件** 选项卡上的 **合并期间结束日期** 字段中为合并定义的日期将用于获取汇率。 此汇率等同于即期汇率或月底汇率。 将看到汇率的预览，但是不能进行编辑。
