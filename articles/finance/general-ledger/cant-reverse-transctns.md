---
title: 为什么无法冲销此交易记录？
description: 本文介绍了无法冲销交易记录的不同原因。 它还列出了此问题的解决方案。
author: kweekley
ms.date: 07/21/2021
ms.topic: article
ms.prod: ''
...
交易记录必须满足特定条件才能冲销。 本文的其余章节提供对每个模块的验证。 尽管本文侧重于 Microsoft Dynamics 365 Finance 中的交易记录，但某些概念和验证可以应用于其他应用，例如 Dynamics 365 Supply Chain Management。
...
- 可以从日记帐（总帐）或 **凭证交易记录** 页中冲销 *某些* 子分类交易记录。 不必从子分类帐页面中冲销它们。 例如，以前只能从 **供应商交易记录** 页中冲销供应商发票日记帐。 但是，现在也可以从总帐那边、从日记帐或 **凭证交易记录** 页冲销它。 本文的每个章节解释了此好处不适用的交易记录的类型。
...
如果禁用了批量冲销功能，则可以从发票的 **客户交易记录** 页或存款的 **银行帐户** 页单独冲销交易记录。 有关如何冲销付款的信息，请参阅本文后面的[现金和银行管理](cant-reverse-transctns.md#cash-and-bank-management)一节。
