...
从 10.0.21 版本开始，信用管理中的阻止规则已通过以下方式重新架构，以提供更大的灵活性：

- 已启用可扩展性请求，以便您可以创建自己的锁定规则。
- **发放销售订单** 复选框现在可用于所有锁定规则。 以前，它仅适用于销售订单锁定规则。 选中此复选框后，排除规则将下达销售订单，而不考虑可阻止销售订单的任何其他规则。 此复选框仅适用于 **排除** 规则类型。
...
6. 选择 **值类型** 以定义也将用于测试信用额度已用数量的值类型。 锁定规则和排除规则仅允许使用 **过期金额** 的百分比。 阈值与信用额度有关。
