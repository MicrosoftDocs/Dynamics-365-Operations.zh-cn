...
如果客户退回某个发票上的物料，您可能发出贷方通知单。 如果在原始发票上获得了现金折扣，客户的贷项通知单应该是由客户获得的现金折扣的净额。 如果 **应收帐款参数** 页面上的 **计算贷方通知单的现金折扣** 选项设置为 **是**，则将自动计算贷方通知单的折扣。 
...
当客户进行付款时，可能还有必须结算的数量非常少的金额。 例如，您为客户开票 1,000.00，而客户支付 999.90。 如果剩余的金额少于为 **应收帐款参数** 页面上的超额支付或不足支付指定的金额，差额将自动过帐到超额支付/不足支付会计科目。
