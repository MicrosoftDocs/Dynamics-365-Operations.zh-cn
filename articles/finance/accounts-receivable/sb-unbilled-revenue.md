...
顶级计费和未计费收入无法一起使用，因为总帐中可能会发生对帐问题。 例如，在 **物料组设置** 页面上，将设置物料组 A，以便将 **顶行数** 字段设置为 **2**。 在 **计费计划** 页上，添加了三个物料。 所有三个物料都属于物料组 A。为未计费收入功能创建初始日记帐条目时，所有三个物料的金额都将处理到未计费帐户。 创建计费计划的发票后，仅包括前两个物料的金额。 因此，发票金额与已处理到未计费收入帐户的金额不匹配，并且总帐中可能会发生对帐问题。

如果要使用未计费收入，请将 **物料组设置** 页留空，或设置所有物料组，以便将 **顶行数** 字段设置为 **0**（零）。 如果要使用顶级计费，则未计费收入操作不可用。

### <a name="examples"></a>示例

从版本 10.0.29 起，定期合同计费参数中添加了一个新参数。 当设置为“是”时，**Use unbilled offset accounts** 参数将在 **未计费收入设置** 中启用两个新帐户。 未计费的收入对方科目和未计费折扣对方科目将可用，最好在以记帐币种以外的货币创建计费计划时使用。 使用对方科目可确保未计费收入和未计费折扣科目使用与其初始条目相同的汇率进行冲销。 初始 **创建日记帐条目** 过程与借记到未计费收入和贷记到收入相同。 如果使用折扣，初始日记帐条目与借记到折扣和贷记未计费折扣相同。 

此示例说明如何使用未计费收入将资产负债表上的整个合同金额确认为未计费收入。 条目的另一端是收入或延期收入。 当您向客户开具发票时，未计费收入会被冲销。 将在开具发票时或根据设置的递延确认计划进行收入确认。
...
5. 创建包含两个未计费物料的计费计划。 合同的计费计划通过以下项目进行设置。
    ...
    |---|---|---|---|---|---|---|---|
    | 许可证 | 2022 年 1 月 1 日 | 2024 年 12 月 31 日 | 100.00 美元 | 每年 | 否 | 是 | 每年将向客户开具 100.00 美元的发票。 300.00 美元的总额将预先记录为资产负债表上的未计费收入，以及损益表上的收入。 每个发票将减少未计费金额。 |
    | 维护 | 2022 年 1 月 1 日 | 2024 年 12 月 31 日 | 30.00 美元 | 每年 | 是 | 是 | 每年将向客户开具 30.00 美元的发票。 90.00 美元的总额将在资产负债表上预先记录为未计费收入和递延收入。 每个发票将减少未计费金额。 递延收入将在 36 个月内按月确认。 |
...
合同要求在每年年初为客户创建发票。 使用 **生成发票** 流程创建发票。 创建发票时，将过帐以下发票凭证。
...
将按在接下来的两年开始时过帐的发票创建此相同的日记帐条目。 在 **生成发票** 过程中，未计费收入科目每年会减少。 未计费的收入对方科目用于在使用不同汇率时平衡未计费收入科目。 

在最后一步中，每月都会创建确认日记帐条目以确认维护费用中的延期收入。 可以使用 **识别处理** 页面创建日记帐条目。 或者，可以通过为 **延期计划** 页上的行选择 **确认** 来创建它。
...
|---|---|---|
| **物料 1000 日记帐条目** | | | 
| 未计费收入科目 (401250) | 1,465.26 美元 | |
| 延期的收入科目 (250600) | | 1,465.26 美元 |
| **物料 0021 日记帐条目** | | | 
| 未计费收入科目 (401250) | 274.74 美元 | |
| 延期的收入科目 (250600) | | 274.74 美元 |
| **账单** | | |
| 未计费收入帐户 | | 1,465.26 美元 |
| 未计费收入帐户 | | 274.74 美元 |
| AR 科目 (130100) | 1,488.16 美元 | |
