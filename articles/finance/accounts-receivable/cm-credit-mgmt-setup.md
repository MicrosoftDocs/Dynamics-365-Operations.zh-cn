...
您也可以定义宽限天数，然后再次检查信用规则。 尽管您可以指定在过帐时检查信用管理规则，但是在指定的宽限天数内不会检查规则。 例如，您在第一天确认销售订单，并为确认步骤指定两天宽限期。 在这种情况下，直到第四天才会在下一个过帐步骤（例如，创建装箱单或开具发票）中检查信用规则。 在第四天或之后，将在进行过帐时再次检查规则，宽限天数将更改为针对下一个过帐检查点指定的值。
...
- 将 **绕过库存预留** 选项设置为 **是**，以在检查信用管理锁定规则时忽略库存预留。 在这种情况下，无论库存预留数量如何，都会检查数量并启用检查点宽限期。
