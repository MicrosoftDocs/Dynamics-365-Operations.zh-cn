...
本主题提供有关如何开始使用税务计算的信息。 首先，它指导您完成 Microsoft Dynamics Lifecycle Services (LCS)、Regulatory Configuration Service (RCS) 以及 Dynamics 365 Finance 和 Dynamics 365 Supply Chain Management 中的配置步骤。 然后，它审查在 Finance 和 Supply Chain Management 交易中使用税务计算功能的通用流程。
...
13. 选择 **配置版本**。 您应该看到在步骤 8 中导入的配置版本。
    ...
    Microsoft 为税务计算加载项提供默认的税务配置。 此配置涵盖了税务计算行为的大多数要求。 它将根据市场反馈进行更新。 如果必须扩展配置以满足特定要求，请参阅[如何在税务服务中生成扩展](./tax-service-add-data-fields-tax-integration-by-extension.md)，以获取有关如何生成和选择自己的税务配置的信息。

    在选择 **配置版本** 后，将显示其他几个选项卡：

    - **税码** – 此选项卡是必需的。 它用于维护税码的主数据。 当您在法人中启用税务功能设置的当前版本时，在此选项卡上创建的所有税码会自动同步到 Finance。
    - **税码适用性** – 此选项卡是必需的。 它用于定义确定税码、税组和物料税组的矩阵。 确定的税码用于计算税额。 **税码**、**税组** 和 **物料税组** 字段的值将返回到 Finance。
    - **客户税务登记编号适用性** – 此选项卡是可选的。 如果针对一位客户有多个税务登记编号，税务计算可以自动确定正确的税务登记编号。 在此选项卡上的矩阵中，定义用于进行确定的规则。 否则，Finance 和 Supply Chain Management 将继续在销售交易的应纳税单据上使用默认的税务登记编号。
    - **供应商税务登记编号适用性** – 此选项卡是可选的。 如果针对一位供应商有多个税务登记编号，税务计算可以自动确定正确的税务登记编号。 在此选项卡上的矩阵中，定义用于进行确定的规则。 否则，Finance 和 Supply Chain Management 将继续在采购交易的应纳税单据上使用默认的税务登记编号。
    - **清单代码适用性** – 此选项卡是可选的。 它可以通过更灵活、可配置的规则帮助自动确定 **清单代码** 字段的值。 在此选项卡上的矩阵中，您可以定义用于进行确定的规则。 否则，Finance 和 Supply Chain Management 将继续在应纳税单据上使用默认代码。
...
17. 使用以下选项以标识税码的性质：
    ...
    对于不应包括在价内交易的税务基数金额计算中的某些税务类型（例如某些国家/地区中的关税），请选中 **从基数金额计算中排除** 复选框。
...
如上一部分所述，在 RCS 中完成设置后，您将具有税务功能的已发布版本。 请按照以下步骤在 Finance 中设置税务计算。
...
4. 在 **错误处理** 选项卡上，为法人定义预期的错误处理方法。 每个结果代码有三个可用的选项：
