...
3. 选择 **存储库** 。
4. 选择 **全局资源 \> 打开** 。
5. 导入 **响应消息模型** 、 **CFDI 错误日志导入 (MX)** 、 **CFDI 错误日志导入 (MX)** 和 **CFDI 响应消息导入 (MX)** 。
...
打开 **可配置电子开票附加产品集成** 功能后，用于提交 CFDI 发票的 **导出/导入电子发票** 流程（ **应收账款 \> 发票 \> 电子发票** ）流程将不能再使用。 取而代之的是一个名为 **提交电子单据** 的新流程。
