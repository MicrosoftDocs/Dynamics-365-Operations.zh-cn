...
回复 URL 非常重要，因为当您的站点调用 Azure AD B2C 以对用户进行身份验证时，其允许返回域白名单通过。 这将允许把已通过身份验证的用户返回到其用于登录（您的站点域）的域。 
...
1. 选择上面的 B2C 标识提供程序项之后，在**回复方**下执行以下操作：
    ...
    1. 在**用户类型**框中，输入**客户**。
1. 在操作窗格上，选择**保存**。
1. 在 Commerce 搜索框中，搜索**编号规则**（组织管理 > 编号规则）。
1. 在操作窗格下，选择**维护**下的**编辑**。
1. 在**常规**快速选项卡中，为**手动**选择**否**。
1. 在操作窗格上，选择**保存**。 
