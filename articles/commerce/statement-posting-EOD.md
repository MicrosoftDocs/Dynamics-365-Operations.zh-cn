...
下表列出了上述参数的推荐值。 应测试这些值并根据部署配置和可用基础结构进行定制。 推荐值的任何增加都将对其他批处理产生负面影响，因此应进行验证。
...
| 每个对帐单用于订单处理的最大线程数 | 开始测试 **4** 时的值。 通常，此值不应超过 **8**。 | 此参数指定用于创建和过帐销售订单的线程数。 它表示可用于每个对帐单过帐的线程数。 |
| 包括在聚合中的最大交易记录行数 | 开始测试 **1000** 时的值。 根据总部配置，较小的订单可能更有利于提高性能。 | 此参数确定对帐单过帐期间将包含在每个销售订单中的行数。 达到此数字后，行将拆分到新订单。 虽然销售行数不精确，但由于拆分发生在销售订单级别，因此会接近所设置的数字。 此参数用于为没有指定客户的零售交易生成销售订单。 |
| 可验证商店交易记录的最大线程数 | 我们建议您将此参数设置为 **4**，且仅在您未达到可接受的性能时再增加参数值。 此流程使用的线程数不能超过批处理服务器可用的处理器数。 如果您在此处分配的线程太多，可能会影响其他批处理。 | 此参数控制可同时为给定商店验证的交易数。 |
