...
可通过*属性*和用户定义的字段（**如内存大小**、**硬盘容量**、**符合能源之星的要求**等）进一步描述产品及其特征。 可以将属性与各种 Commerce 实体（如产品类别和通道）关联，并且可以为属性设置默认值。 当产品与产品类别或渠道关联时，产品将继承属性及默认值。 默认值可在单个产品级别、在渠道级别或在目录中被覆盖。
...
可通过属性的筛选器设置定义属性的筛选器如何在 POS 中显示。 若要访问属性的筛选器设置，请在的**属性**页面上选择属性，然后在操作窗格中选择**筛选器设置**。
...
- **阈值** - 如果选择了**范围**作为显示控制类型，则需要此设置。 可将分号 (;) 用作分隔符来定义值。
    ...
    例如，对于**包容量**这样的筛选器，阈值可以为 **10; 20; 50; 100; 200; 500; 1000; 5000**。 在此情况下，POS 将显示以下范围。 所有在结果集中无任何产品的范围都将灰显。
...
定义属性之后，可将其分配给属性组。 *属性组*用于对产品中的一个组件或子组件的单个属性进行分组。 一个属性可以包含在多个属性组中。 属性组可帮助用户配置产品，因为可将多种选择安排到特定上下文中。 可将属性组分配给类别或渠道。
...
1. 以促销经理的身份登录后端办公客户端。
2. 转至 **Retail 和 Commerce** &gt; **类别和产品管理** &gt; **Commerce 产品层次结构**。
3. 选择**时尚导航层次结构**。
...
>
> - Retail 和 Commerce &gt; 类别和产品管理 &gt; 渠道导航类别
> - Retail 和 Commerce &gt; 类别和产品管理 &gt; 补充产品类别

### <a name="assign-attribute-groups-to-stores"></a>将属性组分配到商店

一个或多个属性组可与商店层次结构中的一个或多个商店关联。 然后，为特定商店丰富产品后，这些产品将继承属性组中包含的属性。

1. 以促销经理的身份登录后端办公客户端。
2. 转到 **Retail 和 Commerce** &gt; **渠道设置** &gt; **渠道类别和产品属性**。
3. 为休斯顿渠道分配属性组：
...
单个产品的默认属性值可在产品级别覆盖。 也可以覆盖针对特定渠道的特定目录中的单个产品的默认值。

### <a name="override-the-attribute-values-of-an-individual-product"></a>覆盖单个产品的属性值

1. 以促销经理的身份登录后端办公客户端。
2. 转至 **Retail 和 Commerce** &gt; **类别和产品管理** &gt; **已发布的产品(按类别)**。
...
1. 以促销经理的身份登录后端办公客户端。
2. 转至 **Retail 和 Commerce** &gt; **目录管理** &gt; **所有目录**。
