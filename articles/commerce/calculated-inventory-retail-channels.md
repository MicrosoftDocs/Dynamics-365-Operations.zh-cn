...
要使用这两个 API，必须先在 Headquarters 中通过 **功能管理** 工作区启用 **优化的产品可用性计算** 功能。 如果您的在线渠道和商店渠道使用相同的履行仓库，您还必须启用 **增强的电子商务渠道端库存计算逻辑** 功能，以让两个 API 中的渠道端计算逻辑将在商店渠道中创建的未过帐交易（现金和结转、客户订单、退货）考虑在内。 启用这些功能后，您需要运行 **1070**（**渠道配置**）作业。
...
要将 Headquarters 中的 POS **库存查找** 操作配置为使用渠道端计算逻辑并关闭实时服务调用，请按照以下步骤操作。
