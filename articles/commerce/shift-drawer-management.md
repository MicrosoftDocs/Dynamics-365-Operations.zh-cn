...
ms.locfileid: "7779736"
---
# <a name="shift-and-cash-drawer-management"></a>班次和银箱管理

[!include [banner](includes/banner.md)]

此主题介绍如何在商业销售点 (POS) 中设置和使用班次。

在 Dynamics 365 Commerce 中，*班次* 描述两个数据点之间对 POS 交易记录数据和活动的收集。 对于每个班次，将把预计金额与盘点和清点的金额进行比较。
...
Commerce 提供多个配置选项和 POS 操作，为 POS 的各种日结业务流程提供支持。 此节介绍一些典型班次方案。
...
若要在商店中使用共享班次，必须在 **Retail 和 Commerce \> 渠道设置 \> POS 设置 \> POS 配置文件 \> 硬件配置文件 \> 银箱** 中将银箱配置为“共享班次银箱”。 此外，用户还必须拥有共享班次权限（“允许管理共享班次”和“允许使用共享班次”）之一或全部。
...
可以执行各个操作来更改班次状态或者增加或减少银箱中的金额。 此部分描述 Modern POS 和 Cloud POS 的这些班次操作。
...
转到 **Retail 和 Commerce \> 渠道 \> 商店 \> 所有商店 \> 对帐/结算**，然后为每个商店，将 **对帐方法** 字段和 **结算方法** 字段设置为 **班次**。
