...
但是，即使使用了所有这些宣传方式，客户也可能错过享受促销的机会。 若要让销售助理轻松了解哪些促销适用于所选行，甚至适用于整个购物车，零售商可以将“查看所有折扣”操作的按钮添加到 POS 中的任何按钮网格中。 建议将该按钮添加到**交易记录**页的按钮网格中。 这样，销售助理就可以选择一个交易记录行，然后选择该按钮以显示所选行的所有可用折扣。 销售助理还可以选择另一个选项卡以显示适用于整个交易的折扣。

前面介绍的**所有折扣**页仅显示不与应用的任何折扣冲突的折扣。 此行为有助于确保在销售助理向客户介绍折扣，并且客户执行了所需操作（例如，客户多购买一件商品以享受 10% 的折扣）时，将该折扣应用于交易。 前面介绍过，仅当开启了**应用但不使用优惠券代码**选项时才显示基于优惠券的折扣。
