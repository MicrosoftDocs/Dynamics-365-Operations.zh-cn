...
站点选取器模块可以添加到 [标题模块](author-header-module.md)的 **站点选取器** 插槽中。 添加站点选取器模块后，您可以定义模块标题和站点选项。 通常，标题模块包含在可以跨站点的电子商务页面共享的标题片段中。 在以下示例中，站点选取器模块已被添加到标题模块的 **站点选取器** 插槽中，该模块包含在名为 **HeaderContainer** 的标题片段中。
