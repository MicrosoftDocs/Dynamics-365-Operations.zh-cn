---
title: 裁剪图像
description: 本文介绍如何在 Microsoft Dynamics 365 Commerce 站点构建器中裁剪图像。
author: josaw1
ms.date: 04/14/2020
ms.topic: article
ms.prod: ''
...
1. 在 **模块** 下拉菜单中选择模块类型。
1. 在 **视图类型** 下拉菜单中选择视图类型。
1. 在 **放置** 下拉菜单中选择图像放置。
1. 在 **视区** 下拉菜单中选择视区。
1. 将使用代表裁剪区域的区域覆盖图像。 根据需要移动裁剪区域和调整裁剪区域的大小。 将自动保持长宽比。
1. 完成后，在命令栏中选择 **保存**，然后选择 **完成编辑**。 

完成自定义裁剪之后，图像修改几乎会立即生效。
