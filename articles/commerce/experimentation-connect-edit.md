...
本主题介绍了如何在 Commerce 中连接试验和更改您的变体，以使其与您的假设相符。 下图显示了在 Dynamics 365 Commerce 中的电子商务网站上设置和运行试验所涉及的所有步骤。 其他步骤在单独的主题中介绍。
...
- 如果您要在整个页面或片段上进行试验，则选择**整个**。 将创建默认页面或片段的单独副本。 您无需选择试验中包括哪些模块，因为整个编辑界面都可以更改。 您可以根据需要添加、删除和重新排序模块。 但是，如果对与试验相关联的默认页面或片段进行了任何更改，则必须在所有变体之间手动同步这些更改。
...
1. 若要启动向导，请在站点构建器中选择**试验**选项卡，然后选择**连接**。 或者，可以从页面或片段编辑器中访问该向导。 在编辑模式下，在命令栏中选择**连接试验**。
