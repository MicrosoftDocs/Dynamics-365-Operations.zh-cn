...
除了管理不符合项的功能外，质量管理还包括按问题类型（即使问题是内部问题）跟踪问题以及将解决方案标识为短期或长期的功能。  有关关键绩效指标 (KPI) 的统计能让您了解以前的不符合项问题以及用于更正它们的解决方案的历史记录。 您可以使用历史数据检查以前质量度量的效果并确定相应的度量以备将来使用。
...
- 根据预定义的标准自动启动质量控制流程（例如，进行特定供应商的某一采购订单的仓库登记时）。
- 在检查时锁定库存以防止使用未审核的库存（采购订单数量的完全锁定）。
- 使用物料抽样作为质量关联的一部分来定义必须检查的当前实际库存的金额。 抽样可以基于固定数量、百分比或完整牌照。
- 为部分收货创建质检订单。 若要创建基于某订单已实际接收的数量的质检订单，必须在 **物料抽样** 页上选择 **按照更新的数量** 复选框。
- 创建测试包括最小值、最大值和目标测试值在内的类型，然后执行具有预定义验证结果的定性与定量测试。
- 指定一个可接受质量级别 (AQL) 来控制质量度量容差。
- 指定检查操作所需的资源，如测试区域和测试仪器。

> [!NOTE]
> _仓库流程质量管理_ 功能扩展了质量管理能力。 如果您正在使用此功能，请参阅[仓库流程质量管理](quality-management-for-warehouses-processes.md)获取演示启用此功能时质量管理工作方式的示例。
...
- 记录不符合项的管理成本。 此成本可以包含物料（如备件）、杂项费用以及更正不符合项所需的工时单工时。
...
- 定义必须为物料执行的测试。 这些测试包括质量规范，适用的测试仪器、描述测试的文档，抽样计划和 AQL。
- 创建一个质检订单，它标识必须为特定订单（例如采购订单或生产订单）或为特定库存数量执行的测试。 您可以手动创建一个生产订单，或者基于质量准则自动生成一个质检订单。
- 在每个每个业务流程中定于与采购、检验、生产订单或销售订单相关的质量准则，以可以自动生成标识进出物料的测试要求的质检订单。
...
- [检查货物质量](tasks/inspect-quality-goods.md)
