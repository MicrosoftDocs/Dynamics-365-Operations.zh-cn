---
title: 仓库流程质量管理
description: 本文提供有关仓库流程质量管理功能的信息。 此功能扩展了质量管理功能，并允许用户使用仓库管理流程 (WMS) 将物料抽样控制集成到仓库接收流程中。
author: yufeihuang
ms.date: 03/23/2021
ms.topic: article
ms.prod: ''
...
_仓库流程质量管理_ 功能允许您使用仓库管理流程 (WMS) 将物料抽样控制集成到仓库接收流程中。 可以自动生成仓库工作，以根据百分比或固定数量或每隔 *n* 个牌照将库存移至质量控制位置。 质检订单完成后，可以根据质量结果自动生成工作，以将库存移至流程中的下一个位置。
...
1. 为 _质检物料抽样_ 和 _质检订单_ 工作订单类型创建单独的工作类。 这样，您就能确保可以基于两种工作订单类型自动生成适当的工作，然后此工作可以使用仓库管理移动应用来运行。
1. 设置每个工作订单类型的工作模板：
...
有关显示如何完成此设置的分步示例，请参阅本文末尾的[示例方案](#example-scenario)。
...
1. 转到 **仓库管理 \> 设置 \> 仓库 \> 仓库**。
1. 选择要启用质量管理的仓库。
1. 在 **仓库** 快速选项卡上，将 **启用仓库流程质检订单** 选项设置为 _是_。 （请注意，仅对于使用仓库管理流程 (WMS) 的仓库，可以将此选项设置为 _是_。）
...
> [!NOTE]
> 仅当原始凭证行上的物料使用仓库管理流程 (WMS)，并且对于原始凭证行上的仓库，**启用仓库流程质检订单** 选项设置为 _是_ 时，_仓库流程质量管理_ 功能才有效。
...
当工作人员使用仓库管理移动应用接收物料时，系统将验证是否为每个传入物料设置质量关联。 如果设置了质量关联，系统将使用为该质量关联配置的物料抽样记录，来确定它将如何创建质检订单、质检物料抽样工作和采购订单工作。
...
|---|---|---|---|---|---|---|
| 订单 | 完全牌照 | 是 _（锁定/不可编辑）_ | <p>位置：是</p><p>牌照：是 _（锁定/不可编辑）_</p> | 否 | 3 | <p>**订单行数量：100 EA**</p><ol><li>在仓库管理移动应用中登记 20 EA、LP1 的收货<p>20 EA 的质检物料抽样工作</p><p>20 EA 的质检订单 1</p></li><li>在仓库管理移动应用中登记 20 EA、LP2 的收货<p>20 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 20 EA、LP3 的收货<p>20 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 20 EA、LP4 的收货<p>20 EA 的质检物料抽样工作</p></li><li>在仓库管理移动应用中登记 20 EA、LP5 的收货<p>20 EA 的采购订单工作（储存）</p></li></ol> |
| 订单 | 固定数量 = 1 | 是 | <p>位置：是</p><p>牌照：是</p> | 否 | 不适用 | <p>**订单行数量：100**</p><ol><li>在仓库管理移动应用中登记 20 EA、LP1 的收货<p>1 EA 的质检物料抽样工作</p><p>1 EA 的质检订单 1</p><p>19 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 20 EA、LP2 的收货<p>1 EA 的质检物料抽样工作</p><p>1 EA 的质检订单 1</p><p>19 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 20 EA、LP3 的收货<p>1 EA 的质检物料抽样工作</p><p>1 EA 的质检订单 1</p><p>19 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 20 EA、LP4 的收货<p>1 EA 的质检物料抽样工作</p><p>1 EA 的质检订单 1</p><p>19 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 20 EA、LP5 的收货<p>1 EA 的质检物料抽样工作</p><p>1 EA 的质检订单 1</p><p>19 EA 的采购订单工作（储存）</p></li></ol> |
| 订单 | 百分比 = 10 | 否 | <p>位置：否</p><p>牌照：否</p> | 否 | 不适用 | <p>**订单行数量：100 EA**</p><ol><li>在仓库管理移动应用中登记 50 EA、LP1 的收货<p>10 EA 的质检物料抽样工作</p><p>10 EA 的质检订单 1</p><p>40 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 50 EA、LP2 的收货<p>50 EA 的采购订单工作（储存）</p></li></ol> |
| 装入 | 百分比 = 5 | 是 _（锁定/不可编辑）_ | <p>位置：否</p><p>牌照：否</p> | 否 | 不适用 | <p>**订单行数量：500 EA**</p><p>**两个负荷：第一个负荷 200 EA，第二个负荷 300 EA**</p><ol><li>在仓库管理移动应用中登记 100 EA 的第一个负荷的收货<p>5 EA 的质检物料抽样工作</p><p>5 EA 的质检订单 1</p><p>95 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 100 EA 的第一个负荷的收货<p>5 EA 的质检物料抽样工作</p><p>5 EA 的质检订单 1</p><p>95 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 300 EA 的第二个负荷的收货<p>15 EA 的质检物料抽样工作</p><p>15 EA 的质检订单 1</p><p>285 EA 的采购订单工作（储存）</p></li></ol> |
| 顺序 | 百分比 = 10 | 是 | <p>位置：是</p><p>牌照：是</p> | 否 | 不适用 | <p>**订单行数量：100**</p><ol><li>在仓库管理移动应用中登记 50 EA、LP1 的收货<p>5 EA 的质检物料抽样工作</p><p>5 EA 的质检订单 1</p><p>45 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 50 EA、LP2 的收货<p>5 EA 的质检物料抽样工作</p><p>5 EA 的质检订单 1</p><p>45 的采购订单工作（储存）</p></li></ol> |
| 装入 | 完全牌照 | 是 _（锁定/不可编辑）_ | <p>位置：是</p><p>牌照：是 _（锁定/不可编辑）_</p> | 否 | 3 | <p>**两种物料：**</p><ul><li>**物料 A 的订单行数量：120 EA（4 个托盘）**</li><li>**物料 B 的订单行数量：90 EA（3 个托盘）**</li></ul><p>**一个负荷，每个订单行两个负荷行**</p><ol><li>在仓库管理移动应用中登记物料 A、30 EA、LP1 的收货<p>30 EA 的质检物料抽样工作</p><p>30 EA 的质检订单 1</p></li><li>在仓库管理移动应用中登记物料 A、30 EA、LP2 的收货<p>30 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记物料 A、30 EA、LP3 的收货<p>30 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记物料 A、30 EA、LP4 的收货<p>30 EA 的质检物料抽样工作</p><p>30 EA 的质检订单 1</p></li><li>在仓库管理移动应用中登记物料 B、30 EA、LP5 的收货<p>30 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记物料 B、30 EA、LP6 的收货<p>30 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记物料 A、30 EA、LP7 的收货<p>30 EA 的质检物料抽样工作</p><p>30 EA 的质检订单 1</p></li></ol> |
| 装入 | 完全牌照 | 是 _（锁定/不可编辑）_ | <p>位置：是</p><p>牌照：是 _（锁定/不可编辑）_</p> | 是 | 3 | <p>**两种物料：**</p><ul><li>**物料 A 的订单行数量：120 EA（4 个托盘）**</li><li>**物料 B 的订单行数量：90 EA（3 个托盘）**</li></ul><p>**一个负荷，每个订单行两个负荷行**</p><ol><li>在仓库管理移动应用中登记物料 A、30 EA、LP1 的收货<p>30 EA 的质检物料抽样工作</p><p>30 EA 的质检订单 1</p></li><li>在仓库管理移动应用中登记物料 A、30 EA、LP2 的收货<p>30 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记物料 A、30 EA、LP3 的收货<p>30 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记物料 A、30 EA、LP4 的收货<p>30 EA 的质检物料抽样工作</p><p>30 EA 的质检订单 1</p></li><li>在仓库管理移动应用中登记物料 B、30 EA、LP5 的收货<p>30 EA 的质检物料抽样工作</p><p>30 EA 的质检订单 1</p></li><li>在仓库管理移动应用中登记物料 B、30 EA、LP6 的收货<p>30 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记物料 A、30 EA、LP7 的收货<p>30 EA 的采购订单工作（储存）</p></li></ol> |
| 装入 | 百分比 = 10 | 是 _（锁定/不可编辑）_ | <p>位置：否</p><p>牌照：否</p> | 否 | 不适用 | <p>**订单行数量：100 EA**</p><p>**未创建负荷。应用订单范围。**</p><ol><li>在仓库管理移动应用中登记 50 EA、LP1 的收货<p>5 EA 的质检物料抽样工作</p><p>5 EA 的质检订单 1</p><p>45 EA 的采购订单工作（储存）</p></li><li>在仓库管理移动应用中登记 50 EA、LP2 的收货<p>5 EA 的质检物料抽样工作</p><p>5 EA 的质检订单 1</p><p>45 EA 的采购订单工作（储存）</p></li></ol> |

当工作人员验证上表中显示的质检订单之一时，系统会自动生成质检订单工作，以将库存从质量控制位置移到 _质检订单_ 工作订单类型的位置指令中定义的位置。 您可以为此目的设置任何位置，如退货或存储位置，具体取决于质检订单的测试结果。 有关此设置的示例，请参阅本文末尾的[示例方案](#example-scenario)。
...
现在为供应商 104 创建了数量为 10 的物料 A0001 的采购订单。 然后，使用仓库管理移动应用将数量为 10 的采购订单行登记为已在一个牌照上接收。 结果如下：
