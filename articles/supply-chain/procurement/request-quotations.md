...
本主题提供询价 (RFQ) 概览，即当组织必须购买物料或服务，且他们要从几个供应商接收竞争性的报价时，他们要发出的询价。 询价时，您要求供应商提供您指定的物料数量的价格和交货时间。 您还可以就有关是否存在任何附加费用，如运费，或者是否对订购数量多的大订单有折扣价或能否提前支付供应商发票等问题询问供应商。
...
您可以覆盖特定询价案例的这些值。 还应配置改正流程。 作为此配置的一部分，您可以打开字段锁定。 在字段锁定打开时，要改正询价的采购专业人员必须在选项卡**报价单**的**改正**部分单击**创建**。使用此改正更新询价后，采购专业人士必须单击**完成**完成该流程。 **完成**操作生成一封电子邮件消息，通知供应商改正的询价。 您为发送到**采购参数**页面的供应商的电子邮件通知选择模板。 在模板创建时，可以包含以下替换标志：
...
**注意：**对于公共部门配置，对已发送的询价进行任何更改将要求使用改正流程。 发送询价后，字段被锁定，因此单击**创建**以使用上述改正流程是对询价进行更改的必需步骤。
它由**采购参数**中的字段锁定参数**发送询价时将其锁定**控制。 此参数设置为**是**，并且对于公共部门配置，这是无法更改的默认值。 这意味着，在非公共部门配置中可以手动处理改正流程，而对于公共部门则要求通过在发送询价后锁定字段的方式处理改正的流程。
...
有时，在您发送询价后，必须更改询价。 例如，交货日期已更改，或者你想要附加产品或不同数量的产品，则可能发生这种情况。 您可以配置更正流程，以便其更严格或更宽松。  
...
您可以接受出价的某些行并拒绝其他行。 您还可以接受来自不同供应商的行。 请注意，如果你接受某些行，系统将提示你拒绝所有其他行。 因此，如果您要接收其他行，在您收到提示时您必须单击**取消**。  
