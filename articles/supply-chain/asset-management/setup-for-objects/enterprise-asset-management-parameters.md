...
可使用**创建数据向导**按钮自动为 Dynamics 365 for Finance and Operations 中的公司内的测试或演示数据用途创建设置数据。 有关如何使用此向导的信息，请参阅白皮书《在资产管理中设置测试数据》。
...
- 在**项目语言来源**字段中，选择要为资产管理报表中的产品名使用的语言。 可选择公司帐户中设置的语言，或为 Dynamics 365 for Finance and Operations 中当前登录的用户设置的语言。  
...
>[!NOTE]
>与**资产管理**模块中是否使用主计划有关的选择和用于获取产能预留或产能负荷概览的相关窗体为标准 Dynamics 365 for Finance and Operations 设置。 您可以在**组织管理**模块的**产能预留**中或**产能负荷**中访问产能信息，具体取决于您在**主计划**中进行的设置。 不能创建两个视图中均显示产能预留的设置。  
