...
在您开始使用仓库管理工作负荷之前，必须按照此节中所述准备好您的系统。

### <a name="deploy-a-scale-unit-with-the-warehouse-management-workload"></a>使用仓库管理工作负荷部署缩放单元
...
### <a name="turn-on-required-features-in-feature-management"></a>在功能管理中打开必需的功能

使用[功能管理](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md)工作区打开以下功能。 （这两个功能都在 *仓库管理* 模块下列出。）
...
在云或边缘缩放单元上部署仓库管理工作负荷之前，请确保您在企业中心启用了 *对于发布到出站订单仓库的缩放单元支持* 功能。 管理员可以使用[功能管理](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md)设置检查功能状态和开启功能（如果需要）。 在 **功能管理** 工作区中，此功能按照下面的方式列出：

- **模块**：*仓库管理*
- **功能名称**：*对于发布到出站订单仓库的缩放单元支持*
...
仓库管理工作负荷支持 Warehouse Management 应用中的以下三个生产流：

- 报告为完工入库并储存
- 开始生产订单
- 登记物料消耗量

### <a name="report-as-finished-and-put-away"></a>报告为完工入库并储存

工作人员可以在 Warehouse Management 应用中使用 **完工入库并储存** 流将生产订单或批次订单报告为完工入库。 他们还可以将批次订单上的联产品和副产品报告为完工入库。 当作业报告为完工入库时，系统通常会在缩放单元上生成储存仓库工作。 如果您不需要储存工作，可以设置工作策略来忽略此工作。

### <a name="start-production-order"></a>开始生产订单

工作人员可以在 Warehouse Management 应用中使用 **开始生产订单** 流登记生产订单或批次订单的开始。

### <a name="register-material-consumption"></a>登记物料消耗量

工作人员可以在 Warehouse Management 应用中使用 **登记物料消耗量** 流报告生产订单或批次订单的物料消耗量。 然后将在缩放单元的生产或批次订单上为报告的物料创建领料单日记帐。 日记帐行将对已消耗的库存进行实际预留。 当在缩放单元和中心之间同步数据时，将在中心实例上生成并过帐领料单日记帐。
...
缩放单元当前不支持其他类型的原始凭证处理或仓库工作。 例如，当您对缩放单元运行仓库执行工作负荷时，不能使用销售退货单接收流程来处理退货单。 此处理必须由中心实例完成。
...
|--------------------------------------------------------------|-----|------------------------------|
| 原始凭证处理                                   | 是 | 否 |
| 负荷和运输管理处理                | 是，但只有装载计划流程。 不支持运输管理处理  | 否 |
| 发放到仓库                                         | 是 | 否 |
| 计划越库配送                                        | 否  | 否 |
| 装运合并                                       | 是，在使用装载计划时 | 是 |
| 装运波次处理                                     | 否  |是，除 **装载计划和排序** 外 |
| 维护波次装运                                  | 否  | 是|
| 仓库工作处理（包括牌照打印）        | 否  | 是，但仅针对上述支持的功能 |
| 群集领料                                              | 否  | 是|
| 手动装箱处理，包括“已装箱的集装箱领料”工作处理 | 否 <P>某些处理可以在由缩放单元处理的初始领料流程之后进行，但是由于后续的锁定操作不建议使用。</p>  | 否 |
| 从组中删除集装箱                                  | 否  | 否 |
| 出库排序处理                                  | 否  | 否 |
| 打印负荷相关的凭证                           | 是 | 是|
| 提单和 ASN 生成                            | 否  | 是|
...
| 冲销工作                                                 | 否  | 是|
| 冲销装运确认                                | 否  | 是|
| 请求取消仓库订单行                      | 是 | 否，但请求将被批准或拒绝 |
| <p>发放转移单以便接收</p><p>此流程将作为转移单装运流程的一部分自动执行。 但是，如果入站仓库订单行已被取消或作为新工作负荷部署流程的一部分，则可以手动使用它在缩放单元启用牌照接收。</p> | 是 | 否|
...
| 加载物料接收                                              | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 否 |
| 采购订单牌照接收和储存              | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 否 |
| 转移单牌照接收和储存             | 否 | 是 |
| 转移单物料接收和储存                       | 是 | 否 |
| 转移订单行接收和储存                       | 是 | 否 |
| 包含欠交的采购订单接收                      | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 是，但只能通过从中心发起取消请求来进行 |
| 包含超交的采购订单接收                       | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 是  |
| 接收并创建 *越库配送* 工作                 | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 否 |
| 接收并创建 *质检订单* 工作                  | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 否 |
| 接收并创建 *质检物料抽样* 工作          | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 否 |
| 接收并创建 *质量检查质量* 工作       | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 否 |
| 接收并创建质检订单                            | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 否 |
| 工作处理 - 由 *群集储存* 导向                 | 是 | 否 |
| 包含 *领料短缺* 的工作处理                               | 是 | 否 |
| 取消工作（入站）                                            | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | <p>是，但仅在<b>仓库管理参数</b>页上的<b>取消工作时取消收货登记</b>选项清除时</p> |
| 牌照加载                                           | 是 | 是 |
...
| 创建仓库订单                        | 是 | 否 |
| 请求取消仓库订单行        | 是 | 否，但请求将被批准或拒绝 |
| 报告为完工入库和成品储存 | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 是|
| 联产品和副产品储存             | <p>是，当没有仓库订单时</p><p>否，当有仓库订单时</p> | 是|
| 登记物料消耗量                  | 是 | 是|
| 生产波次处理                     | 是 | 否 |
| 原材料领取                           | 是 | 否 |
| 看板储存                                | 是 | 否 |
...
在中心部署上，您可以手动维护以下批处理作业：

- 在 **仓库管理 \> 定期任务 \> 后台工作负荷管理** 管理以下批处理作业：

    - 缩放单元到中心消息处理程序
    - 登记源订单收货
    - 完成仓库订单

- 在 **仓库管理 \> 定期任务 \> 工作负荷管理** 管理以下批处理作业：

    - 仓库中心到缩放单元消息处理器
    - 处理仓库订单行收货以进行仓库收货过帐

在缩放单元部署上，您可以在 **仓库管理 \> 定期任务 \> 工作负荷管理** 管理以下批处理作业：
