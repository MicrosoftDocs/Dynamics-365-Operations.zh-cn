...
- **装运波次处理数据** – 选定的波次处理方法将作为缩放单元波次处理的一部分进行处理。
- **工作处理数据** – 在缩放单元上创建的仓库工作将归此特定缩放单元所有。 支持以下类型的工作订单处理：

  - **库存变动**（手动变动和通过模板工作变动）
  - 作为盘点操作一部分的 **周期盘点** 和批准/拒绝流程
...
一旦最终工作流程将库存放到最终装运位置（货架门），缩放单元就会向中心发出信号，指示将源单据库存交易更新为 *已领料*。 在此流程运行并重新同步之前，缩放单元工作负荷上的现有库存将在仓库级别实际预留。
...
缩放单元当前不支持其他类型的原始凭证处理或仓库工作。 例如，对于缩放单元上的 WES 工作负荷，您无法执行转移单接收流程（转移收货）；这必须由中心实例处理。
