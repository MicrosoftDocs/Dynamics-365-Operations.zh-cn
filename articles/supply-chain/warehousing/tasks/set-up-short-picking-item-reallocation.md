...
2. 单击**新建**。 可以使用不同物料重新分配策略定义多个工作异常，以便让仓库工作人员可以根据处理的装运需求选择一个。  
3. 在**工作异常代码**字段中，键入一个值。 为工作异常分配标题以指示其用途。 例如，“手动领料短缺”。  
4. 在**描述**字段中，键入一个值。
5. 在**异常类型**字段中，选择“领料短缺”。
6. 选中**调整库存**复选框。 此选项意味着领料短缺库位自动将库存调整为 0。  
7. 在**默认调整类型代码**字段中，输入或选择一个值。 例如，在 USMF 中，可以选择“删除 Res Adj Out”。  
8. 在**物料重新分配**字段中，选择“手动”。 如果选择“手动”或“自动和手动”，需要启用仓库工作人员才能使用手动重新分配。  
