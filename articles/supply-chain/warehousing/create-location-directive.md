...
## <a name="turn-the-location-directive-scopes-feature-on-or-off"></a><a name="scopes-feature"></a>打开或关闭“位置指令范围”功能

[!INCLUDE [preview-banner-section](../../includes/preview-banner-section.md)]
<!-- KFM: Preview until 10.0.31 GA -->

*位置指令范围* 功能可以在您设计位置指令时为您提供更多自由，并有助于减少冗余配置。 它增加了一个 **范围** 选项，取代了以前的 **多 SKU** 选项。 **多 SKU** 选项只能设置为 *是* 或 *否*，**范围** 选项则不仅提供这两个设置（通过 *单个物料* 和 *多个物料* 值），还提供另外两个设置（通过 *单个物料或订单* 和 *所有* 值）。 有关这些设置的更多信息，请参阅[位置指令快速选项卡](#location-directives-tab)。

启用后，**范围** 选项将取代 **多 SKU** 选项，与现有配置 100% 兼容。

要使用此功能，您必须在系统中打开它。 管理员可以使用[功能管理](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md)设置检查功能状态，以及打开或关闭功能。 在 **功能管理** 工作区中，此功能按照下面的方式列出：
...
- **复制** – 打开一个对话框，您可以在其中创建当前位置指令的精确副本。
- **编辑查询** – 打开一个对话框，您可以在其中定义处理选定库位指令所遵循的条件。 例如，您可能希望它仅应用于特定的仓库。
- **接受度测试** – 打开一个页面，您可以在其中设置自动化测试，以确定您的位置指令在不同起始条件下的行为方式。 通过这种方式，您可以在创建和维护指令时快速验证指令。 有关详细信息，请参阅[使用接受度测试测试位置指令](location-directive-acceptance-tests.md)。
...
## <a name="location-directives-fasttab"></a><a name="location-directives-tab"></a>库位指令快速选项卡
...
- **范围** – 使用此选项指定将应用位置指令的场景。 此选项将取代 **多 SKU** 选项，仅在系统中已打开 *位置指令范围* 功能时可用。 （有关详细信息，请参阅[打开或关闭“位置指令范围”功能](#scopes-feature)。）

    | 范围设置 | 具有一个物料的单个订单 | 具有同一个物料的多个订单 | 具有多个物料的单个订单 | 具有多个物料的多个订单 |
    |---|---|---|---|---|
    | 单个物料 | 是 | 是 | 否 | 否 |
    | 多个物料 | 否 | 否 | 是 | 是 |
    | 单个物料或订单 | 是 | 是 | 是 | 否 |
    ...
    下表描述了范围何时可用，以及它们是否允许 **编辑查询** 功能。

    | 范围 | 支持的工作类型 | 支持的工作订单类型 | 允许编辑查询 |
    |---|---|---|---|
    | 单个物料 | 所有 | 所有 | 是 |
    | 多个物料 | 所有 | 所有 | 否 |
    | 单个物料或订单 | 放置 | 联产品和副产品储存、成品储存、看板储存、采购订单、质检订单、补货、退货单、销售订单、转移发货和转移收货 | 是 |
    | 所有 | 放置 | 所有 | 否 |

    > [!NOTE]
    > - 要同时为多个物料和单个物料执行放置，您必须确保存在同时覆盖这两个场景的位置指令。 例如，您可以设置一个或多个 *单个物料或订单* 位置指令来覆盖需要优化的场景（如通过编辑查询），然后设置一个或者多个 *所有* 位置指令来覆盖其余场景。
    > - 虽然 *单个物料* 和 *多个物料* 范围可以用于放置，但这种方法通常会导致冗余配置。 应考虑改用 *单个物料或订单* 和 *所有* 范围，因为此方法会产生更干净的设置。

- **多 SKU** – 使用此选项指定将应用位置指令的场景。 如果系统中已打开 *位置指令范围* 功能，此设置将被 **范围** 设置替换。 （有关详细信息，请参阅 [打开或关闭位置指令范围功能](#scopes-feature)。）将此选项设置为 *是* 可以在一个位置使用多个库存单位 (SKU)。 例如，必须为货架门库位启用多个 SKU。 如果启用多个 SKU，将按预期在工作中指定放置库位。 但是，放置库位将只能处理多物料放置（如果工作包括必须领料和放置的不同 SKU）。 它将无法处理单个 SKU 放置。 如果将此选项设置为 *否*，则仅在您的放置只有一种 SKU 时才指定放置库位。
