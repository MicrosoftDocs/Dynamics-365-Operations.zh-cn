...
ms.locfileid: "6751689"
---
# <a name="import-inbound-asns-through-the-v2-data-entity"></a>通过 V2 数据实体导入入站 ASN
...
本主题显示了一系列场景，通过示例演示如何使用 ASN 文件。

> [!IMPORTANT]
> *入站 ASN* 导入仅适用于为高级仓库管理 (WMS) 启用的物料。 在您收到 ASN 之前，必须在系统中针对发送该 ASN 的供应商注册采购订单。

## <a name="inbound-asn-v2-entity"></a>入站 ASN V2 实体

您可以使用 *入站 ASN V2* 复合数据实体导入入站 ASN。 *入站 ASN V2* 利用以下实体：
...
*入站 ASN V2* 复合数据实体专门面向使用基于 XML 文件的导入的异步集成场景。
