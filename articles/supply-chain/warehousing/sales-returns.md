...
>[!NOTE] 
>从退货单的到达必须与其他类型的到达事物分开处理。 在 **到达概览** 页中标识传入的包（如通过使用随附的 RMA 文档）之后，请在操作窗格中，单击 **开始到达** 创建并启动与到达匹配的到达日记帐。
...
> [!NOTE]
> 默认情况下，如果已启用了 **应收帐款参数** 页中的 **用于更正的贷方通知单** 选项，则已激活 **信用更正** 选项。 但是，建议您不使用 Storno 过帐退货。
...
> [!NOTE]
> 物料主价格用作 **退货成本价** 参数的默认值。 默认价格与出库时的成本价不同。 因此，影响是相差 3。 此外，退货单中不包含在销售订单中向客户提供的折扣。 因此，将出现透支。
