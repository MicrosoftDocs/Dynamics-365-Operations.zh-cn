---
title: 适用于 Dynamics 365 Supply Chain Management 的学习目录
description: 查找适用于 Dynamics 365 Supply Chain Management 的所有培训选项。
author: margoc
ms.topic: article
ms.date: 07/23/2020
ms.author: margoc
ms.prod: ''
ms.technology: ''
audience: Application user
ms.reviewer: margoc
ms.search.region: Global
ms.search.validFrom: 2020-07-23
ms.dyn365.ops.version: 8.0999999999999996
ms.openlocfilehash: 46302589ac5139ee6297aba26572225c0e392287
ms.sourcegitcommit: dca3279a8b7cd5d0bcd4e4a3aa9938b337aa8849
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 08/19/2021
ms.locfileid: "7402512"
---
# <a name="learning-catalog-for-dynamics-365-supply-chain-management"></a>适用于 Dynamics 365 Supply Chain Management 的学习目录

查找面向您作为 Dynamics 365 Supply Chain 应用程序用户角色的合适的在线培训、现场研讨会和活动。

<!-- ![Universal Windows Platform (UWP).](images/platform-uwp.png)  -->  

:::row:::
    :::column:::
<!-- ![Universal Windows Platform (UWP).](images/platform-uwp.png)  -->  

    ### <a name="business-and-technical-decision-makers"></a>业务和技术决策者

    您在决定是否要投资新技术吗？ 

    [业务和技术决策者学习目录](learning-catalog-bdm.md#)

    [开始](learning-catalog-bdm.md#get-started)

    [考试](learning-catalog-bdm.md#exam)

    :::column-end:::
    :::column:::

    ### <a name="business-users"></a>业务用户

    您刚刚有一个新应用程序要使用吗？ 

    [业务用户学习目录](learning-catalog-business-user.md)

    [开始](learning-catalog-business-user.md#get-started)

    [核心平台知识](learning-catalog-business-user.md#core-platform-knowledge)

    [供应链管理](learning-catalog-business-user.md#supply-chain-management)

    [制造](learning-catalog-business-user.md#manufacturing)

    [考试](learning-catalog-business-user.md#exam)

    :::column-end:::
:::row-end:::
:::row:::
    :::column:::

    ### <a name="implementation-project-managers"></a>实施项目经理

    您负责确保公司的实施顺利进行吗？

    [实施项目经理学习目录](learning-catalog-project-manager.md)

    [开始](learning-catalog-project-manager.md#get-started)

    [核心平台知识](learning-catalog-project-manager.md#core-platform-knowledge)

    [供应链管理](learning-catalog-project-manager.md#supply-chain-management)

    [制造](learning-catalog-project-manager.md#manufacturing)

    [考试](learning-catalog-project-manager.md#exams)

    :::column-end:::
    :::column:::

    ### <a name="administrators"></a>管理员

    您是否需要全天候保持系统和数据的流动、调配和安全？

    [管理员学习目录](learning-catalog-administrator.md)

    [开始使用](learning-catalog-administrator.md#get-started)

    [核心平台知识](learning-catalog-administrator.md#core-platform-knowledge)

    [Supply Chain Management](learning-catalog-administrator.md#scm)

    [核心开发](learning-catalog-administrator.md#core-development)

    [迁移和升级](learning-catalog-administrator.md#migration-and-upgrade)

    [服务](learning-catalog-administrator.md#servicing)

    [正在测试](learning-catalog-administrator.md#testing)

    [考试](learning-catalog-administrator.md#exam)

  :::column-end:::
:::row-end:::
:::row:::
    :::column:::

    ### <a name="developers"></a>开发人员

    您是否需要编写代码来与其他数据源集成、扩展核心系统功能或构建复杂的应用程序？

    [开发人员学习目录](learning-catalog-developer.md)

    [入门](learning-catalog-developer.md#get-started)

    [核心开发概念](learning-catalog-developer.md#core-development-concepts)

    [服务](learning-catalog-developer.md#servicing)

    [迁移和升级](learning-catalog-developer.md#migration-and-upgrade)

    [正在测试](learning-catalog-developer.md#testing)

    [考试](learning-catalog-developer.md#exam)

    :::column-end:::
    :::column:::

    ### <a name="functional-consultants"></a>功能顾问

    您是业务领域的实施专家吗？ 

    [功能应用程序顾问学习目录](learning-catalog-functional-consultant.md)

    [开始](learning-catalog-functional-consultant.md#get-started)

    [核心平台知识](learning-catalog-functional-consultant.md#core-platform-knowledge)

    [制造](learning-catalog-functional-consultant.md#manufacturing)

    [供应链管理](learning-catalog-functional-consultant.md#supply-chain-management)

    [考试](learning-catalog-functional-consultant.md#exams)

    :::column-end:::
:::row-end:::
:::row:::
    :::column:::

    ### <a name="partner-sales-and-marketing"></a>合作伙伴销售和市场营销

    您负责帮助您的客户购买正确的解决方案吗？ 

    [Microsoft 合作伙伴销售和市场营销学习目录](learning-catalog-partner-sales.md)

    [开始](learning-catalog-partner-sales.md#get-started)

    [考试](learning-catalog-partner-sales.md#exam)

    :::column-end:::

    :::column:::

    ### <a name="solution-architects"></a>解决方案架构师

    您在设计满足客户需求和预算的解决方案吗？

    [解决方案架构师学习目录](learning-catalog-solution-architect.md)

    [入门](learning-catalog-solution-architect.md#get-started)

    [核心平台知识](learning-catalog-solution-architect.md#core-platform-knowledge)

    [核心开发概念](learning-catalog-solution-architect.md#core-development-concepts)

    [迁移和升级](learning-catalog-solution-architect.md#migration-and-upgrade)

    [服务](learning-catalog-solution-architect.md#servicing)

    [正在测试](learning-catalog-solution-architect.md#testing)

    [考试](learning-catalog-solution-architect.md#exam)

    :::column-end:::
:::row-end:::


[!INCLUDE[footer-include](../../includes/footer-banner.md)]