...
| 采购 | 禁用采购申请分配重置按钮 | 此功能将禁用正在审核的采购申请的 **会计分配** 页面上的 **重置** 按钮。 |
...
| 仓库管理 | 禁用从锁定库存取样的质检订单的预期收货 | 此功能阻止系统为具有锁定状态的示例库存的质检订单创建预期收货交易记录。 由于锁定的库存不可用，此功能将删除预期收货。 这有助于确保库存不会以多个锁定状态结束，这可能导致数据完整性问题。 |
...
| 模块 | 功能名称 | 功能状态 |
| --- | --- | --- |
| 资产管理 | 在运行维护计划时应用对工作订单进行分组的规则 | 正式发布 |
| 资产管理 | 基于计数器的维护增强 | 正式发布 |
| 成本管理 | 成本计算级别 | 正式发布 |
| 成本管理 | 为库存结转冲销启用用户定义的批处理号设置 | 正式发布 |
| 成本管理 | 库存结算进度详细信息 | 正式发布 |
| 成本管理 | 用于库存标准成本重估的默认财务维度的选项 | 正式发布 |
| 成本管理 | 库存值报表数据清除 | 默认打开 |
| 成本管理 | 库存值报表存储 | 默认打开 |
| 成本管理 | 在网格中显示库存结算日志 | 默认打开 |
| 工程更改管理 | 对现有产品启用更改管理 | 默认打开 |
| 工程更改管理 | 工程更改管理 | 默认打开 |
| 工程更改管理 | 面向生产部门的工程通知 | 默认打开 |
| 工程更改管理 | 工程更改管理的已改进属性继承 | 默认打开 |
| 工程更改管理 | 管理对配方及其成分的更改 | 默认打开 |
| 工程更改管理 | 产品准备情况检查 | 默认打开 |
| 工程更改管理 | 生成工程产品的变型 | 默认打开 |
| 库存和仓库管理 | 从物料清单行导航到物料清单版本 | 强制 |
| 主计划 | 计划散装批次订单和包装批次订单的可批处理确认和合并 | 正式发布 |
| 主计划 | 资源计划与维护 | 正式发布 |
| 主计划 | 启用主计划设置向导功能 | 强制 |
| 主计划 | 需求预测详细信息中的预测模型选择 | 强制 |
| 主计划 | 主计划进度可视化 | 强制 |
| 主计划 | 并行确认计划订单 | 强制 |
| 主计划 | 通过筛选确认计划订单 | 默认打开 |
| 主计划 | 计划订单的已保存视图 | 默认打开 |
| 采购 | 禁用采购申请分配重置按钮 | 正式发布 |
| 采购 | 启用重置与采购相关的工作流 | 正式发布 |
| 采购 | 批量确认通过供应商协作接受的采购订单的能力 | 强制 |
| 采购 | 采购协议“已关闭”状态 | 强制 |
| 采购 | 向与采购协议关联的采购订单发票中添加行 | 默认打开 |
| 采购 | 将“订购数量”字段添加到“过账物料收货”页 | 默认打开 |
| 采购 | 允许供应商通过供应商协作申请采购类别 | 默认打开 |
| 采购 | 采购订单上起始金额和终止金额 | 默认打开 |
| 采购 | 站点和仓库的费用设置 | 默认打开 |
| 采购 | 启用基于年度关税的进项税计算 | 默认打开 |
| 采购 | 采购协议负责方 | 默认打开 |
| 采购 | 采购订单的已保存视图 | 默认打开 |
| 产品信息管理 | 对默认订单数量的严格验证 | 强制 |
...
| 生产控制 | 在作业卡设备中报告为完工入库时，启用牌照编号的自动生成 | 强制 |
| 生产控制 | 启用分包物料的部分收货，并解决有关“供应商”类型的物料清单行的废料计算问题 | 强制 |
| 生产控制 | 用于锁定作业卡设备和作业卡终端以便对其进行净化的功能 | 强制 |
| 生产控制 | 对“审核”和“转移作业”对话框进行的改进 | 强制 |
| 生产控制 | 用于报告为完工入库的牌照已添加作业卡设备 | 强制 |
| 生产控制 | 通过作业卡设备打印标签 | 强制 |
| 生产控制 | 生产车间执行 | 强制 |
| 生产控制 | 生产车间执行界面的资产管理功能 | 默认打开 |
| 生产控制 | 生产车间执行界面的作业搜索 | 默认打开 |
| 生产控制 | 覆盖默认生产预留 | 默认打开 |
| 生产控制 | 在生产车间执行界面中显示完整序列、批处理和牌照编号 | 默认打开 |
| 销售和市场营销 | 销售订单详细信息性能增强 | 正式发布 |
| 销售和市场营销 | 销售报价详细信息性能增强 | 正式发布 |
| 销售和市场营销 | 销售订单引用数据导出策略 | 强制 |
| 销售和市场营销 | 销售订单到采购订单行删除策略 | 强制 |
| 销售和市场营销 | 销售报价引用数据导出策略 | 强制 |
| 销售和市场营销 | 联系人数据实体导出优化 | 默认打开 |
| 销售和市场营销 | 启用对销售报价单文档介绍和文档结论字段的查找 | 默认打开 |
| 销售和市场营销 | 改善“前 100 位”客户的报表性能 | 默认打开 |
| 销售和市场营销 | 重新计算估计的客户余额 | 默认打开 |
| 销售和市场营销 | 包含和不包含实际称重的销售退货单行登记(具有小数精度) | 默认打开 |
| 销售和市场营销 | 销售和市场营销的保存视图 | 默认打开 |
| 销售和市场营销 | 一键销售订单确认 | 默认打开 |
| 仓库管理 | 带有库位指令的越库配送模板 | 正式发布 |
| 仓库管理 | 禁用从锁定库存取样的质检订单的预期收货 | 正式发布 |
| 仓库管理 | 牌照接收历史记录 | 正式发布 |
| 仓库管理 | 在发放到仓库时将数量向下舍入到最近的销售单位 | 正式发布 |
| 仓库管理 | 对仓库应用工作列表的缩放单元支持 | 正式发布 |
| 仓库管理 | 装运波次标签详细信息 | 正式发布 |
| 仓库管理 | 对装箱工作站上关闭/重新打开的集装箱使用更快的 API | 正式发布 |
| 仓库管理 | 验证为补货作业选择的模板 | 正式发布 |
| 仓库管理 | 允许补货模板使用现有的即时补货工作(跨单位) | 强制 |
| 仓库管理 | 创建 WHS 用户时自动分配 GUID | 强制 |
| 仓库管理 | 在接收负荷物料期间捕获仓库应用中的产品变型和跟踪维度 | 强制 |
| 仓库管理 | 更改由跟踪维度控制的物料的库存状态 | 强制 |
| 仓库管理 | 更改工作的工作池 | 强制 |
| 仓库管理 | 群集位置已满 | 强制 |
| 仓库管理 | 群集储存功能 | 强制 |
| 仓库管理 | 确认并转移 | 强制 |
| 仓库管理 | 确认来自批处理作业的出站装运 | 强制 |
| 仓库管理 | 控制是否在移动设备上显示接收汇总页 | 强制 |
| 仓库管理 | 延迟处理手动库存变动操作 | 强制 |
| 仓库管理 | 不允许创建不符合波次负荷构建模板要求的负荷 | 强制 |
| 仓库管理 | 增强的牌照标签布局 | 强制 |
| 仓库管理 | 评估针对多 SKU 库位指令的所有操作 | 强制 |
...
| 仓库管理 | 阻止转移单已装运牌照用于目标仓库以外的其他仓库 | 强制 |
| 仓库管理 | 提示解析不明确的“位置/牌照”名称 | 强制 |
| 仓库管理 | 质量检查 | 强制 |
| 仓库管理 | 新仓库应用的用户设置、图标和步骤标题 | 强制 |
| 仓库管理 | 其他位置区域 | 默认打开 |
| 仓库管理 | 通过仓库应用创建和处理转移单 | 默认打开 |
| 仓库管理 | 启用仓库移动设备的快速验证 | 默认打开 |
| 仓库管理 | 物料合并库位利用率 | 默认打开 |
| 仓库管理 | 仓库管理现有条目清理作业的最长执行时间 | 默认打开 |
| 仓库管理 | 出站工作负荷可视化 | 默认打开 |
| 仓库管理 | 处理仓库应用事件 | 默认打开 |
| 仓库管理 | 用于装载计划工作台的已保存视图 | 默认打开 |
| 仓库管理 | 工作详细信息页的已保存视图 | 默认打开 |
| 仓库管理 | 用于波次处理的已保存视图 | 默认打开 |
| 仓库管理 | 用于负荷处理的已保存视图 | 默认打开 |
| 仓库管理 | 用于装运处理的已保存视图 | 默认打开 |
| 仓库管理 | 波次批处理作业详细信息 | 默认打开 |
| 仓库管理 | 波次执行通知 | 默认打开 |
