...
| 资源分配 | 当使用无限容量时，内置的主计划引擎会将所有计划订单分配给给定资源组上的同一资源。 计划优化通过随机选择资源来改进这一点，让不同的生产订单可以使用不同的资源。 如果您想要对所有计划订单使用相同资源，您必须在路线中指定该资源。 |
| 扩展数据类型 (EDT) | 计划优化不支持更改 EDT 的精度。 例如，如果您将产品数量精度从两位小数（默认）扩大到四位，计划优化仍会使用两位小数。 |
