...
- **已审核** – 此状态指示计划订单已审核，可以确认。 如果要确认计划订单，可将其状态更改为 *已审核*。 如果要保留对计划订单所做的编辑，或者打算确认计划订单，请将其状态更改为 *已审核*。 状态为 *已审核* 的计划订单将被主计划视为固定供应和预期供应。 因此，在之后的主计划运行中不会对其进行修改或删除。 为实现此行为，计划逻辑会在主计划期间将状态为 *已审核* 的计划订单从旧计划版本复制到新计划版本。 请注意，状态为 *已审核* 的计划订单仅在特定主计划内被视为供应。
