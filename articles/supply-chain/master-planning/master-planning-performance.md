...
---
# <a name="improve-master-planning-performance"></a>改进主计划绩效

[!include [banner](../includes/preview-banner.md)]
[!include [banner](../includes/banner.md)]

本主题介绍可帮助您提高主计划绩效和诊断问题的各种选项。 包括有关参数、设置，以及建议的配置和操作的信息。 还包括需要执行长时间运行的主计划作业时应考虑的所有重要参数的摘要。

本主题面向有能力进行故障诊断的系统管理员或 IT 用户。 也面向生产或供应规划员，因为其中包含有关与业务计划需求有关的参数的信息。 

## <a name="parameters-that-are-related-to-master-planning-performance"></a>与主计划绩效有关的参数

许多参数会影响主计划的运行时间，因此应加以注意。

### <a name="number-of-threads"></a>线程数

可通过**线程数**参数调整主计划编制流程，以便在使用特定数据集时效果更出色。 该参数指定将用于运行主计划的线程总数。 它会导致主计划运行并行化，而这样的并行化有助于缩短运行时间。 
...
可设置**主计划参数**页（**主计划 \> 设置 \> 主计划参数**）的**常规**选项卡上**绩效**部分中的**固定捆绑销售中的订单数**参数。 自动确认流程的并行化基于必须一起处理的订单。 因此，例如如果此参数设置为 **50**，则每个线程或批处理一次性选取 50 个订单并一起处理。 建议通过试错流程找到最佳值。 但是，可使用以下公式计算初始值：
...
还可以在时限中找到**行动消息**参数。 计算行动消息会导致主计划的运行时间更长。 如果不定期（每天、每周等）分析和采用行动消息，请考虑在主计划运行期间关闭计算。 若要关闭计算，请在**主计划**页（**主计划 \> 设置 \> 计划 \> 主计划**）上，将正在运行的主计划的**行动消息**时限设置为 **0**（零）。 并确保为所有覆盖范围组关闭**行动消息**设置。
...
系统可收集有关主计划期间运行的任务的更多信息。 若要让系统收集此信息，请开启**主计划运行**对话框中的**跟踪处理时间**设置。 收集的信息可帮助您找到运行中的瓶颈。 例如，如果**帮助程序任务捆绑中的任务数**参数设置为 **1**，则可识别运行时间最长的物料。 也可以比较状态为**覆盖**的大量线程的运行时间，以及比较每个任务的持续时间。
...
- **更新**应该不用太长时间（通常应该最多 30 分钟）。 但是这是单线程。
