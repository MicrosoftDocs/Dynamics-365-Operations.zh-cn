...
## <a name="run-forecast-planning"></a>运行预测计划

在输入需求预测和/或供应预测后，您可以运行预测计划以计算对材料的总需求和产能，并生成计划订单。

1. 转到 **主计划 \> 预测 \> 预测计划**。
1. 在 **预测计划** 字段中，选择一个预测计划。
1. 启用 **跟踪处理时间** 以记录每个计划任务的处理时间。
1. 在 **线程数** 字段中，输入一个值。 （有关详细信息，请参阅[改善主计划性能](master-planning-performance.md)。）
1. 在 **注释** 字段中，输入文本以捕获所需的任何其他信息。
1. 在 **要包括的记录** 快速选项卡上，选择 **筛选器** 以限制物料的选择。
1. 在 **在后台运行** 快速选项卡上，指定批处理的参数。
1. 选择 **确定**。

若要查看计算的需求，请打开 **总需求** 页面。 例如，在 **发布产品** 页面的 **计划** 选项卡中，在 **需求** 部分中，选择 **总需求**。
