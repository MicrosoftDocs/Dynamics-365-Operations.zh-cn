...
2. 在 **设置** 下，选择 **休假和缺勤类型**。
3. 选择 **新建**。
4. 在 **类型** 下输入休假类型的名称，在 **说明** 下输入说明，然后在 **工作流 ID** 字段中选择工作流。 根据休假类型，在 **申请类型** 字段中选择申请类型。 例如，选择 **休息时间** 或 **休假**。
5. 在 **一般** 中，从 **类别** 下拉列表中选择 **无**、**计划** 或 **计划外**。
6. 从 **收入代码** 下拉列表中选择一个收入代码。
7. 在 **需要原因代码** 下，选择您是否希望提供原因代码。 如果您希望提供原因代码，可能必须添加原因代码。 在 **原因代码** 下，选择 **添加**，选择一个原因代码，然后选择它旁边的 **已启用** 复选框。
8. 如果申请类型为 **休假**，请执行以下步骤：

      1. 在 **开放式** 下，选择用户是否应该能够创建开放式休假。
      2. 如果启用了 **开放式**，您可以选择工作人员休假后返回时是否必须提交重返工作岗位通知。
      3. 如果工作人员必须提交重返工作岗位通知，您可以启用 **启用重返工作岗位通知**。 如果启用了 **启用重返工作岗位通知**，**需要附件** 将自动启用且无法禁用。

9. 如果用户应该在创建或更新休假申请时上传文档，您可以启用 **需要附件**。
10. 在 **限制对选定角色的访问** 下，选择是否要限制访问。 然后在 **此休假类型的安全角色** 下选择安全角色。 安全角色在您在此过程前面在 **工作流 ID** 下选择的工作流中定义。
11. 在 **日历颜色** 下，选择该请假类型在休假和缺勤日历上显示的颜色。
11. 在 **暂停关系** 下，选择此休假类型是否应暂停还是由其他休假类型暂停。 当提交休假请求以暂停休假类型时，将自动为暂停的休假类型创建休假暂停。
12. 选择 **保存**。
...
2. 在列表中选择 **休假和缺勤类型**。 在 **常规** 部分，使用 **需要附件** 字段指定当员工为所选休假类型提交新休假请求时是否必须上载附件。 
