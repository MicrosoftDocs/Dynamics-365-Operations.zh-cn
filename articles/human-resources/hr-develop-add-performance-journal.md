---
title: 添加到绩效日记账并向某人发送表扬
description: 绩效日记帐中包含有关员工如何在某个期间内实现目标或其表现的信息。
author: twheeloc
ms.date: 08/25/2021
...
5. 在 **描述** 字段中，键入一个值。
    * 绩效日记帐日期是日记帐的创建日期。  
    * 源表示绩效日记帐的来源。 如果您创建一个，则创建的这个来自“我的日记帐”。 如果您的经理创建一个，则创建的这个来自“经理日记帐”。      * 您可以与您的经理共享此日记帐，或将其设置为只有您才可以看到。  
6. 在 **开始日期** 字段中，输入一个日期。
7. 在 **完成日期** 字段中，输入一个日期。
8. 在 **开发计划** 字段中，选择 **是**。
9. 在 **关键字** 字段中，键入一个值。
