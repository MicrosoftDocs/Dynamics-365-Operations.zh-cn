...
通过 Human Resources 与 Power Apps 环境之间的集成，可使用 Power Apps 工具集成和扩展 Human Resources 数据的使用。 了解 Power Apps 环境的用途不仅有助于您构建扩展 Human Resources 的应用，还将帮助您在预配 Human Resources 时选择正确的环境。 有关 Power Apps 环境的信息，包括环境范围、环境访问和创建与选择环境，请参阅[介绍 Power Apps 环境](https://powerapps.microsoft.com/blog/powerapps-environments/)。 
...
4. 应该考虑数据集成和测试策略，如沙盒、UAT 或生产。 建议您考虑对您的部署的各种影响，因为以后不容易更改将哪个 Human Resources 环境映射到 Power Apps 环境。

5. 以下 Power Apps 环境不能用于 Human Resources，将从 LCS 内的选择列表中筛除：
 
    - **默认 Power Apps 环境** - 虽然每个租户均自动预配为默认的 Power Apps 环境，但我们不建议在 Human Resources 中使用它们，因为所有租户用户均有权访问 Power Apps 环境，有可能会在使用 Power Apps 或 Power Automate 集成进行测试和探索时意外损坏生产数据。
