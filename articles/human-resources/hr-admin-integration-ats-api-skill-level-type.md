---
title: 技能级别类型
description: 本主题介绍 Dynamics 365 Human Resources 的技能级别类型选项集。
author: jaredha
manager: tfehr
ms.date: 02/05/2021
ms.topic: article
ms.prod: ''
ms.service: dynamics-365-human-resources
ms.technology: ''
audience: Application User
ms.reviewer: anbichse
ms.custom: ''
ms.assetid: ''
ms.search.region: Global
ms.author: jaredha
ms.search.validFrom: 2021-02-05
ms.dyn365.ops.version: Human Resources
ms.openlocfilehash: 2d87b1a62d592b7ea9be35ba3852e9ea0cb88eae
ms.sourcegitcommit: 33b5c8bc4f9461e290513aa22de1ec1fba3b0742
ms.translationtype: HT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/05/2021
ms.locfileid: "5125898"
---
# <a name="skill-level-type"></a>技能级别类型

本主题介绍 Dynamics 365 Human Resources 的技能级别类型选项集。

物理名称：mshr_hrmskillleveltype

此枚举提供对分配给应聘者技能的级别进行分类的值选项集。

| 值 | 标签 | 说明 |
| --- | --- | --- |
| 200000000 | 实际值 | 实际技能级别。 |
| 200000001 | 目标 | 技能级别的目标值。 |

## <a name="see-also"></a>请参阅

[申请人跟踪系统集成 API 简介](hr-admin-integration-ats-api-introduction.md)<br>
[可雇用的应聘者的查询示例](hr-admin-integration-ats-api-candidate-to-hire-example-query.md)



[!INCLUDE[footer-include](../includes/footer-banner.md)]