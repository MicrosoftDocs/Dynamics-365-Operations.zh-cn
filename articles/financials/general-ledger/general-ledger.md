...
---
# <a name="general-ledger-overview"></a>总帐概览
...
分配是基于分配规则将金额分配到一个或多个帐户或帐户/维度组合的过程。 存在两种类型的分配：固定和可变。 您还可以在结算会计科目之间的交易记录和重估原币金额。 在某一会计年度结束时，您必须为下一会计年度准备您的科目，并且关闭当前会计年度。 您可以使用合并功能以合并若干子公司的财务结果为唯一的合并公司的财务结果。 这些字公司可以存在于相同的 Microsoft Dynamics 365 for Finance and Operations 数据库中或在单独的数据库中。
