...
在进行部分付款时，您可以计划进行部分付款以完全结算发票。 要为部分付款获取现金折扣，则必须设置<strong>应付帐款参数</strong>页上的<strong>计算部分付款的现金折扣</strong>选项为**是**。 
...
您可能要退回某些发票上的物料，并且接收贷方通知单。 如果现金折扣已在原始发票上执行，则可以减去折扣的值和接收退回的正确金额。 如果<strong>应付帐款参数</strong>页上的<strong>计算贷方通知单的现金折扣</strong>选项设置为**是**，则将自动计算贷方通知单的折扣。 
