...
**业务文档管理**建立在 ER 框架的基础上，可供业务用户使用 Microsoft Office 365 服务或相应 Microsoft Office 桌面应用程序编辑业务文档模板。 对此类文档的编辑包括在不执行源代码更改和不进行新部署的情况下，从 Dynamics 365 for Finance and Operations 内部更改业务文档设计和为更多数据添加占位符。 若要更新业务文档模板，无需了解 ER 框架。
...
若要使用业务文档管理和 Microsoft Office 桌面应用程序以 Excel 或 Word 格式编辑文档，则必须安装 Microsoft Office 2010 或更高版本。 Finance and Operations 的云部署和本地部署中支持此功能。
...
必须将包含业务文档模板的 ER 配置导入到当前 Finance and Operations 实例。 下载以下文件并存储在本地，以完成此过程。
...
1. 以具有**业务文档参数**页面访问权限的用户的身份登录 Finance and Operations。
...
默认情况下，在未启用业务文档管理权限的访问时，每位可访问业务文档管理工作区的用户都可以看到 Finance and Operations 中的所有可用 ER 解决方案模板。 业务文档管理工作区将仅显示 ER 格式配置中的模板和带有**业务文档类型**标记的模板。
...
1. 以具有**访问权限配置器**页面访问权限的用户的身份登录 Finance and Operations。
...
通过此设置，已为其分配了为执行**管理业务文档模板**（AOT 名称为 **ERBDManageTemplates**）职责而配置的任何安全角色的每位用户都可以打开业务文档管理工作区，并可编辑 Finance and Operations 中的所有可用模板。
...
> [!NOTE]
> **每个配置的访问权限**规则通过使用 ER 格式配置的唯一标识 ID 来存储。 这意味着如果删除引用这些规则的 ER 配置，不会删除这些规则。 将删除的配置导入回此 Finance and Operations 实例时，这些规则将再次引用这些配置。 再次导入删除的配置之后，无需再次设置规则。
...
1. 以具有业务文档管理工作区访问权限的用户的身份登录 Finance and Operations。
2. 打开业务文档管理工作区。
...
将打开 Finance and Operations 的**BDM 模板编辑器**页。 可通过使用 Office 365 在线编辑所选模板。
...
1. 在 Finance and Operations 中，更改为公司 **USMF**。
2. 转至**应收帐款** \> **发票** \> **所有普通发票**。
...
2. 如果选择**确定**以确认发布，将把包含修改后的模板的派生 **Customer FTI report (GER) Copy** ER 格式的草稿版本标记为已完成。 其他用户 Finance and Operations 将可使用修改后的模板。 此 ER 格式的完成版本将仅保留模板的最后一个有效版本。 将删除其他修订。 选择**取消**以浏览其他选项。
...
这是从 Office 365 重定向回 Finance and Operations 时的已知问题。 首次登录 Office 365 时会出现此情况。 若要解决此问题，请选择浏览器的**后退**按钮回到 Finance and Operations。
...
您当前登录的 Finance and Operations 实例所在 Azure AD 域很可能不是用于部署此 Finance and Operations 实例的 Azure AD 域。 因为 SharePoint 服务（用于存储模板来使其可通过使用 Office 桌面应用程序进行编辑）属于与 Finance and Operations 相同的域，所以我们无权访问 SharePoint 服务。 若要解决此问题，请使用具有正确 Azure AD 域的用户的凭据登录当前 Finance and Operations 实例。
