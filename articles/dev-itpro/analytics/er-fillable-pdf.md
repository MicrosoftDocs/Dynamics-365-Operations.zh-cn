...
本主题中的过程为示例，用于演示**系统管理员**角色或**电子申报开发人员**角色的用户如何通过将可填写 PDF 文档用作报表模板来配置用于生成 PDF 文件格式报表的电子申报 (ER) 格式。 Microsoft Dynamics 365 for Finance and Operations 或 Regulatory Configuration Services (RCS) 的任何公司都可以执行这些步骤。
