...
以下步骤介绍系统管理员或电子报表开发人员角色的 Regulatory Configuration Service (RCS) 用户如何使用 Dynamics 365 for Finance and Operations 应用程序的元数据设计新的电子报表 (ER) 模型映射。 可访问应用程序元数据，方法是使用其中包含一组示例元数据的 ER 配置访问外贸交易。 为了完成这些步骤，您必须首先在 RCS 中完成[创建配置提供程序并标记为当前运行的](er-configuration-provider-mark-it-active-2016-11.md)主题中的步骤。 然后在 Finance and Operations 中完成 [(ER) 在 RCS 中准备要使用的应用程序元数据](prepare-application-metadata-rcs.md)主题中的步骤。
...
1. 单击**元数据配置**。 
2. 导入包含来自 Finance and Operations 应用程序且为了生成外贸业务所用电子单据而配置的元数据的 ER 元数据配置。 由于已完成了 [(ER) 在 RCS 中准备要使用的应用程序元数据](prepare-application-metadata-rcs.md)过程中的步骤，所以此 ER 元数据配置已导出为 XML 文件。 
3. 单击**交换**。 
4. 单击**从 XML 文件加载**。 
...
37. 关闭该页面。 
38. 关闭该页面。 
39. 如果需要扩展现有元数据集，请在 Finance and Operations 中操作。 然后就可以从 Finance and Operations 中导出新的 ER 元数据配置已完成版本，导入到 RCS 中，然后更新引用新的所导入元数据配置版本的所配置模型映射配置的必备条件。 

> [!NOTE]
> 本地部署的应用程序（此时对 Dynamics 365 for Finance and Operations 使用本地业务数据 (LBD) 或本地部署的部署模型）只能使用这种方法获取有关应用程序元数据的信息。
