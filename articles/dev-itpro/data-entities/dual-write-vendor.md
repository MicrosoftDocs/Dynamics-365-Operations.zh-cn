...
*供应商*指的是供应链过程中为企业提供货物的供应商组织或独立经营者。 尽管*供应商*是 Microsoft Dynamics 365 for Finance and Operations 中的一个熟悉的概念，而 Dynamics 365 for Customer Engagement 中则不存在供应商这个概念。 相反，某些企业会重用客户实体来同时存储客户信息和供应商信息。 其他企业则使用自定义的供应商概念。 Common Data Service 集成同时支持这两种设计。 因此，您可以根据业务场景来启用其中一种设计。

Finance and Operations 与 Customer Engagement 之间的供应商数据集成让您可以对数据实现多主控处理。 无论供应商数据源自何处，都跨越应用程序边界和基础结构差异在后台集成。 

### <a name="vendor-data-flow"></a>供应商数据流

如果要使用 Customer Engagement 进行供应商主控，并且希望隔离供应商信息与客户信息，可以使用这种新的供应商设计。

![供应商数据流](media/dual-write-vendor-data-flow.png)

如果要使用 Customer Engagement 进行供应商主控，并且希望继续使用客户实体存储供应商信息，可以使用这种新的扩展供应商设计。 在这种设计中，扩展供应商信息（如供应商组和供应商过帐模板）存储在供应商供应商详细信息中。
...
使用供应商实体存储供应商信息的企业可以继续按照相同方法使用此实体。 还可以利用 Finance and Operations 集成带来的显式供应商功能。

## <a name="vendor-v2-and-msdyn_vendors"></a>供应商 V2 和 Msdyn\_vendors

使用适用于供应商的自定义解决方案的企业可以利用因 Finance and Operations 集成而在 Common Data Service 中引入的现成供应商概念。 
...
此模板在 Finance and Operations 与 Customer Engagement 之间同步客户和供应商的所有第一、第二和第三联系人信息。 有关实体映射的详细信息，请参阅[集成客户主控](dual-write-customer.md#contacts)。

## <a name="vendor-groups"></a>供应商组

此模板在 Finance and Operations 与 Customer Engagement 之间同步供应商组信息。
...
此模板在 Finance and Operations 与 Customer Engagement 之间同步供应商付款方式。
